...
static bool mrsh_group_extl(ExtlFn fn, WGroupParams *param) {
  ...
  MRSH_ANY(param, t);
  if (param->group != -1) extl_table_sets_i(t, "group", param->group + 1);
...
static bool mrsh_bell_extl(ExtlFn fn, WBellParams *param) {
  ...
  if (param->window) extl_table_sets_o(t, "window", (Obj *)param->window);
...
#define CHANGED(NM, FLD) \
  (kev->state.changed & XkbGroup##NM##Mask) ? kev->state.FLD : -1
...
bool handle_xkb_event(XEvent *ev) {
  ...
  if (ev->type != xkb_event_code) return FALSE;
  ...
  switch (kev->any.xkb_type) {
    ...
    case XkbBellNotify: {
      ...
      pbp.name = NULL;
      if (kev->bell.name != None) pbp.name = get_atom_name(kev->bell.name);
  ...
  if (hook && p && fn) hook_call_p(hook, p, fn);
...
void mod_xkbevents_deinit() { mod_xkbevents_unregister_exports(); }
...
bool mod_xkbevents_init() {
  ...
  if (!XkbLibraryVersion(&major, &minor)) {
    warn(TR("X library built with XKB version %d.%02d but mod_xkbevents was "
            "built with XKB version %d.%02d. Going to try to work anyway."),
         major, minor, XkbMajorVersion, XkbMinorVersion);
  }

  if (!XkbQueryExtension(ioncore_g.dpy, &opcode, &xkb_event_code,
                         &xkb_error_code, &major, &minor) > 0) {
    if ((major != 0) || (minor != 0))
      warn(TR("Server supports incompatible XKB version %d.%02d. Going to try "
              "to work anyway."),
           major, minor);
    else
  ...
  if (!mod_xkbevents_register_exports()) return FALSE;
