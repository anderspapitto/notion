...
bool debrush_init(DEBrush *brush, Window win, const GrStyleSpec *spec,
                  DEStyle *style) {
  brush->d = style;
...
static DEBrush *do_get_brush(Window win, WRootWin *rootwin,
  ...
  if (!gr_stylespec_load(&spec, stylename)) return NULL;
...
void debrush_get_border_widths(DEBrush *brush, GrBorderWidths *bdw) {
  ...
  switch (bd->style) {
    ...
    case DEBORDER_GROOVE:
      tmp = bd->sh + bd->hl + pad;
      bdw->top = tbf * tmp + spc;
      bdw->bottom = tbf * tmp + spc;
      bdw->left = lrf * tmp + spc;
      bdw->right = lrf * tmp + spc;
      break;
    ...
    default:
      tmp = bd->hl;
      bdw->top = tbf * tmp + pad;
      bdw->left = lrf * tmp + pad;
      tmp = bd->sh;
      bdw->bottom = tbf * tmp + pad;
      bdw->right = lrf * tmp + pad;
      break;
  }
...
bool debrush_get_extra(DEBrush *brush, const char *key, char type, void *data) {
  DEStyle *style = brush->d;
  while (style != NULL) {
    if (extl_table_get(style->extras_table, 's', type, key, data)) return TRUE;
