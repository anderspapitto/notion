...
static bool get_spec(ExtlTab tab, const char *name, GrStyleSpec *spec,
  ...
  if (!extl_table_gets_s(tab, name, &str)) return FALSE;
...
void de_get_border_style(uint *ret, ExtlTab tab) {
  ...
  if (!extl_table_gets_s(tab, "border_style", &style)) return;
...
void de_get_border_sides(uint *ret, ExtlTab tab) {
  ...
  if (!extl_table_gets_s(tab, "border_sides", &style)) return;
...
static bool de_get_colour_(WRootWin *rootwin, DEColour *ret, ExtlTab tab,
  ...
  if (!set) de_duplicate_colour(rootwin, substitute, ret);
...
void de_get_extra_cgrps(WRootWin *rootwin, DEStyle *style, ExtlTab tab) {
  ...
  if (style->extra_cgrps == NULL) return;

  for (i = 0; i < n - nfailed; i++) {
    GrStyleSpec spec;

    if (!extl_table_geti_t(tab, i + 1, &sub)) goto err;
    ...
    de_get_colour_group(rootwin, style->extra_cgrps + i - nfailed, sub, style);
...
void de_get_text_align(int *alignret, ExtlTab tab) {
  ...
  if (!extl_table_gets_s(tab, "text_align", &align)) return;
...
static const char *const known_values[] = {
    ...
    "highlight_pixels",  "padding_pixels", "border_style",
    "border_sides",      "spacing",        "foreground_colour",
    "background_colour", "shadow_colour",  "highlight_colour",
    "padding_colour",    "text_align",     NULL};

static bool filter_extras_iter_fn(ExtlAny k, ExtlAny v, void *p) {
  ExtlTab *tgt = (ExtlTab *)p;
  int i;

  if (k.type != 's' && k.type != 'S') return TRUE;

  for (i = 0; known_values[i]; i++) {
    if (strcmp(known_values[i], k.value.s) == 0) return TRUE;
  }

  if (*tgt == extl_table_none()) *tgt = extl_create_table();
...
bool de_defstyle_rootwin(WRootWin *rootwin, const char *name, ExtlTab tab) {
  ...
  if (based_on != NULL && gr_stylespec_equals(&based_on->spec, &style->spec)) {
    ...
    if (!based_on->is_fallback) destyle_dump(based_on);
...
bool de_defstyle(const char *name, ExtlTab tab) {
  ...
  FOR_ALL_ROOTWINS(rw) {
    if (!de_defstyle_rootwin(rw, name, tab)) ok = FALSE;
  }
