...
DEStyle *de_get_style(WRootWin *rootwin, const GrStyleSpec *spec) {
  ...
  for (style = styles; style != NULL; style = style->next) {
    if (style->rootwin != rootwin) continue;
...
void destyle_deinit(DEStyle *style) {
  ...
  if (style->cgrp_alloced) de_free_colour_group(style->rootwin, &(style->cgrp));

  for (i = 0; i < style->n_extra_cgrps; i++)
    de_free_colour_group(style->rootwin, style->extra_cgrps + i);

  if (style->extra_cgrps != NULL) free(style->extra_cgrps);
...
void destyle_add(DEStyle *style) { LINK_ITEM_FIRST(styles, style, next, prev); }
...
void de_reset() {
  ...
  for (style = styles; style != NULL; style = next) {
    next = style->next;
    if (!style->is_fallback) destyle_dump(style);
  }
}
