...
static void draw_borderline(Window win, GC gc, WRectangle *geom, uint tl,
  ...
  if (line == GR_BORDERLINE_LEFT && geom->h > 0 && tl > 0) {
    ...
    XFillRectangle(ioncore_g.dpy, win, gc, geom->x + geom->w - br, geom->y, br,
                   geom->h);
    geom->w -= br;
  } else if (line == GR_BORDERLINE_BOTTOM && geom->w > 0 && br > 0) {
    XSetForeground(ioncore_g.dpy, gc, PIXEL(brc));
    XSetBackground(ioncore_g.dpy, gc, PIXEL(brc));
    XFillRectangle(ioncore_g.dpy, win, gc, geom->x, geom->y + geom->h - br,
                   geom->w, br);
    geom->h -= br;
  }
}
...
void debrush_do_draw_padline(DEBrush *brush, WRectangle geom, DEColourGroup *cg,
                             GrBorderLine line) {
  DEBorder *bd = &(brush->d->border);
  GC gc = brush->d->normal_gc;
  Window win = brush->win;
...
void debrush_draw_borderline(DEBrush *brush, const WRectangle *geom,
  ...
  if (cg != NULL) debrush_do_draw_borderline(brush, *geom, cg, line);
}
...
void debrush_draw_border(DEBrush *brush, const WRectangle *geom) {
  DEColourGroup *cg = debrush_get_current_colour_group(brush);
  if (cg != NULL) debrush_do_draw_border(brush, *geom, cg);
}
...
static void copy_masked(DEBrush *brush, Drawable src, Drawable dst, int src_x,
                        int src_y, int w, int h, int dst_x, int dst_y) {
  GC copy_gc = brush->d->copy_gc;

  XSetClipMask(ioncore_g.dpy, copy_gc, src);
  XSetClipOrigin(ioncore_g.dpy, copy_gc, dst_x, dst_y);
  XCopyPlane(ioncore_g.dpy, src, dst, copy_gc, src_x, src_y, w, h, dst_x, dst_y,
             1);
}
...
void debrush_tab_extras(DEBrush *brush, const WRectangle *g, DEColourGroup *cg,
                        const GrBorderWidths *bdw, const GrFontExtents *fnte,
                        const GrStyleSpec *a1, const GrStyleSpec *a2, bool pre,
                        int index) {
  DEStyle *d = brush->d;
  GC tmp;
    ...
    if (cg != NULL) {
      ...
      if (s != NULL) {
        ...
        if (w < g->w - bdw->right - bdw->left) {
          ...
          int tx =
              (d->textalign == DEALIGN_RIGHT ? g->x + bdw->left
                                             : g->x + g->w - bdw->right - w);
  ...
  if (ISSET(a2, GR_ATTR(tagged)) || ISSET(a1, GR_ATTR(tagged))) {
    ...
    copy_masked(brush, d->tag_pixmap, brush->win, 0, 0, d->tag_pixmap_w,
                d->tag_pixmap_h, g->x + g->w - bdw->right - d->tag_pixmap_w,
                g->y + bdw->top);
  }
...
void debrush_menuentry_extras(DEBrush *brush, const WRectangle *g,
                              ...
                              const GrFontExtents *fnte, const GrStyleSpec *a1,
                              const GrStyleSpec *a2, bool pre,
                              int UNUSED(index)) {
  int tx, ty;
  ...
  if (ISSET(a2, GR_ATTR(submenu)) || ISSET(a1, GR_ATTR(submenu))) {
    ...
    debrush_do_draw_string(brush, tx, ty, DE_SUB_IND, DE_SUB_IND_LEN, FALSE,
                           cg);
  }
}
...
void debrush_do_draw_box(DEBrush *brush, const WRectangle *geom,
  ...
  if (TRUE /*needfill*/) {
    ...
    XFillRectangle(ioncore_g.dpy, brush->win, gc, geom->x, geom->y, geom->w,
                   geom->h);
  }
...
static void debrush_do_draw_textbox(DEBrush *brush, const WRectangle *geom,
                                    ...
                                    bool needfill, const GrStyleSpec *a1,
                                    const GrStyleSpec *a2, int index) {
  uint len;
  GrBorderWidths bdw;
  GrFontExtents fnte;
  uint tx, ty, tw;
...
void debrush_draw_textbox(DEBrush *brush, const WRectangle *geom,
  ...
  if (cg != NULL) {
    debrush_do_draw_textbox(brush, geom, text, cg, needfill, attr, NULL, -1);
  }
}

void debrush_draw_textboxes(DEBrush *brush, const WRectangle *geom, int n,
  ...
  for (i = 0;; i++) {
    ...
    if (bdw.spacing > 0 && needfill) {
      XClearArea(ioncore_g.dpy, brush->win, g.x, g.y, brush->d->spacing, g.h,
                 False);
    }
    g.x += bdw.spacing;
  }
...
void debrush_set_window_shape(DEBrush *brush, bool UNUSED(rough), int n,
  ...
  XShapeCombineRectangles(ioncore_g.dpy, brush->win, ShapeBounding, 0, 0, r, n,
                          ShapeSet, Unsorted);
}
...
void debrush_clear_area(DEBrush *brush, const WRectangle *geom) {
  XClearArea(ioncore_g.dpy, brush->win, geom->x, geom->y, geom->w, geom->h,
             False);
}
...
static void debrush_set_clipping_rectangle(DEBrush *brush,
  ...
  XSetClipRectangles(ioncore_g.dpy, brush->d->normal_gc, 0, 0, &rect, 1,
                     Unsorted);
  brush->clip_set = TRUE;
}
...
void debrush_begin(DEBrush *brush, const WRectangle *geom, int flags) {
  if (flags & GRBRUSH_AMEND) flags |= GRBRUSH_NO_CLEAR_OK;

  if (!(flags & GRBRUSH_KEEP_ATTR)) debrush_init_attr(brush, NULL);

  if (!(flags & GRBRUSH_NO_CLEAR_OK)) debrush_clear_area(brush, geom);

  if (flags & GRBRUSH_NEED_CLIP) debrush_set_clipping_rectangle(brush, geom);
}

void debrush_end(DEBrush *brush) { debrush_clear_clipping_rectangle(brush); }
