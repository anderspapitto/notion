...
static bool extl_stack_get(lua_State *st, int pos, char type, bool copystring,
                           bool *wasdeadobject, void *valret);
...
static Obj *extl_get_obj(lua_State *st, int pos, bool *invalid, bool *dead) {
  int val;
  ...
  *invalid = TRUE;
  ...
  if (!lua_getmetatable(st, pos)) return NULL;
...
static int extl_obj_gc_handler(lua_State *st) {
  ...
  if (proxy != NULL) EXTL_END_PROXY_OBJ(proxy, obj);

  if (EXTL_OBJ_OWNED(obj)) EXTL_DESTROY_OWNED_OBJ(obj);
...
static int extl_obj_typename(lua_State *st) {
  ...
  if (!extl_stack_get(st, 1, 'o', FALSE, NULL, &obj) || obj == NULL) return 0;
...
static int extl_current_file_or_dir(lua_State *st, bool dir) {
  ...
  if (lua_getstack(st, 1, &ar) != 1) goto err;
  if (lua_getinfo(st, "S", &ar) == 0) goto err;

  if (ar.source == NULL || ar.source[0] != '@') return 0; /* not a file */
...
static int extl_stack_trace(lua_State *st) {
  ...
  for (; lua_getstack(st, lvl, &ar); lvl++) {
    ...
    if (!is_c || ar.name != NULL) {
      ...
      if (ar.currentline != -1) lua_pushfstring(st, ":%d", ar.currentline);
      if (ar.name != NULL) lua_pushfstring(st, ": in '%s'", ar.name);
...
static int extl_do_collect_errors(lua_State *st) {
  ...
  if (err != 0) extl_warn("%s", lua_tostring(st, -1));

  if (el->msgs_len == 0) return 0;
...
static bool extl_stack_get(lua_State *st, int pos, char type, bool copystring,
  ...
  if (wasdeadobject != NULL) *wasdeadobject = FALSE;

  if (type == 'b') {
    if (valret) *((bool *)valret) = lua_toboolean(st, pos);
    return TRUE;
  }

  switch (lua_type(st, pos)) {
    case LUA_TNUMBER:
      if (type != 'i' && type != 'd' && type != 'a') return FALSE;

      d = lua_tonumber(st, pos);

      if (type == 'i') {
        if (d - floor(d) != 0) return FALSE;
        if (valret) *((int *)valret) = d;
      } else if (type == 'a') {
        if (valret) {
    ...
    case LUA_TNONE:
      if (type == 'a') {
        if (valret) ((ExtlAny *)valret)->type = 'v';
      } else if (type == 't' || type == 'f') {
        if (valret) *((int *)valret) = LUA_NOREF;
      } else if (type == 's' || type == 'S') {
        if (valret) *((char **)valret) = NULL;
      } else if (type == 'o') {
        if (valret) *((Obj **)valret) = NULL;
      } else {
        return FALSE;
      }
      return TRUE;
    ...
    case LUA_TSTRING:
      if (type != 's' && type != 'S' && type != 'a') return FALSE;
      if (valret) {
    ...
    case LUA_TUSERDATA:
      if (type == 'o' || type == 'a') {
        ...
        if (wasdeadobject != NULL) *wasdeadobject = dead;
        if (valret) {
          if (type == 'a') {
...
static void extl_to_any(ExtlAny *a, char type, void *ptr) {
  ...
  switch (type) {
    ...
    case 'S':
      ...
      break;
    case 't':
      a->value.t = *(ExtlTab *)ptr;
      break;
    case 'f':
      a->value.f = *(ExtlFn *)ptr;
      break;
  }
}
...
static void extl_to_any_vararg(ExtlAny *a, char type, va_list *argsp) {
  ...
  switch (type) {
    case 'i':
      a->value.i = va_arg(*argsp, int);
      break;
    case 'd':
      a->value.d = va_arg(*argsp, double);
      break;
    case 'b':
      a->value.b = va_arg(*argsp, bool);
      break;
    case 'o':
      a->value.o = va_arg(*argsp, Obj *);
      break;
    case 's':
    case 'S':
      a->value.s = va_arg(*argsp, char *);
      break;
    case 't':
      a->value.t = va_arg(*argsp, ExtlTab);
      break;
    case 'f':
      a->value.f = va_arg(*argsp, ExtlFn);
      break;
  }
}
...
static void extl_stack_pusha(lua_State *st, ExtlAny *a) {
  switch (a->type) {
    case 'i':
      lua_pushnumber(st, a->value.i);
      break;
    case 'd':
      lua_pushnumber(st, a->value.d);
      break;
    case 'b':
      lua_pushboolean(st, a->value.b);
      break;
    case 'o':
      extl_push_obj(st, a->value.o);
      break;
    case 's':
    case 'S':
      lua_pushstring(st, a->value.s);
      break;
    case 't':
      lua_rawgeti(st, LUA_REGISTRYINDEX, a->value.t);
      break;
    case 'f':
      lua_rawgeti(st, LUA_REGISTRYINDEX, a->value.f);
      break;
    default:
      lua_pushnil(st);
  }
...
static void extl_any_free(ExtlAny *a, int strings) {
  if ((a->type == 's' && strings != STRINGS_NONE) ||
    ...
    if (a->value.s != NULL) free((char *)a->value.s);
  } else if (a->type == 't') {
...
ExtlFn extl_fn_none() { return LUA_NOREF; }

ExtlTab extl_table_none() { return LUA_NOREF; }

/* ref */

static bool extl_do_ref(lua_State *st, int *refp) {
  if (!extl_getref(st, *refp)) return FALSE;
  *refp = luaL_ref(st, LUA_REGISTRYINDEX);
  return TRUE;
}

ExtlTab extl_ref_table(ExtlTab ref) {
  if (extl_cpcall(l_st, (ExtlCPCallFn *)extl_do_ref, &ref)) return ref;
  return LUA_NOREF;
}
...
ExtlTab extl_create_table() {
  ExtlTab ref;
  if (extl_cpcall(l_st, (ExtlCPCallFn *)extl_do_create_table, &ref)) return ref;
  return LUA_NOREF;
}
...
static bool extl_do_eq(lua_State *st, EqParams *ep) {
  if (!extl_getref(st, ep->o1)) return FALSE;
  if (!extl_getref(st, ep->o2)) return FALSE;
#if LUA_VERSION_NUM >= 502
...
static bool extl_table_do_geti(ExtlTab ref, int entry, char type,
                               void *valret) {
...
static bool extl_push_args(lua_State *st, const char *spec, va_list *argsp) {
  ...
  while (*spec != '\0') {
    if (!extl_stack_push_vararg(st, *spec, argsp)) return FALSE;
    i++;
    spec++;
  }
...
static bool extl_dodo_call_vararg(lua_State *st, ExtlDoCallParam *param) {
  ...
  if (n > 0) {
    if (!extl_push_args(st, param->spec, param->args)) return FALSE;
  }

  if (param->rspec != NULL) m = strlen(param->rspec);
  ...
  if (m > 0) return extl_get_retvals(st, m, param);
...
static bool extl_cpcall_call(lua_State *st, ExtlCPCallFn *fn,
  ...
  if (extl_cpcall(st, fn, param)) return TRUE;
...
static bool extl_do_call_vararg(lua_State *st, ExtlDoCallParam *param) {
  if (!extl_getref(st, *(ExtlFn *)(param->misc))) return FALSE;
  return extl_dodo_call_vararg(st, param);
}

bool extl_call_vararg(ExtlFn fnref, const char *spec, const char *rspec,
                      va_list *args) {
  ExtlDoCallParam param;

  if (fnref == LUA_NOREF || fnref == LUA_REFNIL) return FALSE;
...
static void l1_warn_handler(const char *message) {
  ...
  if (called == 0 && notrace == 0) ch->need_trace = TRUE;
...
static void flushtrace() {
  if (warnchain && warnchain->need_trace) do_trace(warnchain);
}
...
static int extl_l1_call_handler2(lua_State *st) {
  ...
  if (spec->untraced) notrace++;

  if (!spec->l2handler(spec->fn, param->ip, param->op)) return 0;
...
static int extl_l1_call_handler(lua_State *st) {
  ...
  if (ret != 0 || ch.need_trace) do_trace(&ch);
#else
  if (ret != 0) lua_error(st);
#endif
...
static int extl_protected(lua_State *st) {
  ...
  if (ret != 0) lua_pushboolean(st, TRUE);
  return 1;
}
...
static void extl_do_unregister_functions(ExtlExportedFnSpec *spec, int max,
  ...
  for (i = 0; spec[i].name && i < max; i++) {
    regdata.spec = &(spec[i]);
    extl_cpcall(l_st, (ExtlCPCallFn *)extl_do_unregister_function, &regdata);
...
static bool extl_do_register_class(lua_State *st, ClassData *data) {
  ...
  if (data->parent != NULL) {
    ...
    lua_pushvalue(st, -1);
    lua_insert(st, -3);
    /* Stack: cls, parent_meta, meta, parent_meta.__index, "__index",
     * parent_meta.__index */
...
void extl_unregister_class(const char *cls, ExtlExportedFnSpec *fns) {
  ...
  if (!extl_cpcall(l_st, (ExtlCPCallFn *)extl_do_unregister_class, &clsdata))
    return;
...
static bool extl_do_serialize(lua_State *st, SerData *d) {
  if (!extl_getref(st, d->tab)) return FALSE;
