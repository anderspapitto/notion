...
bool extl_set_searchpath(const char *path) {
  ...
  if (scriptpath != NULL) free(scriptpath);
...
bool extl_set_userdirs(const char *appname) {
  ...
  if (home == NULL) {
    extl_warn("$HOME not set");
  } else {
    libtu_asprintf(&userdir, "%s/.%s", home, appname);
    if (userdir != NULL) fails -= extl_add_searchdir(userdir);
...
bool extl_set_sessiondir(const char *session) {
  ...
  if (strchr(session, '/') != NULL) {
    ...
    extl_warn(
        "User directory not set. Unable to set session directory.");
    return FALSE;
  }

  if (tmp == NULL) return FALSE;

  if (sessiondir != NULL) free(sessiondir);
...
const char *extl_userdir() { return userdir; }
const char *extl_sessiondir() { return sessiondir; }
const char *extl_searchpath() { return scriptpath; }
...
static int do_try(const char *dir, const char *file, ExtlTryConfigFn *tryfn,
  ...
  if (tmp == NULL) return EXTL_TRYCONFIG_MEMERROR;
...
static int try_dir(const char *const *files, const char *cfdir,
  ...
  for (file = files; *file != NULL; file++) {
    ...
    if (ret >= 0) return ret;
    if (ret2 == EXTL_TRYCONFIG_NOTFOUND) ret2 = ret;
  }

  return ret2;
}

static int try_etcpath(const char *const *files, ExtlTryConfigFn *tryfn,
                       void *tryfnparam) {
...
static int try_load(const char *file, TryCallParam *param) {
  ...
  if (param->status == 1) extl_warn("Falling back to %s.", file);
...
static int try_call(const char *file, TryCallParam *param) {
  ...
  if (ret != EXTL_TRYCONFIG_OK) return ret;
...
int extl_try_config(const char *fname, const char *cfdir,
                    ExtlTryConfigFn *tryfn, void *tryfnparam, const char *ext1,
                    const char *ext2) {
  ...
  if (search) {
    ...
    if (ret == EXTL_TRYCONFIG_NOTFOUND) ret = ret2;
    if (ret < 0) ret = try_dir((const char **)&files, cfdir, tryfn, tryfnparam);
  } else {
...
char *extl_lookup_script(const char *file, const char *sp) {
  ...
  if (file != NULL) {
    ...
    if (sp != NULL) try_dir(files, sp, (ExtlTryConfigFn *)try_lookup, &tmp);
    if (tmp == NULL) try_etcpath(files, (ExtlTryConfigFn *)try_lookup, &tmp);
  }
...
bool extl_read_config(const char *file, const char *sp, bool warn_nx) {
  ...
  if (retval == EXTL_TRYCONFIG_NOTFOUND && warn_nx) {
    extl_warn("Unable to find '%s.%s' or '%s.%s' on search path...", file,
              EXTL_COMPILED_EXTENSION, file, EXTL_EXTENSION);
...
ExtlTab extl_extl_read_savefile(const char *basename) {
  ExtlTab tab;
  if (!extl_read_savefile(basename, &tab)) return extl_table_none();
  return tab;
}
...
char *extl_get_savefile(const char *basename) {
  ...
  if (!ensuredir(sessiondir)) {
    extl_warn("Unable to create session directory \"%s\".", sessiondir);
    return NULL;
  }
