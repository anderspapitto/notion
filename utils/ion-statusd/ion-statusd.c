...
static OptParserOpt ion_opts[] = {
    ...
    {'q', "quiet", 0, NULL, DUMMY_TR("Quiet mode")},

    {'m', "meter", OPT_ARG, "meter_module", DUMMY_TR("Load a meter module")},
...
int main(int argc, char *argv[]) {
...
#ifndef CF_NO_LOCALE
  if (setlocale(LC_ALL, "") == NULL) warn("setlocale() call failed.");
#endif
  ...
  if (!statusd_register_exports()) return EXIT_FAILURE;
    ...
    switch (opt) {
      ...
      case 'm':
        ...
        if (strchr(mod, '/') == NULL && strchr(mod, '.') == NULL) {
          ...
          if (mod2 == NULL) return EXIT_FAILURE;
          mod = mod2;
        }
        if (extl_read_config(mod, NULL, !quiet)) loaded++;
        if (mod2 != NULL) free(mod2);
        break;
      default:
...
ExtlTab statusd_getloadavg() {
  ...
  if (n >= 1) extl_table_sets_d(t, "1min", l[0]);
  if (n >= 2) extl_table_sets_d(t, "5min", l[1]);
  if (n >= 3) extl_table_sets_d(t, "15min", l[2]);
#endif
  return t;
}
