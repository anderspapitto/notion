  ...
  if (hasXrandR && ev->type == xrr_event_base + RRScreenChangeNotify) {
    ...
    if (screen != NULL) {
      ...
      if (!found) {
        ...
        fp.rotation =
            (r > or ? SCREEN_ROTATION_0 + r - or : (SCREEN_ROTATION_270 + 1) + r - or );
        node->v.ival = r;
      }
...
static bool check_pivots() {
  ...
  FOR_ALL_SCREENS(scr) {
    ...
    int randr_screen_id =
        XRRRootToScreen(ioncore_g.dpy, ((WMPlex *)scr)->win.win);
...
ExtlTab mod_xrandr_get_all_outputs() {
  ...
  XRRScreenResources *res =
      XRRGetScreenResources(ioncore_g.dpy, ioncore_g.rootwins->dummy_win);
  ExtlTab result = extl_create_table();

  for (i = 0; i < res->noutput; i++) {
    XRROutputInfo *output_info =
        XRRGetOutputInfo(ioncore_g.dpy, res, res->outputs[i]);
    if (output_info->crtc != None) {
      XRRCrtcInfo *crtc_info =
          XRRGetCrtcInfo(ioncore_g.dpy, res, output_info->crtc);
