...
static void receive_data(int fd, void *buf_) {
  ...
  for (i = 0; i < n; i++) {
    if (buf->data[buf->ndata + i] == '\0') end = TRUE;
  }
...
static void connection_attempt(int lfd, void *UNUSED(data)) {
  ...
  /* unblock */ {
    ...
    if (fl != -1) fl = fcntl(fd, F_SETFL, fl | O_NONBLOCK);
    if (fl == -1) {
      warn_err();
      close(fd);
      return;
  ...
  /* close socket on exec */ {
    ...
    if (fl != -1) fl = fcntl(fd, F_SETFD, fl | FD_CLOEXEC);
    if (fl == -1) {
      warn_err();
      close(fd);
      return;
...
static bool start_listening() {
  ...
  if (listenfd < 0) goto errwarn;

  if (fchmod(listenfd, S_IRUSR | S_IWUSR) < 0) goto errwarn;
...

  {
    ...
    if (fl != -1) fl = fcntl(listenfd, F_SETFD, fl | FD_CLOEXEC);
    if (fl == -1) goto errwarn;
  }
  ...
  if (listen(listenfd, MAX_SERVED) < 0) goto errwarn;

  if (!mainloop_register_input_fd(listenfd, NULL, connection_attempt)) goto err;
...
void close_connections() {
  ...
  for (i = 0; i < MAX_SERVED; i++) {
    if (bufs[i].fd >= 0) close_conn(&(bufs[i]));
  }
...
bool mod_notionflux_init() {
  ...
  if (!extl_loadstring(tostringstr, &tostringfn)) return FALSE;
  ...
  flux_socket =
      XInternAtom(ioncore_g.dpy, "_NOTION_MOD_NOTIONFLUX_SOCKET", False);

  FOR_ALL_ROOTWINS(rw) {
    xwindow_set_string_property(region_xwindow((WRegion *)rw), flux_socket,
                                listenfile);
  }
...
void mod_notionflux_deinit() {
  ...
  if (flux_socket != None) {
    FOR_ALL_ROOTWINS(rw) {
      XDeleteProperty(ioncore_g.dpy, region_xwindow((WRegion *)rw),
                      flux_socket);
    }
  }
