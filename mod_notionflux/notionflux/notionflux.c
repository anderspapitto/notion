...
static void mywrite(int fd, const char *buf, int n) {
  while (n > 0) {
    int k;
    k = write(fd, buf, n);
    if (k < 0 && (errno != EAGAIN && errno != EINTR)) die_e("Writing");
    if (k > 0) {
      n -= k;
      buf += k;
    }
...
static ulong xwindow_get_property_(Window win, Atom atom, Atom type,
  ...
  do {
    status = XGetWindowProperty(dpy, win, atom, 0L, n32expected, False, type,
...
void xwindow_set_string_property(Window win, Atom a, const char *value) {
  if (value == NULL) {
    ...
    XChangeProperty(dpy, win, a, XA_STRING, 8, PropModeReplace, (uchar *)value,
                    strlen(value));
  }
}
...
static char *get_socket() {
  ...
  if (dpy == NULL) die_e("Unable to open display.");

  a = XInternAtom(dpy, "_NOTION_MOD_NOTIONFLUX_SOCKET", True);

  if (a == None) die_e("Missing atom. Notion not running?");
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1) {
    ...
    if (strlen(argv[2]) >= MAX_DATA) die("Too much data.");

    use_stdin = 0;
  }

  sockname = get_socket();
  if (sockname == NULL) die("No socket.");

  if (strlen(sockname) > SOCK_MAX) die("Socket name too long.");

  sock = socket(AF_UNIX, SOCK_STREAM, 0);
  if (sock < 0) die_e("Opening socket");
  ...
  if (!use_stdin) {
    ...
    char c = '\0';
    while (1) {
      if (fgets(buf, MAX_DATA, stdin) == NULL) break;
  ...
  if (n != 1 || (res != 'E' && res != 'S')) die("Invalid response");
