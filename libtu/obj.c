...
void destroy_obj(Obj *obj) {
  ...
  if (OBJ_IS_BEING_DESTROYED(obj)) return;
...
bool obj_is_str(const Obj *obj, const char *str) {
  ...
  while (d != NULL) {
    if (strcmp(d->name, str) == 0) return TRUE;
    d = d->ancestor;
  }
  return FALSE;
}
...
DynFun *lookup_dynfun(const Obj *obj, DynFun *func, bool *funnotfound) {
  ClassDescr *descr;
  DynFunTab *df;
  ...
  for (; descr != &Obj_classdescr; descr = descr->ancestor) {
    ...
    if (descr->funtab_n == -1) {
      ...
      if (descr->funtab_n > 0) {
        qsort(descr->funtab, descr->funtab_n, sizeof(DynFunTab), comp_fun);
      }
    }
...
bool watch_setup(Watch *watch, Obj *obj, WatchHandler *handler) {
  if (OBJ_IS_BEING_DESTROYED(obj)) return FALSE;
...
void do_watch_reset(Watch *watch, bool call) {
  ...
  if (call && handler != NULL) handler(watch, obj);
}

void watch_reset(Watch *watch) { do_watch_reset(watch, FALSE); }

bool watch_ok(Watch *watch) { return watch->obj != NULL; }
...
void watch_call(Obj *obj) { do_watches(obj, FALSE); }
