...
bool objlist_reinsert_last(ObjList **objlist, Obj *obj) {
  ...
  if (node == NULL) return objlist_insert_last(objlist, obj);
...
bool objlist_remove(ObjList **objlist, Obj *obj) {
  ...
  if (node != NULL) free_node(objlist, node);
...
void objlist_clear(ObjList **objlist) {
  while (*objlist != NULL) free_node(objlist, *objlist);
}
...
bool objlist_empty(ObjList *objlist) {
  ...
  FOR_ALL_ON_OBJLIST(Obj *, obj, objlist, tmp) { return FALSE; }
