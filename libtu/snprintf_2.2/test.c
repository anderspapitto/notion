...
int main(int argc, char *argv[]) {
  ...
  if (0) {
    ...
    for (breakpoint = 0; breakpoint <= 35; breakpoint++) {
      ...
      t = clock();
      printf(" %1.3f", (float)(t - t0) / CLOCKS_PER_SEC);
      t0 = t;
      for (j = 10 * cnt; j > 0; j--) {
    ...
    p[1] = 's';
    for (j = cnt; j > 0; j--)
      l1 = portable_snprintf(NULL, (size_t)0, p, "1234567890");
    t = clock();
    printf("t_port_nul = %1.3f s\n", (float)(t - t0) / CLOCKS_PER_SEC);
    t0 = t;
    for (j = cnt; j > 0; j--)
      l2 = portable_snprintf(str, (size_t)8, p, "1234567890");
    t = clock();
    printf("t_port     = %1.3f s\n", (float)(t - t0) / CLOCKS_PER_SEC);
    t0 = t;
    assert(l1 == l2);
    p[0] = p[1] = 'h';
    ...
    for (j = cnt; j > 0; j--)
      l1 = portable_snprintf(NULL, (size_t)0, "%.18000s", p);
    t = clock();
    printf("t_port_nul = %1.3f s\n", (float)(t - t0) / CLOCKS_PER_SEC);
    t0 = t;
    for (j = cnt; j > 0; j--)
      l2 = portable_snprintf(str, sizeof(str), "%.18000s", p);
    t = clock();
    printf("t_port     = %1.3f s\n", (float)(t - t0) / CLOCKS_PER_SEC);
    t0 = t;
    assert(l1 == l2);
    ...
    for (j = cnt; j > 0; j--)
      l1 = portable_snprintf(NULL, (size_t)0, "%018000d", 1);
    t = clock();
    printf("t_port_nul = %1.3f s\n", (float)(t - t0) / CLOCKS_PER_SEC);
    t0 = t;
    for (j = cnt; j > 0; j--)
      l2 = portable_snprintf(str, sizeof(str), "%018000d", 1);
    t = clock();
    printf("t_port     = %1.3f s\n", (float)(t - t0) / CLOCKS_PER_SEC);
    t0 = t;
    assert(l1 == l2);
    ...
    printf(
        "\nmeasuring system's sprintf to efficiently handle truncated "
        "strings\n");
    memset(p, 'h', size);
    p[size - 1] = '\0';
    t0 = clock();
  ...
  /* second preliminary halfhearted test */
  {
    ...
    /*  len2 = sprintf (str2,               "%.*s", 4294967296U,"13"); */ /* core
                                                                             dumps
                                                                             on
                                                                             HPUX
                                                                             */
  ...
  if (argc >= 3) {
    ...
    const char fspec[] =
        "scdpoxXuiy"; /* set of test formats (including '\0') */
    ...
    const long int iargs[] = /* set of numeric test arguments */
        {0,    1,    9,     10,    28,    99,    100,    127,    128,   129,
         998,  1000, 32767, 32768, 32769, -1,    -9,     -10,    -28,   -99,
         -100, -127, -128,  -129,  -998,  -1000, -32767, -32768, -32769};
    ...
    for (fs = 0; fs <= fspec_l; fs++) { /* format specifier */
      ...
      for (fw = -1; fw <= 3; fw++) { /* minimal field width */
        ...
        for (fp = -2; fp <= 3; fp++) { /* format field precision */
          ...
          /* data type modifiers */
          for (dt = 0; dt < ((strtype || fspec[fs] == 'c') ? 1 : datatype_l);
               dt++) {
                  ...
                  for (fl4 = 0; fl4 <= flags_l; fl4++) {
                    for (fl5 = 0; fl5 <= flags_l; fl5++) {
                      for (a = 0; a < args_l; a++) { /* test arguments */
                        ...
                        if (a == 0 && fl1 == flags_l && fl2 == flags_l &&
                            fl3 == flags_l && fl4 == flags_l && fl5 == flags_l)
                          printf("%s\n", fmt);
                        ...
                        if (strtype) {
                          ...
                          len1 = portable_snprintf(str1, str_m, fmt,
                                                   (void *)iargs[a]);
                          ...
                          switch (dataty) {
                            ...
                            case '2':
                              len1 = portable_snprintf(str1, str_m, fmt,
                                                       (long long int)iargs[a]);
                              len2 =
                                  sprintf(str2, fmt, (long long int)iargs[a]);
#ifdef HAVE_SNPRINTF
                              len3 = snprintf(str3, str_m, fmt,
                                              (long long int)iargs[a]);
#endif
                        ...
                        if (0) {
                          ...
                          printf("sys snprintf: |%s|  len = %d\n", str3, len3);
                                   ...
                                   (len1 > 0 &&
                                    memcmp(str1, str2, min(len1, str_m) - 1) !=
                                        0)) {
                          bad = 1;
                          if (strtype)
                            printf("\n1: %s, <%s>\n", fmt, sargs[a]);
