...
#define sibling(n) \
  ((isleft(n)) ? n->p.parent->c.child.right : n->p.parent->c.child.left)

static void insert(Rb_node item,
                   Rb_node list) /* Inserts to the end of a list */
{
  Rb_node last_node;
...
void rb_delete_node(Rb_node n) {
  ...
  p = n->p.parent;
  s = sibling(n);
  while (isblack(p) && isblack(s) && isint(s) && isblack(s->c.child.left) &&
         isblack(s->c.child.right)) {
    setred(s);
    n = p;
    if (isroot(n)) return;
    p = n->p.parent;
...
void rb_iprint_tree(Rb_node t, int level) {
  ...
  if (ishead(t) && t->p.parent == t) {
    ...
    printf("Head: %p.  Root = %p, < = %p, > = %p\n", DONT_COMPLAIN t,
           DONT_COMPLAIN t->p.root, DONT_COMPLAIN t->c.list.blink,
...
void *rb_val(Rb_node n) { return n->v.val; }
