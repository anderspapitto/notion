...
void verbose_enable(bool enable) { verbose_mode = enable; }

int verbose_indent(int depth) {
  int old = verbose_indent_lvl;

  if (depth >= 0) verbose_indent_lvl = depth;

  return old;
}

void warn_progname_enable(bool enable) { progname_enable = enable; }
...
void warn(const char *p, ...) { CALL_V(warn_v, (p, args)); }
...
void warn_v(const char *p, va_list args) { DO_DISPATCH(errmsg_v, (p, args)); }

void warn_obj_line_v(const char *obj, int line, const char *p, va_list args) {
  DO_DISPATCH(errmsg_obj_line_v, (obj, line, p, args));
}

void warn_err() { DO_DISPATCH(errmsg_err, ()); }

void warn_err_obj(const char *obj) { DO_DISPATCH(errmsg_err_obj, (obj)); }
...
char *errmsg(const char *p, ...) { CALL_V_RET(errmsg_v, (p, args)); }
...
char *errmsg_obj_line_v(const char *obj, int line, const char *p,
  ...
  if (obj != NULL) {
    ...
    if (line > 0) libtu_asprintf(&res1, "%d: ", line);
  }
