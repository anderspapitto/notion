...
static ConfOpt common_opts[] = {{"include", "s", opt_include, NULL},
                                {NULL, NULL, NULL, NULL}};
...
static int read_statement(Tokenizer *tokz, Token *tokens, int *ntok_ret) {
  ...
  while (1) {
    ...
    if (ntokens == MAX_TOKENS - 1) {
      ...
      if (!(tokz->flags & TOKZ_ERROR_TOLERANT)) break;
    } else {
      ntokens++;
    }
  ...
  handle_error:
    ...
    if (!(tokz->flags & TOKZ_ERROR_TOLERANT) || retval != 0) break;
  }

end:
  if (e != 0) retval = -retval;
...
static bool find_beg_sect(Tokenizer *tokz) {
  ...
  while (tokz_get_token(tokz, &tok)) {
    if (TOK_IS_OP(&tok)) {
      if (TOK_OP_VAL(&tok) == OP_NEXTLINE) continue;

      if (TOK_OP_VAL(&tok) == OP_SCOLON) return FALSE;

      if (TOK_OP_VAL(&tok) == OP_L_BRC) return TRUE;
    }
...
static const ConfOpt *lookup_option(const ConfOpt *opts, const char *name) {
  while (opts->optname != NULL) {
    if (strcmp(opts->optname, name) == 0) return opts;
    opts++;
  }
  return NULL;
}
...
static bool call_end_sect(Tokenizer *tokz, const ConfOpt *opts) {
  ...
  if (opts != NULL) return opts->fn(tokz, 0, NULL);
...
bool parse_config_tokz(Tokenizer *tokz, const ConfOpt *options) {
  ...
  for (i = 0; i < MAX_TOKENS; i++) tok_init(&tokens[i]);

  while (1) {
    had_error = FALSE;

    /* free the tokens */
    while (ntokens--) tok_free(&tokens[ntokens]);

    /* read the tokens */
    t = read_statement(tokz, tokens, &ntokens);

    if ((had_error = t < 0)) t = -t;
    ...
    switch (t) {
      ...
      case P_STMT_SECT:
        ...
        if (!TOK_IS_IDENT(tokens + 0)) {
          had_error = TRUE;
          tokz_warn_error(tokz, tokens->line, E_TOKZ_IDENTIFIER_EXPECTED);
        }

        if (t == P_STMT) {
          if (find_beg_sect(tokz)) t = P_STMT_SECT;
        }
        ...
        if (!had_error && options->fn != NULL) {
          ...
          if (t == P_STMT_SECT && had_error) tokz->nest_lvl--;
        }
        break;
      ...
      case P_END_SECT:
        ...
        if (tokz->nest_lvl < init_nest_lvl) goto eof;
    }

    if (!had_error) continue;

    if (t == P_STMT_SECT) errornest++;
...
eof:
  ...
  if (tokz->flags & TOKZ_PARSER_INDENT_MODE) verbose_indent(init_nest_lvl);
...
again:
  ...
  while (*p != '\0') {
    ...
    if (e == 0) {
      p++;
      while (mode == 2 && *p == ':') {
        if (*++p == '\0') break; /* Invalid argument format string, though... */
        p++;
      }
      *pret = p;
      return 0;
...
static bool opt_include(Tokenizer *tokz, int UNUSED(n), Token *toks) {
  ...
  if (fname[0] != '/' && tokz->name != NULL) lastndx = strrchr(tokz->name, '/');
  ...
  if (tokz->includepaths != NULL) {
    while (tokz->includepaths[i] != NULL) {
      if (try_include_dir(tokz, tokz->includepaths[i], -1, fname)) return TRUE;
      i++;
    }
  }
...
ConfOpt libtu_dummy_confopts[] = {END_CONFOPTS};
