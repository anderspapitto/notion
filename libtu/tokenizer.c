...
#define STRING_DECL(X)   \
  int err = 0;           \
  char *X = NULL;        \
  char X##_tmp[STRBLEN]; \
  int X##_tmpl = 0
#define STRING_DECL_P(X, P)  \
  int err = 0;               \
  char *X = NULL;            \
  char X##_tmp[STRBLEN] = P; \
  int X##_tmpl = sizeof(P) - 1
#define STRING_APPEND(X, C)                                        \
  {                                                                \
    if (!_string_append(&X, X##_tmp, &X##_tmpl, c)) err = -ENOMEM; \
  }
#define STRING_FREE(X) \
  if (X != NULL) free(X)
#define STRING_FINISH(X)                                       \
  {                                                            \
    if (err != 0) return err;                                  \
    if (!_string_finish(&X, X##_tmp, X##_tmpl)) err = -ENOMEM; \
  }
...
static void _ungetch(Tokenizer *tokz, int c) { tokz->ungetc = c; }
...
static int skip_c_comment(Tokenizer *tokz) {
  ...
  while (1) {
    ...
    if (c == EOF) return E_TOKZ_UNEXPECTED_EOF;
...
static int scan_char_escape(Tokenizer *tokz) {
  ...
  for (i = 0; special_chars[i]; i++) {
    if (special_chars[i] == c) return specials[c];
  }
...
static int scan_char(Token *tok, Tokenizer *tokz) {
  ...
  if (c == '\\') {
    ...
    if (c == -2) return E_TOKZ_UNEXPECTED_EOL;
  }

  c2 = GETCH();

  if (c2 != '\'') return E_TOKZ_MULTICHAR;
...
static int scan_number(Token *tok, Tokenizer *tokz, int c) {
  ...
  if ((e = parse_number(&num, tokz, c))) return e;

  if (num.type == NPNUM_INT) {
    long l;
    if ((e = num_to_long(&l, &num, TRUE))) return e;

    TOK_SET_LONG(tok, l);
  } else if (num.type == NPNUM_FLOAT) {
    double d;
    if ((e = num_to_double(&d, &num))) return e;
...
static bool is_opch(uint ch) { return map_isset(op_map, ch); }
...
static int scan_op(Token *tok, Tokenizer *tokz, int c) {
  ...
  if (!is_opch(c)) return E_TOKZ_INVALID_CHAR;
...
void tokz_warn_error(const Tokenizer *tokz, int line, int e) {
  if (e == E_TOKZ_UNEXPECTED_EOF) line = 0;
...
bool tokz_get_token(Tokenizer *tokz, Token *tok) {
  ...
  if (!(tokz->flags & TOKZ_READ_FROM_BUFFER)) assert(tokz->file != NULL);
  ...
  while (1) {
    ...
    switch (c) {
      ...
      case '\n':
        ...
        if (tokz->flags & TOKZ_IGNORE_NEXTLINE) continue;
      ...
      case '\\':
        do {
          ...
          if (!isspace(c) && e == 0) {
            ...
            if (!(tokz->flags & TOKZ_ERROR_TOLERANT)) return FALSE;
          }
        } while (c != '\n');
...
static bool do_tokz_pushf(Tokenizer *tokz) {
  ...
  finfo = REALLOC_N(tokz->filestack, Tokenizer_FInfo, tokz->filestack_n,
                    tokz->filestack_n + 1);
...
bool tokz_pushf_file(Tokenizer *tokz, FILE *file, const char *fname) {
  ...
  if (tokz->file != NULL) {
    if (!do_tokz_pushf(tokz)) {
      warn_err();
      if (fname_copy != NULL) free(fname_copy);
      return FALSE;
    }
  }
...
static bool do_tokz_popf(Tokenizer *tokz, bool shrink) {
  ...
  if (tokz->filestack_n <= 0) return FALSE;

  if (tokz->file != NULL) fclose(tokz->file);
  if (tokz->name != NULL) free(tokz->name);
  ...
  if (tokz->filestack_n == 0) {
    ...
    finfo = REALLOC_N(tokz->filestack, Tokenizer_FInfo, tokz->filestack_n + 1,
                      tokz->filestack_n);
    if (finfo == NULL)
      warn_err();
    else
...
bool tokz_popf(Tokenizer *tokz) { return do_tokz_popf(tokz, TRUE); }

void tokz_close(Tokenizer *tokz) {
  while (tokz->filestack_n > 0) do_tokz_popf(tokz, FALSE);
...
void tok_free(Token *tok) {
  if (TOK_IS_STRING(tok) || TOK_IS_IDENT(tok) || TOK_IS_COMMENT(tok)) {
    if (TOK_STRING_VAL(tok) != NULL) free(TOK_STRING_VAL(tok));
  }
