...
static const OptParserOpt *find_chain_opt(char p, const OptParserOpt *o) {
  for (; O_ID(o); o++) {
    if ((O_ID(o) & ~OPT_ID_RESERVED_FLAG) == p) return o;
  }
  return NULL;
}
...
#define RET(X) return o_tmp = p, o_error = X
  ...
  while (o_args_left) optparser_get_arg();
  ...
  if (o_chain_ptr != NULL) {
    p = o_chain_ptr++;
    if (!*o_chain_ptr) o_chain_ptr = NULL;

    o = find_chain_opt(*p, o_opts);

    if (o == NULL) RET(E_OPT_INVALID_CHAIN_OPTION);

    goto do_arg;
  }

  if (o_left < 1) return OPT_ID_END;
  ...
  if (*p != '-') {
    dash = FALSE;
    if (o_mode != OPTP_NO_DASH) RET(OPT_ID_ARGUMENT);
    p2 = p;
  } else if (*(p + 1) == '-') {
    /* --foo */
    if (*(p + 2) == '\0') {
    ...
    if (*(p + 2) != '\0' && o_mode == OPTP_MIDLONG) type = MIDLONG;
  ...
  for (; O_ID(o); o++) {
    if (type == LONG) {
      ...
      if (o->longopt == NULL) continue;
      l = strlen(o->longopt);
      if (strncmp(p2, o->longopt, l) != 0) continue;

      if (p2[l] == '\0') {
        if (O_ARGS(o) == OPT_ARG) RET(E_OPT_MISSING_ARGUMENT);
        return O_ID(o);
      } else if (p2[l] == '=') {
        if (!O_ARGS(o)) RET(E_OPT_UNEXPECTED_ARGUMENT);
        if (p2[l + 1] == '\0') RET(E_OPT_MISSING_ARGUMENT);
        o_tmp = p2 + l + 1;
        o_args_left = 1;
        return O_ID(o);
      }
      ...
      if (strcmp(p2, o->longopt) != 0) continue;
    } else { /* type==SHORT */
      if (*p2 != (O_ID(o) & ~OPT_ID_RESERVED_FLAG)) continue;
      ...
      if (*(p2 + 1) != '\0') {
        if (o_mode == OPTP_CHAIN || o_mode == OPTP_NO_DASH) {
          ...
          if (!O_ARGS(o)) {
            if (*(p2 + 1) != '\0') RET(E_OPT_UNEXPECTED_ARGUMENT);
          } else {
            if (*(p2 + 1) == '\0') RET(E_OPT_MISSING_ARGUMENT);
            o_tmp = p2 + 1;
            o_args_left = 1;
          }
          return O_ID(o);
  ...
  do_arg:
    ...
    if (!O_ARGS(o)) return O_ID(o);

    if (!o_left || is_option(*o_current)) {
      if (O_ARGS(o) == OPT_OPT_ARG) return O_ID(o);
  ...
  if (dash) RET(E_OPT_INVALID_OPTION);
...
const char *optparser_get_arg() {
  ...
  if (o_tmp != NULL) {
     ...
     */
    if (o_args_left > 0) o_args_left--;
    p = o_tmp;
    o_tmp = NULL;
    return p;
  }

  if (o_args_left < 1 || o_left < 1) return NULL;
...
static uint opt_w(const OptParserOpt *opt, bool midlong) {
  ...
  if ((opt->optid & OPT_ID_NOSHORT_FLAG) == 0) {
    w += 2;                           /* "-o" */
    if (opt->longopt != NULL) w += 2; /* ", " */
  }

  if (opt->longopt != NULL) w += strlen(opt->longopt) + (midlong ? 1 : 2);
...
static void print_opt(const OptParserOpt *opt, bool midlong, uint maxw,
                      uint tw) {
  FILE *f = stdout;
  const char *p, *p2, *p3;
  uint w = 0;
...
void optparser_printhelp(int mode, const OptParserOpt *opts) {
  ...
  for (; O_ID(o); o++) print_opt(o, midlong, maxw, TERM_W);
}
