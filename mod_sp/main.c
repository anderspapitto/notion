...
static WRegion *create(WMPlex *mplex, int flags) {
  ...
  par.flags = (flags | MPLEX_ATTACH_UNNUMBERED | MPLEX_ATTACH_SIZEPOLICY |
               MPLEX_ATTACH_PSEUDOMODAL);
  par.szplcy = SIZEPOLICY_FULL_EXACT;

  sp = mplex_do_attach_new(mplex, &par, create_scratchws, NULL);

  if (sp == NULL) warn(TR("Unable to create scratchpad."));
...
static bool is_scratchpad(WRegion *reg) {
  ...
  return (inst_off < 0
              ? (strcmp(nm, SP_NAME) == 0 || strcmp(nm, SPWS_NAME) == 0)
...
EXTL_SAFE
EXTL_EXPORT
bool mod_sp_is_scratchpad(WRegion *reg) { return is_scratchpad(reg); }
...
bool mod_sp_create_scratchpad(WScreen *scr) {
  ...
  FOR_ALL_MANAGED_BY_MPLEX((WMPlex *)scr, reg, tmp) {
    if (is_scratchpad(reg)) return TRUE;
  }
...
bool mod_sp_set_shown(WFrame *sp, const char *how) {
  if (sp != NULL) {
    ...
    if (mplex != NULL) return mplex_set_hidden(mplex, (WRegion *)sp, setpar);
  }
...
void mod_sp_deinit() { mod_sp_unregister_exports(); }
...
bool mod_sp_init() {
  if (!mod_sp_register_exports()) return FALSE;
