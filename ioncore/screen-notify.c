...
static WInfoWin *do_get_popup_win(WScreen *scr, Watch *watch, uint pos,
  ...
  if (iw == NULL) {
    ...
    param.flags =
        (MPLEX_ATTACH_UNNUMBERED | MPLEX_ATTACH_SIZEPOLICY | MPLEX_ATTACH_GEOM |
         MPLEX_ATTACH_LEVEL | MPLEX_ATTACH_PASSIVE);
    ...
    iw = (WInfoWin *)mplex_do_attach_new(
        &scr->mplex, &param, (WRegionCreateFn *)create_infowin, style);

    if (iw != NULL) watch_setup(watch, (Obj *)iw, NULL);
  }
...
static bool ws_mapped(WScreen *scr, WRegion *reg) {
  while (1) {
    ...
    if (mgr == (WRegion *)scr) return REGION_IS_MAPPED(reg);
...
static void screen_managed_activity(WScreen *scr) {
  ...
  FOR_ALL_ON_OBJLIST(WRegion *, reg, actlist, tmp) {
    if (!ioncore_g.activity_notification_on_all_screens &&
      ...
      continue;
    if (region_name(reg) == NULL) continue;
    if (ptrlist_insert_last(&found, reg)) nfound++;
  }

  if (found == NULL) goto unnotify;
  ...
  FOR_ALL_ON_PTRLIST(WRegion *, reg, found, tmp2) {
    ...
    if (label != NULL) {
      nstr = (nadded > 0 ? scat3(notstr, ", ", label) : scat(notstr, label));
...
static void screen_do_update_notifywin(WScreen *scr) {
  if (ioncore_g.activity_notification_on_all_screens) {
    ...
    FOR_ALL_SCREENS(scr_iterated) { _screen_do_update_notifywin(scr_iterated); }
  } else
...
static void screen_do_update_infowin(WScreen *scr) {
  ...
  bool act =
      (reg != NULL && region_is_activity_r(reg) && !REGION_IS_ACTIVE(scr));
...
static WInfoWin *get_workspace_indicatorwin(WScreen *scr) {
  return do_get_popup_win(scr, &scr->workspace_indicatorwin_watch,
                          MPLEX_STDISP_BL, "tab-info");
}

void screen_unnotify_workspace_indicatorwin(void) {
  if (workspace_indicator_active_screen != NULL) {
    do_unnotify(
        &workspace_indicator_active_screen->workspace_indicatorwin_watch);
...
static void timer_expired__workspace_indicator_remove(WTimer *UNUSED(dummy1),
                                                      Obj *UNUSED(dummy2)) {
  if (workspace_indicator_active_screen != NULL)
...
void screen_update_workspace_indicatorwin(WRegion *reg_focused) {
  if (ioncore_g.workspace_indicator_timeout <= 0) return;

  WScreen *scr = region_screen_of(reg_focused);
  WRegion *mplex;
  if (scr == NULL || (mplex = mplex_mx_current(&(scr->mplex))) == NULL ||
      !OBJ_IS(mplex, WGroupWS) || mplex->ni.name == NULL) {
    return;
  }
  ...
  if (workspace_indicator_remove_timer == NULL) return;

  timer_set(workspace_indicator_remove_timer,
...
void screen_managed_notify(WScreen *scr, WRegion *UNUSED(reg),
                           WRegionNotify how) {
  if (how == ioncore_g.notifies.tag) screen_update_infowin(scr);
}
...
void ioncore_screen_activity_notify(WRegion *reg, WRegionNotify how) {
  ...
  if (scr != NULL) {
    if (how == ioncore_g.notifies.activity) {
      ...
      if (region_is_activity(reg)) screen_update_notifywin(scr);
      if ((WRegion *)scr == REGION_MANAGER(reg)) screen_do_update_infowin(scr);
    }
  }
}
