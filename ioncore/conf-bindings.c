    ...
    if (!button && keysym != NoSymbol) {
      ...
      if (XKeysymToKeycode(ioncore_g.dpy, keysym) == 0) {
        ioncore_warn_nolog("%s: %s", str,
                           "Could not convert keysym to keycode.");
        break;
      }
      *ksb_ret = keysym;
    } else {
...
static bool do_action(WBindmap *bindmap, const char *str, ExtlFn func, uint act,
  ...
  if (wr && mod == 0) {
    warn("Can not wait on modifiers when no modifiers set in \"%s\".", str);
    wr = FALSE;
  }
  ...
  binding.ksb = ksb;
  binding.kcb =
      (act == BINDING_KEYPRESS ? XKeysymToKeycode(ioncore_g.dpy, ksb) : ksb);
  binding.area = area;
  binding.submap = NULL;

  if (func != extl_fn_none()) {
    binding.func = extl_ref_fn(func);
    if (bindmap_add_binding(bindmap, &binding)) return TRUE;
    extl_unref_fn(binding.func);
    warn("Unable to add binding %s.", str);
  } else {
    binding.func = func;
    if (bindmap_remove_binding(bindmap, &binding)) return TRUE;
  }
...
static bool do_submap(WBindmap *bindmap, const char *str, ExtlTab subtab,
  ...
  if (action != BINDING_KEYPRESS) return FALSE;
  ...
  if (binding.submap == NULL) return FALSE;
...
static StringIntMap action_map[] = {{"kpress", BINDING_KEYPRESS},
...
static bool do_entry(WBindmap *bindmap, ExtlTab tab,
  ...
  if (strcmp(action_str, "kpress_wait") == 0) {
    ...
    if (action < 0) {
      warn("Unknown binding type \"%s\".", action_str);
      goto fail;
    }
  }

  if (!BINDING_IS_PSEUDO(action)) {
    if (!extl_table_gets_s(tab, "kcb", &ksb_str)) goto fail;
  ...
  if (extl_table_gets_t(tab, "submap", &subtab)) {
    ...
    if (!ret) extl_unref_fn(func);
  }

fail:
  if (action_str != NULL) free(action_str);
  if (ksb_str != NULL) free(ksb_str);
  if (area_str != NULL) free(area_str);
  return ret;
}
