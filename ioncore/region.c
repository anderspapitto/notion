...
void region_updategr(WRegion *reg) { CALL_DYN(region_updategr, reg, (reg)); }
...
void region_activated(WRegion *reg) { CALL_DYN(region_activated, reg, (reg)); }
...
static bool region_managed_prepare_focus_default(WRegion *mgr, WRegion *reg,
  ...
  if (!region_prepare_focus(mgr, flags, res)) return FALSE;
...
bool region_managed_prepare_focus(WRegion *mgr, WRegion *reg, int flags,
                                  WPrepareFocusResult *res) {
  bool ret = TRUE;
...
static WRegion *region_rqclose_propagate_default(WRegion *reg,
                                                 WRegion *maybe_sub) {
  if (maybe_sub == NULL) maybe_sub = region_current(reg);
...
WRegion *region_rqclose_propagate(WRegion *reg, WRegion *maybe_sub) {
  WRegion *ret = NULL;
  CALL_DYN_RET(ret, WRegion *, region_rqclose_propagate, reg, (reg, maybe_sub));
  return ret;
}
...
WRegion *region_disposeroot(WRegion *reg) {
  ...
  return (mgr != NULL ? region_managed_disposeroot(mgr, reg) : reg);
}

bool region_rqdispose(WRegion *reg) {
  WRegion *root;

  if (!region_may_dispose(reg)) return FALSE;
...
bool region_dispose_(WRegion *reg, bool not_simple) {
  ...
  if (was_mcf) ph = region_unset_get_return(reg);
...
bool region_dispose(WRegion *reg) { return region_dispose_(reg, TRUE); }
...
WScreen *region_screen_of(WRegion *reg) {
  while (reg != NULL) {
    if (OBJ_IS(reg, WScreen)) return (WScreen *)reg;
...
bool region_is_fully_mapped(WRegion *reg) {
  for (; reg != NULL; reg = REGION_PARENT_REG(reg)) {
    if (!REGION_IS_MAPPED(reg)) return FALSE;
  }
...
void region_notify_change(WRegion *reg, WRegionNotify how) {
  ...
  if (mgr != NULL) region_managed_notify(mgr, reg, how);
...
WRegion *region_managed_within(WRegion *reg, WRegion *mgd) {
  while (mgd != NULL && (REGION_PARENT_REG(mgd) == reg ||
                         REGION_PARENT_REG(mgd) == REGION_PARENT_REG(reg))) {
    if (REGION_MANAGER(mgd) == reg) return mgd;
...
void ioncore_region_notify(WRegion *reg, WRegionNotify how) {
  ...
  if (how == ioncore_g.notifies.name &&
      obj_is((Obj *)reg, &CLASSDESCR(WWindow))) {
    p[0] = region_name(reg);
...
#define REGION_DEBUGPRINT(what, next)                            \
  do {                                                           \
    ...
    while (reg != NULL) {                                        \
      LOG(DEBUG, GENERAL, "%s%p (%s)", line_indent, (void *)reg, \
          reg->ni.name);                                         \
                                                                 \
