...
bool extl_iter_objlist_(ExtlFn fn, ObjIterator *iter, void *st) {
  ...
  while (1) {
    obj = iter(st);
    if (obj == NULL) break;
    if (!extl_iter_obj(fn, obj)) return FALSE;
  }
...
bool extl_table_is_bool_set(ExtlTab tab, const char *entry) {
  ...
  if (extl_table_gets_b(tab, entry, &b)) return b;
  return FALSE;
}
...
bool extl_table_gets_rectangle(ExtlTab tab, const char *nam, WRectangle *rect) {
  ExtlTab g;
  bool ok;

  if (!extl_table_gets_t(tab, nam, &g)) return FALSE;
