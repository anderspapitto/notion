...
bool gr_register_engine(const char *engine, GrGetBrushFn *fn) {
  ...
  if (engine == NULL || fn == NULL) return FALSE;
...
void gr_unregister_engine(const char *engine) {
  ...
  free(eng->name);
  if (current_engine == eng) current_engine = NULL;
  free(eng);
}
...
GrBrush *gr_get_brush(Window win, WRootWin *rootwin, const char *style) {
  ...
  if (eng == NULL || eng->fn == NULL) return NULL;
...
static uint scorefind(const GrStyleSpec *attr, const GrAttrScore *spec) {
  ...
  if (attr->attrs == NULL) return 0;

  if (star_id == STRINGID_NONE) star_id = stringstore_alloc("*");
...
bool gr_stylespec_load_(GrStyleSpec *spec, const char *str,
                        bool no_order_score) {
  ...
  while (str != NULL) {
    ...
    if (a == STRINGID_NONE) goto fail;

    if (!gr_stylespec_add(spec, a, score)) goto fail;

    stringstore_free(a);

    if (!no_order_score) score--;
  }
...
void gr_stylespec_unalloc(GrStyleSpec *spec) {
  ...
  for (i = 0; i < spec->n; i++) stringstore_free(spec->attrs[i].attr);
...
bool gr_stylespec_add(GrStyleSpec *spec, GrAttr a, uint score) {
  ...
  if (a == GRATTR_NONE || score == 0) return TRUE;
...
void gr_stylespec_unset(GrStyleSpec *spec, GrAttr a) {
  ...
  if (a == GRATTR_NONE) return;

  if (!gr_stylespec_find_(spec, a, &idx_ge)) return;
  ...
  if (idsn != NULL || spec->n == 0) spec->attrs = idsn;
}

static bool gr_stylespec_do_init_from(GrStyleSpec *dst,
                                      const GrStyleSpec *src) {
  uint i;
  ...
  if (dst->attrs == NULL) return FALSE;
...
bool gr_stylespec_equals(const GrStyleSpec *s1, const GrStyleSpec *s2) {
  ...
  for (i = 0; i < s1->n; i++) {
    if (s1->attrs[i].attr != s2->attrs[i].attr) return FALSE;
  }
...
DYNFUN bool grbrush_get_extra(GrBrush *brush, const char *key, char type,
                              void *data) {
  bool ret = FALSE;
  CALL_DYN_RET(ret, bool, grbrush_get_extra, brush, (brush, key, type, data));
  return ret;
}
...
uint grbrush_get_text_width(GrBrush *brush, const char *text, uint len) {
  uint ret = 0;
  CALL_DYN_RET(ret, uint, grbrush_get_text_width, brush, (brush, text, len));
  return ret;
}
...
void grbrush_draw_textboxes(GrBrush *brush, const WRectangle *geom, int n,
