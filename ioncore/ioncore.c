...
static void ioncore_init_session(const char *display) {
  ...
  if (sm != NULL) ioncore_load_module("mod_sm");

  if (extl_sessiondir() != NULL) return;

  /* Not running under SM; use display-specific directory */
  dpyend = strchr(display, ':');
  if (dpyend != NULL) dpyend = strchr(dpyend, '.');
  if (dpyend == NULL) {
    libtu_asprintf(&tmp, "default-session-%s", display);
  } else {
    libtu_asprintf(&tmp, "default-session-%.*s", (int)(dpyend - display),
                   display);
  }
...
static bool ioncore_init_x(const char *display, int stflags) {
  ...
  ioncore_g.atom_checkcode =
      XInternAtom(dpy, "_ION_CWIN_RESTART_CHECKCODE", False);
...
static void set_initial_focus() {
  ...
  XQueryPointer(ioncore_g.dpy, None, &root, &win, &x, &y, &wx, &wy, &mask);
...
bool ioncore_startup(const char *display, const char *cfgfile, int stflags) {
  WRootWin *rootwin;
  sigset_t inittrap;
  ...
  if (!extl_init()) return FALSE;

  ioncore_register_exports();

  if (!ioncore_init_x(display, stflags)) return FALSE;

  gr_read_config();

  if (!extl_read_config("ioncore_ext", NULL, TRUE)) return FALSE;

  ioncore_read_main_config(cfgfile);

  if (!ioncore_init_layout()) return FALSE;
...
void ioncore_deinit() {
  ...
  if (ioncore_g.dpy == NULL) return;

  hook_call_v(ioncore_deinit_hook);

  while (ioncore_g.screens != NULL) destroy_obj((Obj *)ioncore_g.screens);

  /*ioncore_unload_modules();*/

  while (ioncore_g.rootwins != NULL) destroy_obj((Obj *)ioncore_g.rootwins);
...
EXTL_SAFE
EXTL_EXPORT
bool ioncore_is_i18n() { return ioncore_g.use_mb; }
...
EXTL_SAFE
EXTL_EXPORT
const char *ioncore_version() { return ION_VERSION; }
...
EXTL_SAFE
EXTL_EXPORT
const char *ioncore_progname() { return progname; }
...
EXTL_SAFE
EXTL_EXPORT
const char *ioncore_aboutmsg() { return ioncore_about; }
