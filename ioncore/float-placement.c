...
static void ggeom(WRegion *reg, WRectangle *geom) { *geom = REGION_GEOM(reg); }

static bool st_filt(WStacking *st, void *lvl) {
  uint level = *(uint *)lvl;

  return (st->reg != NULL && REGION_IS_MAPPED(st->reg) && st->level == level);
}

#define FOR_ALL_STACKING_NODES(VAR, WS, LVL, TMP)                         \
  for (stacking_iter_init(&(TMP), group_get_stacking(ws), st_filt, &LVL), \
       VAR = stacking_iter_nodes(&(TMP));                                 \
       VAR != NULL; VAR = stacking_iter_nodes(&(TMP)))
...
static WRegion *is_occupied(WGroup *ws, uint level, const WRectangle *r) {
  ...
  FOR_ALL_STACKING_NODES(st, ws, level, tmp) {
    ...
    if (r->x >= p.x + p.w) continue;
    if (r->y >= p.y + p.h) continue;
    if (r->x + r->w <= p.x) continue;
...
static int next_least_x(WGroup *ws, uint level, int x) {
  ...
  FOR_ALL_STACKING_NODES(st, ws, level, tmp) {
    ...
    if (p.x + p.w > x && p.x + p.w < retx) retx = p.x + p.w;
  }
...
static int next_least_y(WGroup *ws, uint level, int y) {
  ...
  FOR_ALL_STACKING_NODES(st, ws, level, tmp) {
    ...
    if (p.y + p.h > y && p.y + p.h < rety) rety = p.y + p.h;
  }
...
void group_calc_placement(WGroup *ws, uint level, WRectangle *geom) {
  if (ioncore_placement_method != PLACEMENT_RANDOM) {
    if (tiling_placement(ws, level, geom)) return;
  }
