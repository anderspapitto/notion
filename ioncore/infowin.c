...
void infowin_updategr(WInfoWin *p) {
  ...
  nbrush = gr_get_brush(p->wwin.win, region_rootwin_of((WRegion *)p), p->style);
...
void infowin_set_text(WInfoWin *p, const char *str, int maxw) {
  ...
  if (str == NULL) {
    ...
    if (!set) infowin_do_set_text(p, str);
  }
