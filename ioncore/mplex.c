...
#define PASSIVE(ST)                        \
  ((ST)->level <= STACKING_LEVEL_MODAL1 && \
   ((ST)->reg == NULL || (ST)->reg->flags & REGION_SKIP_FOCUS))
...
WRegion *mplex_iter(WMPlexIterTmp *tmp) { return stacking_iter_mgr(tmp); }
...
bool mplex_do_init(WMPlex *mplex, WWindow *parent, const WFitParams *fp,
  ...
  if (!window_do_init((WWindow *)mplex, parent, fp, win)) return FALSE;
...
void mplex_deinit(WMPlex *mplex) {
  ...
  FOR_ALL_MANAGED_BY_MPLEX(mplex, reg, tmp) { destroy_obj((Obj *)reg); }
...
WStacking *mplex_current_node(WMPlex *mplex) {
  ...
  if (reg != NULL) st = mplex_find_stacking(mplex, reg);
...
int mplex_get_index(WMPlex *mplex, WRegion *reg) {
  ...
  FOR_ALL_NODES_ON_LLIST(lnode, mplex->mx_list, tmp) {
    if (reg == lnode->st->reg) return index;
    index++;
  }
...
static void mplex_map_mgd(WMPlex *mplex) {
  ...
  FOR_ALL_NODES_IN_MPLEX(mplex, node, tmp) {
    if (!STACKING_IS_HIDDEN(node)) region_map(node->reg);
  }
}
...
static void mplex_unmap_mgd(WMPlex *mplex) {
  ...
  FOR_ALL_NODES_IN_MPLEX(mplex, node, tmp) {
    if (!STACKING_IS_HIDDEN(node)) region_unmap(node->reg);
  }
}
...
void mplex_map(WMPlex *mplex) {
   ...
   */
  if (!MPLEX_MGD_UNVIEWABLE(mplex)) mplex_map_mgd(mplex);
}
...
bool mplex_fitrep(WMPlex *mplex, WWindow *par, const WFitParams *fp) {
  ...
  if (!window_fitrep(&(mplex->win), par, fp)) return FALSE;
...
void mplex_do_fit_managed(WMPlex *mplex, WFitParams *fp) {
  ...
  if (!MPLEX_MGD_UNVIEWABLE(mplex) && (fp->g.w <= 1 || fp->g.h <= 1)) {
    ...
    if (REGION_IS_MAPPED(mplex)) mplex_unmap_mgd(mplex);
  } else if (MPLEX_MGD_UNVIEWABLE(mplex) && !(fp->g.w <= 1 || fp->g.h <= 1)) {
    mplex->flags &= ~MPLEX_MANAGED_UNVIEWABLE;
    if (REGION_IS_MAPPED(mplex)) mplex_map_mgd(mplex);
  }
...
void mplex_set_szplcy(WMPlex *mplex, WRegion *sub, WSizePolicy szplcy) {
  ...
  if (node != NULL) node->szplcy = szplcy;
}
...
static WStacking *stacking_within(WMPlex *mplex, WStacking *st) {
  ...
  return (reg == NULL ? NULL
                      : (reg == st->reg ? st : ioncore_find_stacking(reg)));
}
...
static bool mapped_pseudomodal_include_filt(WStacking *st, void *data_) {
  ...
  if (st->reg == NULL || !REGION_IS_MAPPED(st->reg)) return FALSE;

  if (!data->hidelist || (data->to_try == NULL && data->group_st == NULL) ||
      st->level < STACKING_LEVEL_MODAL1) {
    return TRUE;
  }
...
WStacking *mplex_find_to_focus(WMPlex *mplex, WStacking *to_try,
                               WStacking *group_st, PtrList **hidelist) {
...
static WStacking *maybe_focusable(WRegion *reg) {
  if (reg == NULL || !REGION_IS_MAPPED(reg)) return NULL;
...
void mplex_do_set_focus(WMPlex *mplex, bool warp) {
  if (!MPLEX_MGD_UNVIEWABLE(mplex)) {
    ...
    if (st == NULL) {
      st = (mplex->mx_current != NULL ? mplex->mx_current->st : NULL);
    }
...
static void mplex_refocus(WMPlex *mplex, WStacking *node, bool warp) {
  ...
  if (foc == NULL || !within) foc = mplex_to_focus(mplex);
  ...
  if (sub != NULL && CAN_MANAGE_STDISP(sub)) {
    if (stdisp != NULL) {
      ...
      if (omgr != sub && omgr != NULL) {
        if (CAN_MANAGE_STDISP(omgr)) region_unmanage_stdisp(omgr, FALSE, FALSE);
        region_detach_manager(stdisp);
      }

      region_manage_stdisp(sub, stdisp, &(mplex->stdispinfo));
    } else {
...
static void mplex_do_node_display(WMPlex *mplex, WStacking *node,
  ...
  if (node->lnode != NULL) {
    if (mxc != NULL) {
       ...
       */
      if (REGION_IS_MAPPED(mplex)) region_unmap(mxc->st->reg);
      mxc->st->hidden = TRUE;
    }
     ...
     */
    {
      ...
      if (grp != NULL) {
        ...
        if (bottom != NULL) {
          region_managed_rqorder((WRegion *)grp, bottom, REGION_ORDER_BACK);
        }
      }
    }
...
bool mplex_do_prepare_focus(WMPlex *mplex, WStacking *node, WStacking *sub,
...
bool mplex_managed_prepare_focus(WMPlex *mplex, WRegion *disp, int flags,
...
static WStacking *mplex_prv(WMPlex *mplex, WStacking *st, bool wrap) {
  return (st != mplex->mgd ? st->mgr_prev : (wrap ? st->mgr_prev : NULL));
}
  ...
  while (1) {
    ...
    if (!st->hidden) {
      if (OBJ_IS(st->reg, WGroup)) {
        ...
        if (res != NULL && res != st->reg) return res;
      } else {
...
bool mplex_managed_rqorder(WMPlex *mplex, WRegion *reg, WRegionOrder order) {
  ...
  if (stackingp == NULL || *stackingp == NULL) return FALSE;
...
static bool mplex_stack(WMPlex *mplex, WStacking *st) {
  ...
  if (stackingp == NULL) return FALSE;
...
bool mplex_do_attach_final(WMPlex *mplex, WRegion *reg, WMPlexPHolder *ph) {
  ...
  szplcy =
      ((param->flags & MPLEX_ATTACH_SIZEPOLICY &&
        param->szplcy != SIZEPOLICY_DEFAULT)
           ? param->szplcy
           : (param->flags & MPLEX_ATTACH_UNNUMBERED ? SIZEPOLICY_FULL_BOUNDS
                                                     : SIZEPOLICY_FULL_EXACT));

  level =
      (param->flags & MPLEX_ATTACH_LEVEL
           ? param->level
           : (param->flags & MPLEX_ATTACH_UNNUMBERED ? STACKING_LEVEL_NORMAL
                                                     : STACKING_LEVEL_BOTTOM));

  hidden = (param->flags & MPLEX_ATTACH_HIDDEN &&
            (param->flags & MPLEX_ATTACH_UNNUMBERED || !mx_was_empty));
  ...
  if (lnode != NULL) {
    ...
    llist_link_after(&(mplex->mx_list), (ph != NULL ? ph->after : NULL), lnode);
    mplex->mx_count++;
  ...
  if (!(param->flags & MPLEX_ATTACH_WHATEVER)) {
    ...
    sizepolicy(&node->szplcy, reg,
               (param->flags & MPLEX_ATTACH_GEOM ? &(param->geom) : NULL), 0,
               &fp);
  ...
  if (lnode != NULL) mplex_managed_changed(mplex, MPLEX_CHANGE_ADD, sw, reg);
...
#define MPLEX_ATTACH_SET_FLAGS \
  (MPLEX_ATTACH_GEOM | MPLEX_ATTACH_SIZEPOLICY | MPLEX_ATTACH_INDEX)
...
static bool mplex_handle_drop(WMPlex *mplex, int x, int y, WRegion *dropped) {
  ...
  if (curr && HAS_DYN(curr, region_handle_drop)) {
    ...
    if (rectangle_contains(&REGION_GEOM(curr), x - rx, y - ry)) {
      if (region_handle_drop(curr, x, y, dropped)) return TRUE;
    }
  }
...
WPHolder *mplex_prepare_manage(WMPlex *mplex, const WClientWin *cwin,
  ...
  /* Check current */ {
    ...
    if (mplex->mx_current != NULL && mplex->mx_current->st != cur) {
      ph = region_prepare_manage(mplex->mx_current->st->reg, cwin, param,
                                 cpriority);
      if (ph != NULL) return ph;
    }
  }

  if (!MANAGE_PRIORITY_OK(priority, MANAGE_PRIORITY_NORMAL)) return NULL;

  ap.flags =
      ((param->switchto ? MPLEX_ATTACH_SWITCHTO : 0) | MPLEX_ATTACH_SIZEPOLICY);
  ...
  if (node->lnode != NULL) {
    if (mplex->mx_current == node->lnode) {
      ...
      if (lnext == NULL) {
        ...
        if (lnext == node->lnode) lnext = NULL;
      }
      if (lnext != NULL) next = lnext->st;
    }
...
bool mplex_set_stdisp(WMPlex *mplex, WRegion *reg,
  ...
  assert(
      reg == NULL || (reg == oldstdisp) ||
      (REGION_MANAGER(reg) == NULL && REGION_PARENT(reg) == (WWindow *)mplex));
...
static StringIntMap pos_map[] = {{"tl", MPLEX_STDISP_TL},
                                 {"tr", MPLEX_STDISP_TR},
                                 {"bl", MPLEX_STDISP_BL},
                                 {"br", MPLEX_STDISP_BR},
                                 {NULL, 0}};
...
WRegion *mplex_set_stdisp_extl(WMPlex *mplex, ExtlTab t) {
  ...
  if (s == NULL || strcmp(s, "replace") == 0) {
    ...
    stdisp = region_attach_helper((WRegion *)mplex, (WWindow *)mplex, &fp,
                                  do_attach_stdisp, NULL, &data);

    if (stdisp == NULL) return NULL;
...
static ExtlTab mplex_do_get_stdisp_extl(WMPlex *mplex, bool fullconfig) {
  ...
  if (reg == NULL) return extl_table_none();
...
void mplex_call_changed_hook(WMPlex *mplex, WHook *hook, int mode, bool sw,
                             WRegion *reg) {
...
ExtlTab mplex_get_configuration(WMPlex *mplex) {
  ...
  FOR_ALL_NODES_IN_MPLEX(mplex, node, tmp) {
    if (node->lnode == NULL) save_node(mplex, subs, &n, node, TRUE);
  }
