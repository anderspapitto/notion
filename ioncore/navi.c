...
WRegion *region_navi_first(WRegion *reg, WRegionNavi nh,
  ...
  WRegion *ret = NULL;
  CALL_DYN_RET(ret, WRegion *, region_navi_first, reg, (reg, nh, data));
  return ret;
}
...
WRegion *region_navi_next(WRegion *reg, WRegion *mgd, WRegionNavi nh,
                          WRegionNaviData *data) {
  WRegion *ret = NULL;
  CALL_DYN_RET(ret, WRegion *, region_navi_next, reg, (reg, mgd, nh, data));
  return ret;
}
...
bool ioncore_string_to_navi(const char *str, WRegionNavi *nh) {
  ...
  if (!strcmp(str, "any")) {
    *nh = REGION_NAVI_ANY;
  } else if (!strcmp(str, "end") || !strcmp(str, "last") ||
             !strcmp(str, "next")) {
    *nh = REGION_NAVI_END;
  } else if (!strcmp(str, "beg") || !strcmp(str, "first") ||
             !strcmp(str, "prev")) {
    *nh = REGION_NAVI_BEG;
  ...
  } else if (!strcmp(str, "top") || !strcmp(str, "above") ||
             !strcmp(str, "up")) {
    *nh = REGION_NAVI_TOP;
  } else if (!strcmp(str, "bottom") || !strcmp(str, "below") ||
             !strcmp(str, "down")) {
...
static bool get_param(WRegionNaviData *data, const char *dirstr,
                      ExtlTab param) {
  if (!ioncore_string_to_navi(dirstr, &data->nh)) return FALSE;
...
WRegion *ioncore_navi_next(WRegion *reg, const char *dirstr, ExtlTab param) {
  ...
  if (!get_param(&data, dirstr, param)) return NULL;
