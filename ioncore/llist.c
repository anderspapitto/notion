...
void llist_iter_init(WLListIterTmp *tmp, WLListNode *llist) { *tmp = llist; }

WLListNode *llist_iter(WLListIterTmp *tmp) {
  WLListNode *mgd = *tmp;
  if (mgd != NULL) *tmp = mgd->next;
  return mgd;
}
...
void llist_link_after(WLListNode **list, WLListNode *after, WLListNode *node) {
  if (after != NULL) {
...
WLListNode *llist_index_to_after(WLListNode *list, WLListNode *current,
  ...
  if (index == LLIST_INDEX_AFTER_CURRENT_ACT) {
    ...
    while (after != NULL) {
      ...
      if (nxt == NULL || nxt->st == NULL || nxt->st->reg == NULL) break;
      if (!region_is_activity_r(nxt->st->reg)) break;
      after = nxt;
    }
    return after;
