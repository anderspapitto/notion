...
void ioncore_handle_selection_request(XSelectionRequestEvent *ev) {
  ...
  if (selection_data == NULL || ev->property == None) goto refuse;
  ...
  if (!ioncore_g.use_mb && ev->target == XA_STRING) {
    ...
    if (ok) {
      int st =
          XmbTextListToTextProperty(ioncore_g.dpy, (char **)p, 1, style, &prop);
      ok = (st >= 0);
    }
  }
...
static void ins(Window win, const char *str, int n) {
  if (!continuation_set) {
    ...
    if (wwin) window_insstr(wwin, str, n);
  } else {
...
void ioncore_set_selection_n(const char *p, int n) {
  if (selection_data != NULL) free(selection_data);
  ...
  XSetSelectionOwner(ioncore_g.dpy, CLIPATOM(ioncore_g.dpy),
                     DefaultRootWindow(ioncore_g.dpy), CurrentTime);
}
...
void ioncore_request_selection_for(Window win) {
  ...
  if (ioncore_g.use_mb) a = XA_COMPOUND_TEXT(ioncore_g.dpy);
