...
WRegion *create_region_load(WWindow *par, const WFitParams *fp, ExtlTab tab,
                            WPHolder **sm_ph_p) {
  ...
  if (!extl_table_gets_s(tab, "type", &objclass)) return NULL;
...
bool ioncore_init_layout() {
  ...
  FOR_ALL_SCREENS(scr) {
     ...
     */
    if (screen_id(scr) < 0) continue;

    if (ok) scrok = extl_table_geti_t(tab, screen_id(scr), &scrtab);

    n += (TRUE == screen_init_layout(scr, scrtab));

    if (scrok) extl_unref_table(scrtab);
  }
...
bool ioncore_save_layout() {
  ...
  if (tab == extl_table_none()) return FALSE;
