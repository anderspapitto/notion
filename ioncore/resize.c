...
static void moveres_draw_infowin(WMoveresMode *mode) {
  ...
  if (mode->mode == MOVERES_SIZE) {
    ...
    snprintf(buf, INFOWIN_BUFFER_LEN, "%+d %+d", mode->geom.x, mode->geom.y);
  }
...
static void moveres_draw_rubberband(WMoveresMode *mode) {
  ...
  WRootWin *rootwin = (mode->reg == NULL ? NULL : region_rootwin_of(mode->reg));
...
WMoveresMode *moveres_mode(WRegion *reg) {
  if (tmpmode == NULL) return NULL;
  return ((reg == NULL || tmpmode->reg == reg) ? tmpmode : NULL);
}

WRegion *moveresmode_target(WMoveresMode *mode) { return mode->reg; }
...
static WMoveresMode *create_moveresmode(WRegion *reg, WDrawRubberbandFn *rubfn,
                                        bool cumulative) {
...
static void moveresmode_do_newgeom(WMoveresMode *mode, WRQGeomParams *rq) {
  if (XOR_RESIZE) moveres_draw_rubberband(mode);
...
static void moveresmode_delta(WMoveresMode *mode, int dx1, int dx2, int dy1,
                              int dy2, WRectangle *rret) {
  ...
  if (mode->snap_enabled) {
    ...
    if (mode->dx1 != 0 && rq.geom.x + mode->dx1 < sg->x &&
        rq.geom.x + mode->dx1 > sg->x - er)
      realdx1 = sg->x - rq.geom.x;
    if (mode->dx2 != 0 && rq.geom.x + rq.geom.w + mode->dx2 > sg->x + sg->w &&
        rq.geom.x + rq.geom.w + mode->dx2 < sg->x + sg->w + er)
      realdx2 = sg->x + sg->w - rq.geom.x - rq.geom.w;
    if (mode->dy1 != 0 && rq.geom.y + mode->dy1 < sg->y &&
        rq.geom.y + mode->dy1 > sg->y - er)
      realdy1 = sg->y - rq.geom.y;
    if (mode->dy2 != 0 && rq.geom.y + rq.geom.h + mode->dy2 > sg->y + sg->h &&
        rq.geom.y + rq.geom.h + mode->dy2 < sg->y + sg->h + er)
  ...
  if (!mode->resize_cumulative) moveresmode_setorig(mode);

  if (rret != NULL) *rret = mode->geom;
}

void moveresmode_delta_resize(WMoveresMode *mode, int dx1, int dx2, int dy1,
                              int dy2, WRectangle *rret) {
  mode->mode = MOVERES_SIZE;
  moveresmode_delta(mode, dx1, dx2, dy1, dy2, rret);
}

void moveresmode_delta_move(WMoveresMode *mode, int dx, int dy,
                            WRectangle *rret) {
  mode->mode = MOVERES_POS;
...
void region_rqgeom(WRegion *reg, const WRQGeomParams *rq, WRectangle *geomret) {
...
void rqgeomparams_from_table(WRQGeomParams *rq, const WRectangle *origg,
                             ExtlTab g) {
  rq->geom = *origg;
...
void region_managed_rqgeom(WRegion *mgr, WRegion *reg, const WRQGeomParams *rq,
                           WRectangle *geomret) {
...
void region_managed_rqgeom_unallow(WRegion *UNUSED(mgr), WRegion *reg,
                                   ...
                                   WRectangle *geomret) {
  if (geomret != NULL) *geomret = REGION_GEOM(reg);
}
...
bool region_managed_maximize(WRegion *mgr, WRegion *reg, int dir, int action) {
  bool ret = FALSE;
  CALL_DYN_RET(ret, bool, region_managed_maximize, mgr,
               (mgr, reg, dir, action));
  return ret;
}
...
void frame_maximize_vert(WFrame *frame) {
  ...
  if (frame->flags & FRAME_SHADED || frame->flags & FRAME_MAXED_VERT) {
    if (frame->flags & FRAME_SHADED) frame->flags |= FRAME_SHADED_TOGGLE;
    if (frame->flags & FRAME_SAVED_VERT) {
      if (mp != NULL &&
          region_managed_maximize(mp, (WRegion *)frame, VERTICAL, VERIFY))
        region_managed_maximize(mp, (WRegion *)frame, VERTICAL, RESTORE);
      else
        rqh(frame, frame->saved_geom.y, frame->saved_geom.h);
    }
    frame->flags &=
        ~(FRAME_MAXED_VERT | FRAME_SAVED_VERT | FRAME_SHADED_TOGGLE);
...
void frame_maximize_horiz(WFrame *frame) {
  ...
  if (frame->flags & FRAME_MIN_HORIZ || frame->flags & FRAME_MAXED_HORIZ) {
    if (frame->flags & FRAME_SAVED_HORIZ) {
      if (mp != NULL &&
          region_managed_maximize(mp, (WRegion *)frame, HORIZONTAL, VERIFY))
