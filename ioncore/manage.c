...
WScreen *clientwin_find_suitable_screen(WClientWin *cwin,
  ...
  FOR_ALL_SCREENS(scr) {
    if (!region_same_rootwin((WRegion *)scr, (WRegion *)cwin)) continue;
...
static WPHolder *try_target(WClientWin *cwin, const WManageParams *param,
  ...
  if (!region_same_rootwin(r, (WRegion *)cwin)) return NULL;
...
static bool handle_target_winprops(WClientWin *cwin, const WManageParams *param,
  ...
  if (ph == NULL && extl_table_gets_s(cwin->proptab, "new_group", &layout)) {
    ...
    if (lo != extl_table_none()) {
      ...
      if (param->switchto) par.flags |= MPLEX_ATTACH_SWITCHTO;
      ...
      if (reg != NULL && !mgd) {
        if (target != NULL) ph = try_target(cwin, param, target);

        if (ph == NULL) {
          ph = region_prepare_manage(reg, cwin, param, MANAGE_PRIORITY_NONE);

          if (ph == NULL) destroy_obj((Obj *)reg);
        }
      }
    }
  }
...
static bool try_fullscreen(WClientWin *cwin, WScreen *dflt,
  ...
  if (fs_scr == NULL && netwm_check_initial_fullscreen(cwin)) fs_scr = dflt;
  ...
  if (fs_scr != NULL) {
    ...
    if (fs_ph != NULL) {
      ...
      if (!fs) cwin->flags &= ~CLIENTWIN_FS_RQ;
...
bool clientwin_do_manage_default(WClientWin *cwin, const WManageParams *param) {
  WScreen *scr = NULL;
  WPHolder *ph = NULL;
  int swf = (param->switchto ? PHOLDER_ATTACH_SWITCHTO : 0);
  bool uq = FALSE;
  ...
  if (handle_target_winprops(cwin, param, scr, &ph)) return TRUE;

  /* Check if param->tfor or any of its managers want to manage cwin. */
  if (ph == NULL && param->tfor != NULL) {
    assert(param->tfor != cwin);
    ph =
        region_prepare_manage_transient((WRegion *)param->tfor, cwin, param, 0);
    uq = TRUE;
  }
  ...
  if (ph == NULL) {
    ...
    if (try_fullscreen(cwin, scr, param)) {
      if (pholder_target(ph) != (WRegion *)region_screen_of((WRegion *)cwin)) {
        ...
        if (region_do_set_return(grp, ph)) return TRUE;
      }
      destroy_obj((Obj *)ph);
      return TRUE;
    }
  ...
  /* Not in full-screen mode; use the placeholder to attach. */
  {
    ...
    createroot =
        pholder_do_attach(ph, swf | PHOLDER_ATTACH_RETURN_CREATEROOT, &data);
  }

  destroy_obj((Obj *)ph);

  if (uq && createroot != NULL) ioncore_unsqueeze(createroot, FALSE);
...
WPHolder *region_prepare_manage_transient(WRegion *reg, const WClientWin *cwin,
                                          const WManageParams *param,
                                          int unused) {
  WPHolder *ret = NULL;
...
bool region_do_rescue_this(WRegion *tosave_, WRescueInfo *info, int ph_flags) {
  ...
  if (cwin != NULL) {
    if (cwin->flags & CLIENTWIN_UNMAP_RQ) return TRUE;
    tosave = (WRegion *)cwin;
  ...
  if (tosave == NULL) {
    ...
    return (ph == NULL ? FALSE : pholder_attach(info->ph, phf, tosave));
  }
}
...
bool region_rescue_some_clientwins(WRegion *reg, WRescueInfo *info,
  ...
  if (info->failed_get) return FALSE;
...
bool region_rescue(WRegion *reg, WPHolder *ph, int flags) {
  ...
  if (info.ph != ph) destroy_obj((Obj *)info.ph);
