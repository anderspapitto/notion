...
void ioncore_setup_display(int xscr) {
  ...
  if (xscr >= 0) snprintf(tmp + strlen(tmp), 11, ".%u", (unsigned)xscr);
...
void ioncore_setup_environ(const WExecP *p) {
  ...
  ioncore_setup_display(p->target != NULL ? region_rootwin_of(p->target)->xscr
                                          : -1);

  /* Set up working directory */

  if (p->wd != NULL) {
    if (chdir(p->wd) != 0) warn_err_obj(p->wd);
  }
}
...
int ioncore_do_exec_on(WRegion *reg, const char *cmd, const char *wd,
  ...
  return mainloop_popen_bgread(cmd, setup_exec, (void *)&p, extl_fn_none(),
                               errh);
}
...
bool ioncore_do_snapshot(bool save_layout) {
  if (save_layout && !ioncore_save_layout()) return FALSE;
...
void ioncore_do_restart() {
  ioncore_deinit();
  if (other != NULL) {
    if (ioncore_g.display != NULL) ioncore_setup_display(-1);
