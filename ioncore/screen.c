...
bool screen_init(WScreen *scr, WRootWin *parent, const WFitParams *fp, int id,
  ...
  if (parent == NULL) {
    ...
    win = XCreateWindow(
        ioncore_g.dpy, WROOTWIN_ROOT(parent), fp->g.x, fp->g.y, fp->g.w,
        fp->g.h, 0, DefaultDepth(ioncore_g.dpy, parent->xscr), InputOutput,
        DefaultVisual(ioncore_g.dpy, parent->xscr), attrflags, &attr);
    if (win == None) return FALSE;
  }

  if (!mplex_do_init((WMPlex *)scr, (WWindow *)parent, fp, win)) {
    if (!is_root) XDestroyWindow(ioncore_g.dpy, win);
    return FALSE;
  }
...
static void screen_managed_changed(WScreen *scr, int mode, bool sw,
                                   WRegion *reg_) {
  if (ioncore_g.opmode == IONCORE_OPMODE_DEINIT) return;

  if (sw && scr->atom_workspace != None) {
    WRegion *reg = mplex_mx_current(&(scr->mplex));
    const char *n = NULL;

    if (reg != NULL) n = region_displayname(reg);

    xwindow_set_string_property(region_root_of((WRegion *)scr),
                                scr->atom_workspace, n == NULL ? "" : n);
  }

  if (region_is_activity_r((WRegion *)scr)) screen_update_notifywin(scr);

  screen_update_infowin(scr);

  mplex_call_changed_hook((WMPlex *)scr, screen_managed_changed_hook, mode, sw,
                          reg_);
}

static void screen_map(WScreen *scr) { mplex_map((WMPlex *)scr); }
static void screen_unmap(WScreen *scr) { mplex_unmap((WMPlex *)scr); }
void screen_inactivated(WScreen *scr) { screen_update_infowin(scr); }
void screen_activated(WScreen *scr) { screen_update_infowin(scr); }
...
WScreen *ioncore_goto_nth_screen(int id) {
  ...
  if (scr != NULL) {
    if (!region_goto((WRegion *)scr)) return NULL;
  }
  return scr;
}
...
WScreen *ioncore_goto_next_screen() {
  ...
  if (scr != NULL) scr = scr->next_scr;
  if (scr == NULL) scr = ioncore_g.screens;
  if (scr != NULL) {
    if (!region_goto((WRegion *)scr)) return NULL;
  }
  return scr;
...
EXTL_SAFE
EXTL_EXPORT_MEMBER
int screen_id(WScreen *scr) { return scr->id; }
...
static bool screen_may_dispose(WScreen *UNUSED(scr)) { return TRUE; }
...
bool screen_set_managed_offset_extl(WScreen *scr, ExtlTab offset) {
  ...
  if (!extl_table_to_rectangle(offset, &g)) goto err;

  if (-g.w >= REGION_GEOM(scr).w) goto err;
  if (-g.h >= REGION_GEOM(scr).h) goto err;
...
static DynFunTab screen_dynfuntab[] = {
    ...
    {region_inactivated, screen_inactivated},
    {(DynFun *)region_managed_disposeroot,
     (DynFun *)screen_managed_disposeroot},
    {(DynFun *)region_may_dispose, (DynFun *)screen_may_dispose},
    ...
    {(DynFun *)region_get_configuration, (DynFun *)screen_get_configuration},
    {(DynFun *)region_handle_drop, (DynFun *)screen_handle_drop},
    END_DYNFUNTAB};
EXTL_EXPORT
