...
static StringIntMap frame_areas[] = {{"border", FRAME_AREA_BORDER},
                                     {"tab", FRAME_AREA_TAB},
...
#define DO_ALLOC(X, Y, Z)                              \
  ...
  if (ioncore_##X##_bindmap == NULL) return FALSE;

bool ioncore_init_bindmaps() {
  known_bindmaps = make_rb();

  if (known_bindmaps == NULL) return FALSE;
...
WBindmap *ioncore_create_cycle_bindmap(uint kcb, uint state, ExtlFn cycle,
                                       ExtlFn bcycle) {
