...
static void do_gravity(const WRectangle *max_geom, int szplcy,
  ...
  if (geom->h < 1) geom->h = 1;
  if (geom->w < 1) geom->w = 1;
...
static void gravity_stretch_policy(int szplcy, WRegion *reg,
  ...
  if (reg != NULL) region_size_hints_correct(reg, &w, &h, FALSE);
...
static void sizepolicy_free_snap(WSizePolicy *szplcy, WRegion *reg,
  ...
  if (!(rq_flags & REGION_RQGEOM_WEAK_X) && rq_flags & REGION_RQGEOM_WEAK_W) {
  ...
  if (!(rq_flags & REGION_RQGEOM_WEAK_Y) && rq_flags & REGION_RQGEOM_WEAK_H) {
  ...
  *szplcy |= ((fullw || fp->g.x <= max_geom.x ? SIZEPOLICY_HORIZ_LEFT : 0) |
              (fullw || fp->g.x + fp->g.w >= max_geom.x + max_geom.w
                   ? SIZEPOLICY_HORIZ_RIGHT
                   : 0) |
              (fullh || fp->g.y <= max_geom.y ? SIZEPOLICY_VERT_TOP : 0) |
              (fullh || fp->g.y + fp->g.h >= max_geom.y + max_geom.h
                   ? SIZEPOLICY_VERT_BOTTOM
                   : 0));
}

static WSizePolicy org(WSizePolicy base, WSizePolicy g) {
  if ((base & SIZEPOLICY_VERT_MASK) == 0) base |= g & SIZEPOLICY_VERT_MASK;

  if ((base & SIZEPOLICY_HORIZ_MASK) == 0) base |= g & SIZEPOLICY_HORIZ_MASK;

  return base;
}

void sizepolicy(WSizePolicy *szplcy, WRegion *reg, const WRectangle *rq_geom,
                int rq_flags, WFitParams *fp) {
  ...
  switch ((*szplcy) & SIZEPOLICY_MASK) {
    ...
    case SIZEPOLICY_STRETCH_LEFT:
      gravity_stretch_policy(
          org(*szplcy, SIZEPOLICY_HORIZ_LEFT | SIZEPOLICY_VERT_CENTER), reg,
          &tmp, fp, FALSE, TRUE);
      break;

    case SIZEPOLICY_STRETCH_RIGHT:
      gravity_stretch_policy(
          org(*szplcy, SIZEPOLICY_HORIZ_RIGHT | SIZEPOLICY_VERT_CENTER), reg,
          &tmp, fp, FALSE, TRUE);
      break;

    case SIZEPOLICY_STRETCH_TOP:
      gravity_stretch_policy(
          org(*szplcy, SIZEPOLICY_VERT_TOP | SIZEPOLICY_HORIZ_CENTER), reg,
          &tmp, fp, TRUE, FALSE);
      break;

    case SIZEPOLICY_STRETCH_BOTTOM:
      gravity_stretch_policy(
          org(*szplcy, SIZEPOLICY_VERT_BOTTOM | SIZEPOLICY_HORIZ_CENTER), reg,
          &tmp, fp, TRUE, FALSE);
      break;

    case SIZEPOLICY_FULL_EXACT:
      gravity_stretch_policy(
          org(*szplcy, SIZEPOLICY_VERT_CENTER | SIZEPOLICY_HORIZ_CENTER), reg,
          &tmp, fp, TRUE, TRUE);
      break;

    case SIZEPOLICY_FREE:
      if (reg != NULL) region_size_hints_correct(reg, &tmp.w, &tmp.h, FALSE);
    ...
    case SIZEPOLICY_VISIBILITY_CONSTRAINED:
      if (reg != NULL) region_size_hints_correct(reg, &tmp.w, &tmp.h, FALSE);
      {
        ...
        tmp.x =
            maxof(fp->g.x - dx, minof(tmp.x, fp->g.x + fp->g.w + dx - tmp.w));
        tmp.y =
            maxof(fp->g.y - dy, minof(tmp.y, fp->g.y + fp->g.h + dy - tmp.h));
      }
      fp->g = tmp;
      break;

    case SIZEPOLICY_UNCONSTRAINED:
      if (reg != NULL) region_size_hints_correct(reg, &tmp.w, &tmp.h, TRUE);
      fp->g = tmp;
      break;
