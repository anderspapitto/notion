...
void stacking_free(WStacking *st) {
  assert(st->mgr_next == NULL && st->mgr_prev == NULL && st->next == NULL &&
         st->prev == NULL &&
         /*st->above==NULL &&*/
         st->lnode == NULL && st->reg == NULL);
...
void stacking_unassoc(WStacking *st) {
  ...
  if (st->reg == NULL) return;

  if (stacking_of_reg != NULL)
    node = rb_find_pkey_n(stacking_of_reg, st->reg, &found);

  if (node != NULL) rb_delete_node(node);
...
bool stacking_assoc(WStacking *st, WRegion *reg) {
  ...
  if (stacking_of_reg == NULL) {
    ...
    if (stacking_of_reg == NULL) return FALSE;
  }

  if (rb_insertp(stacking_of_reg, reg, st) == NULL) return FALSE;
...
WStacking *stacking_unweave(WStacking **stacking, WStackingFilter *filt,
                            void *filt_data) {
  WStacking *np = NULL;
  WStacking *st, *next;
  ...
  for (st = *stacking; st != NULL; st = st->next) {
    st->to_unweave = 2;
    if (st->above == NULL && cf(filt, filt_data, st)) st->to_unweave = 1;
  }

  for (st = *stacking; st != NULL; st = st->next) check_unweave(st);
...
static int check_above_lvl(WStacking *st) {
  if (st->above == NULL) return st->level;
...
static void enforce_level_sanity(WStacking **np) {
   ...
   */
  for (st = *np; st != NULL; st = st->next) check_above_lvl(st);
...
static void get_bottom(WStacking *st, Window fb_win, Window *other, int *mode) {
...
static void stacking_do_weave(WStacking **stacking, WStacking **np, bool below,
                              Window fb_win) {
  WStacking *st, *ab;
  uint lvl;
  Window other;
  ...
  while (*np != NULL) {
    ...
    while (ab != NULL) {
      if (ab->level > lvl || (below && ab->level == lvl)) break;
      ab = ab->next;
    }
...
static void collect_above(WStacking **dst, WStacking **src, WStacking *regst) {
  ...
  for (stabove = *src; stabove != NULL; stabove = stnext) {
    ...
    if (is_above(stabove, regst)) collect_last(dst, src, stabove);
  }
}
...
WStacking *stacking_iter_nodes(WStackingIterTmp *tmp) {
  ...
  while (tmp->st != NULL) {
    next = tmp->st;
    tmp->st = tmp->st->next;
    if (cf(tmp->filt, tmp->filt_data, next)) break;
    next = NULL;
  }
...
uint stacking_min_level(WStacking *stacking, WStackingFilter *include_filt,
  ...
  if (stacking == NULL) return STACKING_LEVEL_BOTTOM;

  st = stacking;
  do {
    st = st->prev;

    if (st->reg != NULL && !(st->reg->flags & REGION_SKIP_FOCUS) &&
        cf(include_filt, filt_data, st)) {
      if (st->level >= STACKING_LEVEL_MODAL1) min_level = st->level;
...
WStacking *stacking_find_to_focus(WStacking *stacking, WStacking *to_try,
  ...
  do {
    ...
    if (st->level < min_level) break; /* no luck */

    if (st == to_try) return st;

    if (found == NULL && cf(approve_filt, filt_data, st)) {
      found = st;
      if (to_try == NULL) break;
    }
...
WStacking *stacking_find_to_focus_mapped(WStacking *stacking, WStacking *to_try,
                                         WRegion *mgr) {
  if (mgr == NULL) {
    return stacking_find_to_focus(stacking, to_try, mapped_filt, NULL, NULL);
  } else {
    return stacking_find_to_focus(stacking, to_try, mapped_filt, mgr_filt, mgr);
  }
}
...
bool stacking_must_focus(WStacking *stacking, WStacking *st) {
  WStacking *stf =
      stacking_find_to_focus(stacking, NULL, mapped_filt_neq, NULL, st);
