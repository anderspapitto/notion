...
void clientwin_get_protocols(WClientWin *cwin) {
  ...
  if (!XGetWMProtocols(ioncore_g.dpy, cwin->win, &protocols, &n)) return;
  ...
  if (protocols != NULL) XFree((char *)protocols);
}

#define SIZEHINT_PROPS                                                       \
  (CLIENTWIN_PROP_MAXSIZE | CLIENTWIN_PROP_MINSIZE | CLIENTWIN_PROP_ASPECT | \
   CLIENTWIN_PROP_RSZINC | CLIENTWIN_PROP_I_MAXSIZE |                        \
   CLIENTWIN_PROP_I_MINSIZE | CLIENTWIN_PROP_I_ASPECT |                      \
...
#define DO_SZH(NAME, FLAG, IFLAG, SZHFLAG, W, H, C)  \
  if (extl_table_is_bool_set(tab, "ignore_" NAME)) { \
    ...
    if (extl_table_gets_i(tab2, "w", &i1) &&         \
      ...
      C cwin->size_hints.flags |= SZHFLAG;           \
      cwin->flags |= FLAG;                           \
    }                                                \
...
void clientwin_get_set_name(WClientWin *cwin) {
  ...
  if (ioncore_g.use_mb) list = netwm_get_name(cwin);
...
bool clientwin_get_switchto(const WClientWin *cwin) {
  ...
  if (ioncore_g.opmode == IONCORE_OPMODE_INIT) return FALSE;

  if (extl_table_gets_b(cwin->proptab, "switchto", &b)) return b;
...
static void set_sane_gravity(Window win) {
  ...
  XChangeWindowAttributes(ioncore_g.dpy, win, CWWinGravity, &attr);
}
...
static bool clientwin_init(WClientWin *cwin, WWindow *par, Window win,
  ...
  if (cwin->orig_bw != 0 && cwin->size_hints.flags & PWinGravity) {
    fp.g.x += xgravity_deltax(cwin->size_hints.win_gravity, -cwin->orig_bw,
                              -cwin->orig_bw);
    fp.g.y += xgravity_deltay(cwin->size_hints.win_gravity, -cwin->orig_bw,
                              -cwin->orig_bw);
  }
...
WClientWin *clientwin_get_transient_for(const WClientWin *cwin) {
  ...
  if (clientwin_get_transient_mode(cwin) != TRANSIENT_MODE_NORMAL) return NULL;

  if (!XGetTransientForHint(ioncore_g.dpy, cwin->win, &tforwin)) return NULL;

  if (tforwin == None) return NULL;
...
WClientWin *ioncore_manage_clientwin(Window win, bool maprq) {
  ...
  if (hints != NULL) {
    if (hints->flags & StateHint) init_state = hints->initial_state;
    ...
    if (!param.dockapp && init_state == WithdrawnState &&
      ...
      if (!maprq) {
        if (attr.map_state == IsViewable) {
           ...
           */
          if (icon_attr.map_state == IsViewable) param.dockapp = TRUE;
        }
      } else {
        param.dockapp = TRUE;
      }
  ...
  if (!param.dockapp &&
      (attr.override_redirect || (!maprq && attr.map_state != IsViewable))) {
    goto fail2;
  }
  ...
  FOR_ALL_ROOTWINS(rootwin) {
    if (WROOTWIN_ROOT(rootwin) == attr.root) break;
  }
  ...
  param.gravity =
      (cwin->size_hints.flags & PWinGravity ? cwin->size_hints.win_gravity
                                            : ForgetGravity);
  ...
  if (maprq) netwm_check_manage_user_time(cwin, &param);
...
static bool reparent_root(WClientWin *cwin) {
  ...
  if (!XGetWindowAttributes(ioncore_g.dpy, cwin->win, &attr)) return FALSE;
...
void clientwin_deinit(WClientWin *cwin) {
  if (cwin->win != None) {
    ...
    if (cwin->orig_bw != 0) configure_cwin_bw(cwin->win, cwin->orig_bw);
 ...
 */
EXTL_EXPORT_MEMBER
void clientwin_kill(WClientWin *cwin) { XKillClient(ioncore_g.dpy, cwin->win); }
...
static void hide_clientwin(WClientWin *cwin) {
  ...
  if (cwin->flags & CLIENTWIN_PROP_ACROBATIC) {
    XMoveWindow(ioncore_g.dpy, cwin->win, -2 * REGION_GEOM(cwin).w,
...
static void show_clientwin(WClientWin *cwin) {
  if (cwin->flags & CLIENTWIN_PROP_ACROBATIC) {
    XMoveWindow(ioncore_g.dpy, cwin->win, REGION_GEOM(cwin).x,
                REGION_GEOM(cwin).y);
    if (cwin->state == NormalState) return;
  }
...
static void convert_geom(const WFitParams *fp, WClientWin *cwin,
                         WRectangle *geom) {
  WFitParams fptmp = *fp;
...
static bool clientwin_fitrep(WClientWin *cwin, WWindow *np,
  ...
  changes = (REGION_GEOM(cwin).x != geom.x || REGION_GEOM(cwin).y != geom.y ||
             REGION_GEOM(cwin).w != geom.w || REGION_GEOM(cwin).h != geom.h);

  if (np == NULL && !changes) return TRUE;
  ...
  if (np != NULL) {
    ...
    if (!REGION_IS_FULLSCREEN(cwin)) cwin->flags &= ~CLIENTWIN_FS_RQ;
  }

  if (postpone_resize(cwin)) return TRUE;
...
static Window clientwin_x_window(WClientWin *cwin) { return cwin->win; }
...
ExtlTab clientwin_get_ident(WClientWin *cwin) {
  ...
  if (n >= 2 && p[1] != NULL) extl_table_sets_s(tab, "class", p[1]);
  if (n >= 1 && p[0] != NULL) extl_table_sets_s(tab, "instance", p[0]);
  if (wrole != NULL) extl_table_sets_s(tab, "role", wrole);
  ...
  if (dockapp_hack) extl_table_sets_b(tab, "is_dockapp", TRUE);

  if (p != NULL) XFreeStringList(p);
  if (p2 != NULL) XFreeStringList(p2);
  if (wrole != NULL) free(wrole);
...
static bool check_fs_cfgrq(WClientWin *cwin, XConfigureRequestEvent *ev) {
  ...
  if ((ev->value_mask & (CWWidth | CWHeight)) == (CWWidth | CWHeight)) {
    ...
    if (scr != NULL && REGION_MANAGER(grp) != (WRegion *)scr) {
      ...
      if (!region_fullscreen_scr(grp, scr, sw)) cwin->flags &= ~CLIENTWIN_FS_RQ;
...
WMPlex *find_mplexer(WRegion *cwin) {
  if (cwin == NULL) return NULL;
  if (obj_is((Obj *)cwin, &CLASSDESCR(WMPlex))) return (WMPlex *)cwin;
...
static bool check_normal_cfgrq(WClientWin *cwin, XConfigureRequestEvent *ev) {
  ...
  if (ev->value_mask & (CWX | CWY | CWWidth | CWHeight)) {
     ...
     */
    if (cwin->size_hints.flags & PWinGravity) {
      gdx = xgravity_deltax(cwin->size_hints.win_gravity, -cwin->orig_bw,
                            -cwin->orig_bw);
      gdy = xgravity_deltay(cwin->size_hints.win_gravity, -cwin->orig_bw,
                            -cwin->orig_bw);
    }
  ...
  if (ioncore_g.window_stacking_request !=
...
void clientwin_handle_configure_request(WClientWin *cwin,
  ...
  if (ev->value_mask & CWBorderWidth) cwin->orig_bw = ev->border_width;

  cwin->flags |= CLIENTWIN_NEED_CFGNTFY;

  if (!(cwin->flags & CLIENTWIN_PROP_IGNORE_CFGRQ)) {
    if (!check_fs_cfgrq(cwin, ev)) check_normal_cfgrq(cwin, ev);
  }
...
static ExtlTab clientwin_get_configuration(WClientWin *cwin) {
  ...
  if (cfg_cb != NULL) cfg_cb(cwin, tab);
...
static void do_sm(ExtlTab tab) {
  ...
  if (add_cb != NULL) {
    ...
    if (ph != NULL) {
      if (!add_cb(ph, tab)) destroy_obj((Obj *)ph);
    }
  }
}
...
WRegion *clientwin_load(WWindow *par, const WFitParams *fp, ExtlTab tab) {
  ...
  got_chkc =
      xwindow_get_integer_property(win, ioncore_g.atom_checkcode, &real_chkc);
  ...
  if (attr.root != region_root_of((WRegion *)par)) return NULL;
