...
void grouppholder_do_link(WGroupPHolder *ph, WGroup *group,
                          WRegion *stack_above) {
  ph->group = group;
...
bool grouppholder_init(WGroupPHolder *ph, WGroup *ws, const WStacking *st,
...
WGroupPHolder *create_grouppholder(WGroup *ws, const WStacking *st,
                                   const WGroupAttachParams *param) {
...
WRegion *grouppholder_do_attach(WGroupPHolder *ph, int flags,
  ...
  if (ws == NULL) return grouppholder_attach_recreate(ph, flags, data);
...
bool grouppholder_do_goto(WGroupPHolder *ph) {
  return (ph->group != NULL ? region_goto((WRegion *)ph->group)
                            : (ph->recreate_pholder != NULL
...
WRegion *grouppholder_do_target(WGroupPHolder *ph) {
  return (ph->group != NULL ? (WRegion *)ph->group
                            : (ph->recreate_pholder != NULL
...
static DynFunTab grouppholder_dynfuntab[] = {
    {(DynFun *)pholder_do_attach, (DynFun *)grouppholder_do_attach},

    {(DynFun *)pholder_do_goto, (DynFun *)grouppholder_do_goto},

    {(DynFun *)pholder_do_target, (DynFun *)grouppholder_do_target},

    END_DYNFUNTAB};

IMPLCLASS(WGroupPHolder, WPHolder, grouppholder_deinit, grouppholder_dynfuntab);
