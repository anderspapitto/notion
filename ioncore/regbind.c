...
static void grab_ungrabbed_bindings(const WRegion *reg,
...
static WRegBindingInfo *find_rbind(WRegion *reg, WBindmap *bindmap,
  ...
  for (rbind = (WRegBindingInfo *)reg->bindings; rbind != NULL;
       rbind = rbind->next) {
    if (rbind->bindmap == bindmap && rbind->owner == owner) return rbind;
  }
...
static WRegBindingInfo *region_do_add_bindmap_owned(WRegion *reg,
  ...
  if (bindmap == NULL) return NULL;

  rbind = ALLOC(WRegBindingInfo);

  if (rbind == NULL) return NULL;
...
bool region_add_bindmap(WRegion *reg, WBindmap *bindmap) {
  if (find_rbind(reg, bindmap, NULL) != NULL) return FALSE;
...
void region_remove_bindmap(WRegion *reg, WBindmap *bindmap) {
  ...
  if (rbind != NULL) remove_rbind(reg, rbind);
}
...
WBinding *region_lookup_keybinding(WRegion *reg, int act, uint state, uint kcb,
  ...
  *binding_owner_ret = reg;
  ...
  for (rbind = (WRegBindingInfo *)reg->bindings; rbind != NULL;
    ...
    for (s = sc; s != NULL && bindmap != NULL; s = s->next) {
      binding =
          bindmap_lookup_binding(bindmap, BINDING_KEYPRESS, s->state, s->key);
...
WBinding *region_lookup_binding(WRegion *reg, int act, uint state, uint kcb,
                                int area) {
  WRegBindingInfo *rbind;
  WBinding *binding = NULL;
  ...
  for (rbind = (WRegBindingInfo *)reg->bindings; rbind != NULL;
       rbind = rbind->next) {
    if (rbind->owner != NULL) continue;
    binding =
        bindmap_lookup_binding_area(rbind->bindmap, act, state, kcb, area);
    if (binding != NULL) break;
  }
...
static void add_bindings(WRegion *reg, WRegion *r2) {
  ...
  if (r2->flags & REGION_GRAB_ON_PARENT) {
    for (rb2 = (WRegBindingInfo *)r2->bindings; rb2 != NULL; rb2 = rb2->next) {
      ...
      if (rbind == NULL) {
        rbind = region_do_add_bindmap_owned(reg, rb2->bindmap, r2, TRUE);
      }
      if (rbind != NULL) rbind->tmp = 1;
    }
  }
}
...
void region_do_update_owned_grabs(WRegion *reg) {
  ...
  /* make new grabs */
  if (reg->active_sub != NULL) add_bindings(reg, reg->active_sub);

  /* remove old grabs */
  for (rbind = (WRegBindingInfo *)reg->bindings; rbind != NULL; rbind = rb2) {
    rb2 = rbind->next;
    if (rbind->tmp != 1 && rbind->owner != NULL) remove_rbind(reg, rbind);
  }
}
