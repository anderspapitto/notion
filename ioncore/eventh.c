...
void ioncore_handle_unmap_notify(const XUnmapEvent *ev) {
  ...
  if (ev->event != ev->window && ev->send_event != True) return;

  cwin = XWINDOW_REGION_OF_T(ev->window, WClientWin);

  if (cwin != NULL) clientwin_unmapped(cwin);
}
...
void ioncore_handle_destroy_notify(const XDestroyWindowEvent *ev) {
  ...
  if (cwin != NULL) clientwin_destroyed(cwin);
}
...
void ioncore_handle_property(const XPropertyEvent *ev) {
  ...
  if (ev->atom == XA_WM_HINTS) {
    ...
    if (ev->state == PropertyNewValue) {
      if (clientwin_get_size_hints(cwin) < 0)
        LOG(WARN, GENERAL, "Retrieving sizehints failed for cwin '%s'",
            cwin->region.ni.name);
  ...
  /* Call property hook */
  {
    ...
    p[0] = (void *)cwin;
    p[1] = (void *)ev;
    hook_call(clientwin_property_change_hook, p, (WHookMarshall *)pchg_mrsh,
...
void ioncore_handle_expose(const XExposeEvent *ev) {
  ...
  if (wwin != NULL) window_draw(wwin, FALSE);
}
...
void ioncore_handle_enter_window(XEvent *ev) {
  ...
  if (eev->mode != NotifyNormal && !ioncore_g.warp_enabled) return;

  reg = XWINDOW_REGION_OF_T(eev->window, WRegion);

  if (reg == NULL) return;

  if (REGION_IS_ACTIVE(reg)) return;

  if (region_skip_focus(reg)) return;
  ...
  if (region_goto_flags(reg, (REGION_GOTO_FOCUS | REGION_GOTO_NOWARP |
...
static bool pointer_in_root(Window root1) {
  ...
  XQueryPointer(ioncore_g.dpy, root1, &root2, &win, &x, &y, &wx, &wy, &mask);
...
void ioncore_handle_focus_in(const XFocusChangeEvent *ev) {
  ...
  if (ev->detail == NotifyPointer) return;

  /* Input contexts */
  if (OBJ_IS(reg, WWindow)) {
    wwin = (WWindow *)reg;
    if (wwin->xic != NULL) XSetICFocus(wwin->xic);
  }

  if (ev->detail != NotifyInferior) netwm_set_active(reg);
...
void ioncore_handle_focus_out(const XFocusChangeEvent *ev) {
  ...
  D(if (OBJ_IS(reg, WRootWin))
    fprintf(stderr, "scr-out %d %d %d\n", ((WRootWin *)reg)->xscr, ev->mode,
            ev->detail));

  if (OBJ_IS(reg, WWindow)) {
    wwin = (WWindow *)reg;
    if (wwin->xic != NULL) XUnsetICFocus(wwin->xic);
  }
...
void ioncore_handle_buttonpress(XEvent *ev) {
  ...
  if (ioncore_grab_held()) return;

  if (!ioncore_do_handle_buttonpress(&(ev->xbutton))) return;
  ...
  while (!finished && ioncore_grab_held()) {
    ...
    if (ev->type == MotionNotify) {
      /* Handle sequences of MotionNotify (possibly followed by button release) as one. */
    ...
    switch (ev->type) {
      ...
      if (ioncore_do_handle_buttonrelease(&ev->xbutton)) finished = TRUE;
      break;
...
void ioncore_handle_keyboard(XEvent *ev) {
  if (ioncore_handle_grabs(ev)) return;

  if (ev->type == KeyPress) ioncore_do_handle_keypress(&(ev->xkey));
}
