...
int frame_press(WFrame *frame, XButtonEvent *ev, WRegion **reg_ret) {
  ...
  if (frame->barmode != FRAME_BAR_NONE &&
    ...
    FRAME_MX_FOR_ALL(sub, frame, tmp) {
      p_tabnum++;
      if (sub == FRAME_CURRENT(frame)) break;
    }
...
static ExtlExportedFn *tabdrag_safe_fns[] = {
    ...
    (ExtlExportedFn *)&mplex_switch_prev, NULL};
...
static void p_tabdrag_begin(WFrame *frame, XMotionEvent *ev, int dx, int dy) {
  ...
  if (tabdrag_infowin != NULL) window_map((WWindow *)tabdrag_infowin);
}
...
static WRegion *fnd(Window root, int x, int y) {
  ...
  while (w != NULL) {
    if (HAS_DYN(w, region_handle_drop)) reg = (WRegion *)w;

    if (!XTranslateCoordinates(ioncore_g.dpy, root, w->win, x, y, &dstx, &dsty,
                               &win)) {
      break;
    }
...
static bool drop_ok(WRegion *mgr, WRegion *reg) {
  ...
  for (reg2 = mgr; reg2 != NULL; reg2 = region_manager(reg2)) {
    if (reg2 == reg) goto err;
  }

  for (reg2 = REGION_PARENT_REG(mgr); reg2 != NULL;
...
static void p_tabdrag_end(WFrame *frame, XButtonEvent *ev) {
  ...
  if (sub == NULL || ev->root != region_root_of(sub)) return;
...
void frame_p_tabdrag(WFrame *frame) {
  ...
  ioncore_set_drag_handlers(
      (WRegion *)frame, (WMotionHandler *)p_tabdrag_begin,
      (WMotionHandler *)p_tabdrag_motion, (WButtonHandler *)p_tabdrag_end,
