...
static void get_titles_text_width(WFrame *frame) {
  ...
  FRAME_MX_FOR_ALL(sub, frame, itmp) {
    ...
    if (displayname == NULL)
      ...
      frame->titles[i].iw = grbrush_get_text_width(
          frame->bar_brush, displayname, strlen(displayname));
    i++;
  }
}
...
static void frame_tabs_width_calculate_equal(WFrame *frame) {
  ...
  w = bg.w - bdw.left - bdw.right -
      (bdw.tb_ileft + bdw.tb_iright + bdw.spacing) * (m - 1);

  if (w <= 0) {
    for (i = 0; i < m; i++) frame->titles[i].iw = 0;
  } else {
    for (i = 0; i < m; i++) /* Get i:th tab's portion of free area */
...
static int frame_shaped_recalc_bar_size_equal(WFrame *frame) {
  ...
  if (m > 0) {
    ...
    for (i = 0; i < m; i++)
      if (frame->titles[i].iw > tmaxw) tmaxw = frame->titles[i].iw;
...
static bool tab_sizes_propor_elastic(WFrame *frame, bool complete,
                                     bool proportional) {
  ...
  if (m > PROPOR_MAX_TABS) return tab_sizes_equal(frame, complete);
  ...
  for (i = 0; i < m; i++) {
    ...
    titles_total += (iw < frame->tabs_params.propor_tab_min_w
                         ? frame->tabs_params.propor_tab_min_w
                         : iw);
    titles_padded_total1 += (iw < frame->tabs_params.propor_tab_min_w -
                                         2 * frame->tabs_params.requested_pad
                                 ? frame->tabs_params.propor_tab_min_w
                                 : iw + 2 * frame->tabs_params.requested_pad);
    titles_padded_total2 += (iw < frame->tabs_params.tab_min_w -
                                         2 * frame->tabs_params.requested_pad
                                 ? frame->tabs_params.tab_min_w
                                 : iw + 2 * frame->tabs_params.requested_pad);
  }
  bdtotal = ((m - 1) * (bdw.tb_ileft + bdw.tb_iright + bdw.spacing) +
             bdw.right + bdw.left);
  ...
  if (m * frame->tabs_params.propor_tab_min_w >= w) {
    ...
    max_w = sorted_sizes[m - 1];
    titles_total = 0;
    for (i = 0; i < m; i++) {
      if (sorted_sizes[i] > frame->tabs_params.propor_tab_min_w) break;
    ...
    for (i = 0; i < m; i++)
      if (frame->titles[i].iw > max_w) frame->titles[i].iw = max_w;
  ...
  equal_pad:
    ...
    i = m - 1;
    min_w = 0;
    while ((i >= 0) && (sorted_sizes[i] >= max_w)) {
      titles_total += sorted_sizes[i];
      i--;
    }
    while (i >= 0) {
  ...
  equal_tab:
    qsort(sorted_sizes, m, sizeof(int), intcompare);
    min_w = 0;
    titles_total = 2 * m * frame->tabs_params.requested_pad;
    for (i = m - 1; i >= 0; i--) {
  ...
  if (min_w > 0) {
    for (i = 0; i < m; i++)
      if (frame->titles[i].iw < min_w) frame->titles[i].iw = min_w;
  }

  /* Calculate remaining space */
  titles_total = 0;
  for (i = 0; i < m; i++) titles_total += frame->titles[i].iw;
  w -= titles_total;
...
void frame_tabs_calc_brushes_updated(WFrame *frame) {
  ...
  if (grbrush_get_extra(frame->brush, "floatframe_tab_min_w", 'i',
                        &(pars->tab_min_w)) ||
      grbrush_get_extra(frame->brush, "frame_tab_min_w", 'i',
                        &(pars->tab_min_w))) {
    if (pars->tab_min_w <= 0) pars->tab_min_w = 1;
  }

  if (grbrush_get_extra(frame->brush, "frame_propor_tab_min_w", 'i',
                        &(pars->propor_tab_min_w))) {
    if (pars->propor_tab_min_w <= 0) pars->propor_tab_min_w = 1;
  }

  if (grbrush_get_extra(frame->brush, "floatframe_bar_max_w_q", 'd',
                        &(pars->bar_max_width_q))) {
    if (pars->bar_max_width_q <= 0.0 || pars->bar_max_width_q > 1.0)
      pars->bar_max_width_q = 1.0;
  }

  if (grbrush_get_extra(frame->brush, "frame_tab_padding", 'i',
                        &(pars->requested_pad))) {
    if (pars->requested_pad <= 0) pars->requested_pad = 1;
  }

  if (grbrush_get_extra(frame->brush, "frame_tab_width_alg", 's', &str)) {
    alg = STRINGFUNPTRMAP_VALUE(TabCalcPtr, frame_tabs_width_algorithms, str,
                                NULL);

    if (alg != NULL) frame->tabs_params.alg = alg;
  }
}
