...
static bool groupws_attach_framed(WGroupWS *ws, WGroupAttachParams *ap,
  ...
  return (region_attach_framed((WRegion *)ws, fp,
                               (WRegionAttachFn *)group_do_attach, ap,
                               &data) != NULL);
}

bool groupws_handle_drop(WGroupWS *ws, int x, int y, WRegion *dropped) {
...
WPHolder *groupws_prepare_manage(WGroupWS *ws, const WClientWin *cwin,
  ...
  if (b != NULL && !HAS_DYN(b, region_prepare_manage)) b = NULL;

  use_bottom =
      (act_b ? !extl_table_is_bool_set(cwin->proptab, "float") : act_b);
  ...
  if (ph == NULL) {
    /* Check current */
    WRegion *r =
        (ws->grp.current_managed != NULL ? ws->grp.current_managed->reg : NULL);
...
WPHolder *groupws_prepare_manage_transient(WGroupWS *ws, const WClientWin *cwin,
  ...
  if (ap.stack_above == NULL) return NULL;
  ...
  if (!ap.geom_weak_set || param->userpos || !param->maprq ||
      ioncore_g.opmode == IONCORE_OPMODE_INIT) {
    ap.geom_weak_set = 1;
    ap.geom_weak = 0;
  }
...
static bool group_empty_for_bottom_stdisp(WGroup *ws) {
  ...
  FOR_ALL_NODES_IN_GROUP(ws, st, tmp) {
    if (st != ws->bottom && st != ws->managed_stdisp) return FALSE;
  }
...
WRegion *groupws_load(WWindow *par, const WFitParams *fp, ExtlTab tab) {
  ...
  if (extl_table_gets_t(tab, "initial_outputs", &io)) ws->initial_outputs = io;
