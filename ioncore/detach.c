...
static void get_relative_geom(WRectangle *g, WRegion *reg, WRegion *mgr) {
  ...
  *g = REGION_GEOM(reg);
  ...
  for (w = REGION_PARENT(reg); w != rel && (WRegion *)w != mgr;
       w = REGION_PARENT(w)) {
    g->x += REGION_GEOM(w).x;
    g->y += REGION_GEOM(w).y;
  }
...
static bool ioncore_do_detach(WRegion *reg, WGroup *grp, WFrameMode framemode,
  ...
  if (framemode != FRAME_MODE_UNKNOWN) {
    ...
    if (st != NULL) {
       ...
       * non-modal detached.
       */
      if (st->level > STACKING_LEVEL_MODAL1) ap.level = st->level;
    }
...
static WRegion *check_mplex(WRegion *reg, WFrameMode *mode) {
  ...
  *mode = FRAME_MODE_FLOATING;
  ...
  if (OBJ_IS(mplex, WFrame)) {
    ...
    if (framemode_unalt(mode2) == FRAME_MODE_TRANSIENT) *mode = mode2;
  }
...
void do_unsqueeze(WRegion *reg) {
  ...
  if (OBJ_IS(reg, WScreen)) return;

  region_size_hints(reg, &h);

  if (!h.min_set) return;

  if (h.min_width <= REGION_GEOM(reg).w && h.min_height <= REGION_GEOM(reg).h) {
    return;
  }
