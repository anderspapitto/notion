...
void region_focuslist_remove_with_mgrs(WRegion *reg) {
  ...
  if (mgrp != NULL) region_focuslist_remove_with_mgrs(mgrp);
}
...
void region_focuslist_move_after(WRegion *reg, WRegion *after) {
  region_focuslist_remove_with_mgrs(reg);
  LINK_ITEM_AFTER(ioncore_g.focuslist, after, reg, active_next, active_prev);
}

static void region_focuslist_deinit(WRegion *reg) {
  WRegion *replace = region_manager_or_parent(reg);

  if (replace != NULL) region_focuslist_move_after(replace, reg);
...
WRegion *ioncore_goto_previous() {
  ...
  if (ioncore_g.focuslist == NULL) return NULL;
   ...
   * active.
   */
  for (next = ioncore_g.focuslist->active_next; next != NULL;
       next = next->active_next) {
    if (!REGION_IS_ACTIVE(next)) break;
  }

  if (next != NULL) region_goto(next);
...
bool ioncore_focushistory_i(ExtlFn iterfn) {
  ...
  if (ioncore_g.focuslist == NULL) return FALSE;
  ...
  for (next = ioncore_g.focuslist->active_next; next != NULL;
       next = next->active_next) {
    if (!extl_iter_obj(iterfn, (Obj *)next)) return FALSE;
  }
...
void region_set_await_focus(WRegion *reg) {
  if (reg == NULL) {
    ...
    watch_setup(&await_watch, (Obj *)reg, (WatchHandler *)await_watch_handler);
  }
}

static bool region_is_parent(WRegion *reg, WRegion *aw) {
  while (aw != NULL) {
    if (aw == reg) return TRUE;
...
static void check_clear_await(WRegion *reg) {
  if (region_is_await(reg) && reg != (WRegion *)await_watch.obj) return;

  watch_reset(&await_watch);
}

WRegion *ioncore_await_focus() { return (WRegion *)(await_watch.obj); }
...
static void timer_expired__focuslist_insert(WTimer *UNUSED(dummy1),
                                            Obj *UNUSED(dummy2)) {
...
static void schedule_focuslist_insert_timer(WRegion *reg) {
  ...
  if (focuslist_insert_timer == NULL) {
    ...
    if (focuslist_insert_timer == NULL) return;
  }
...
static void region_focuslist_awaiting_insertion_cancel_if_is(WRegion *reg) {
  if (region_awaiting_insertion == reg && focuslist_insert_timer != NULL) {
    timer_reset(focuslist_insert_timer);
  }
}

static void region_focuslist_awaiting_insertion_trigger(void) {
  if (focuslist_insert_timer != NULL && region_awaiting_insertion != NULL) {
...
bool region_manager_is_focusnext(WRegion *reg) {
  if (reg == NULL || ioncore_g.focus_next == NULL) return FALSE;

  if (reg == ioncore_g.focus_next) return TRUE;

  return region_manager_is_focusnext(REGION_MANAGER(reg));
}

bool region_may_control_focus(WRegion *reg) {
  if (OBJ_IS_BEING_DESTROYED(reg)) return FALSE;

  if (REGION_IS_ACTIVE(reg) || REGION_IS_PSEUDOACTIVE(reg)) return TRUE;

  if (region_is_await(reg) || region_is_focusnext(reg)) return TRUE;

  if (region_manager_is_focusnext(reg)) return TRUE;
...
void region_finalise_focusing(WRegion *reg, Window win, bool warp, Time time,
                              int set_input) {
  if (warp) region_do_warp(reg);

  if (REGION_IS_ACTIVE(reg) && ioncore_await_focus() == NULL) return;
...
static WRegion *find_warp_to_reg(WRegion *reg) {
  ...
  if (reg->flags & REGION_PLEASE_WARP) return reg;
...
void region_set_focus(WRegion *reg) { region_maybewarp(reg, FALSE); }

void region_warp(WRegion *reg) { region_maybewarp(reg, TRUE); }
...
bool region_skip_focus(WRegion *reg) {
  while (reg != NULL) {
    if (reg->flags & REGION_SKIP_FOCUS) return TRUE;
 ...
 */
EXTL_EXPORT
WRegion *ioncore_current() { return ioncore_g.focus_current; }
...
void region_pointer_focus_hack(WRegion *reg) {
  ...
  if (ioncore_g.opmode != IONCORE_OPMODE_NORMAL) return;
  ...
  if (act == NULL) act = ioncore_g.focus_current;

  if (act == NULL || OBJ_IS_BEING_DESTROYED(act) ||
      !region_is_fully_mapped(act) || region_skip_focus(act)) {
    return;
  }
