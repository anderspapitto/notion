...
void frame_adjust_to_initial(WFrame *frame, const WFitParams *fp,
  ...
  if (!(fp->mode & (REGION_FIT_BOUNDS | REGION_FIT_WHATEVER))) return;
  ...
  if (!(fp->mode & REGION_FIT_WHATEVER)) rectangle_constrain(&rqg, &fp->g);

  region_fit((WRegion *)frame, &rqg, REGION_FIT_EXACT);
}

WRegion *framed_handler(WWindow *par, const WFitParams *fp, void *ap_) {
  AP *ap = (AP *)ap_;
...
WRegion *framedpholder_do_attach(WFramedPHolder *ph, int flags,
  ...
  return (flags & PHOLDER_ATTACH_RETURN_CREATEROOT ? (WRegion *)frame
                                                   : ap.reg_ret);
}
...
WRegion *framedpholder_do_target(WFramedPHolder *ph) {
  ...
  return (frame != NULL ? frame
                        : (ph->cont != NULL ? pholder_target(ph->cont) : NULL));
}
...
static DynFunTab framedpholder_dynfuntab[] = {
    {(DynFun *)pholder_do_attach, (DynFun *)framedpholder_do_attach},

    {(DynFun *)pholder_do_goto, (DynFun *)framedpholder_do_goto},

    {(DynFun *)pholder_do_target, (DynFun *)framedpholder_do_target},

    {(DynFun *)pholder_stale, (DynFun *)framedpholder_stale},
