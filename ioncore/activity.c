...
bool region_set_activity(WRegion *reg, int sp) {
  ...
  if (!XOR(set, nset)) return nset;

  if (nset) {
    if (REGION_IS_ACTIVE(reg)) return FALSE;

    reg->flags |= REGION_ACTIVITY;
    objlist_insert_last(&actlist, (Obj *)reg);

    if (reg->mgd_activity == 0) propagate_activity(reg);
  } else {
    reg->flags &= ~REGION_ACTIVITY;
    objlist_remove(&actlist, (Obj *)reg);

    if (reg->mgd_activity == 0) propagate_clear(reg);
  }
...
EXTL_SAFE
EXTL_EXPORT_MEMBER
bool region_is_activity(WRegion *reg) { return (reg->flags & REGION_ACTIVITY); }
...
ObjList *ioncore_activity_list() { return actlist; }
