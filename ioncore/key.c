...
static void insstr(WWindow *wwin, XKeyEvent *ev) {
  ...
  if (wwin->xic != NULL) {
    if (XFilterEvent((XEvent *)ev, ev->window)) return;
    n = XmbLookupString(wwin->xic, ev, buf, 16, &ksym, &stat);
    if (stat != XLookupChars && stat != XLookupBoth) return;
  } else {
  ...
  if (ioncore_g.use_mb) {
    if (!iswprint(str_wchar_at(buf, 32))) return;
  } else {
    if (iscntrl(*buf)) return;
  }
...
static bool quote_next_handler(WRegion *reg, XEvent *xev) {
  ...
  if (ioncore_ismod(ev->keycode)) return FALSE;
...
static void waitrelease(WRegion *reg) {
   ...
   */
  ioncore_grab_establish((WRegion *)region_rootwin_of(reg), waitrelease_handler,
                         NULL, 0);
  ioncore_change_grab_cursor(IONCORE_CURSOR_WAITKEY);
}
...
static WBinding *lookup_binding_(WRegion *reg, int act, uint state, uint kcb,
                                 WSubmapState *st, WRegion **binding_owner,
                                 WRegion **subreg) {
  WBinding *binding;
  ...
  *subreg = NULL;
  ...
  do {
    binding = region_lookup_keybinding(reg, act, state, kcb, st, binding_owner);

    if (binding != NULL) break;

    if (OBJ_IS(reg, WRootWin)) break;
...
static WBinding *lookup_binding(WRegion *oreg, int act, uint state, uint kcb,
                                WRegion **binding_owner, WRegion **subreg) {
  WRegion *reg = oreg;

  /* Find the deepest nested active window grabbing this key. */
  while (reg->active_sub != NULL) reg = reg->active_sub;

  return lookup_binding_(reg, act, state, kcb, oreg->submapstat, binding_owner,
                         subreg);
}
...
static int do_key(WRegion *oreg, XKeyEvent *ev) {
  ...
  if (grabbed) {
    ...
    binding =
        region_lookup_keybinding(oreg, BINDING_KEYPRESS, ev->state, ev->keycode,
                                 oreg->submapstat, &binding_owner);
  }
  ...
  if (binding != NULL) {
    ...
    if (binding->submap != NULL) {
      ...
      if (grabbed) XUngrabKeyboard(ioncore_g.dpy, CurrentTime);

      if (ev->state != 0 && !subs && binding->wait) ret = GRAB_WAITRELEASE;
    }
...
static bool submapgrab_handler(WRegion *reg, XEvent *xev) {
  ...
  if (ev->type != KeyPress) {
    if (ioncore_unmod(ev->state, ev->keycode) == 0) {
      ...
      binding = lookup_binding(reg, BINDING_SUBMAP_RELEASEMOD, 0, 0,
                               &binding_owner, &subreg);

      if (binding != NULL) do_call_binding(binding, binding_owner, subreg);
    }
    return FALSE;
  }
