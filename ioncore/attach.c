...
static WRegion *doit_reparent(WRegion *mgr, WWindow *par, const WFitParams *fp,
                              WRegionDoAttachFn *cont, void *cont_param,
                              WRegion *reg) {
  WFitParams fp2;
  WRegion *disposeroot;
  ...
  if (old_scr != NULL) screen_update_notifywin(old_scr);
  ...
  if (!cont(mgr, reg, cont_param)) {
    ...
    if (scr != NULL) {
       ...
       * on recovery.
       */
      int flags = (region_may_control_focus(reg) ? MPLEX_ATTACH_SWITCHTO : 0);
...
static WRegion *doit_load(WRegion *mgr, WWindow *par, const WFitParams *fp,
                          WRegionDoAttachFn *cont, void *cont_param,
                          ExtlTab tab, WPHolder **sm_ph) {
  WRegion *reg = NULL;

  if (extl_table_gets_o(tab, "reg", (Obj **)&reg)) {
    if (!OBJ_IS(reg, WRegion)) return FALSE;
...
WRegion *region_attach_load_helper(WRegion *mgr, WWindow *par,
                                   const WFitParams *fp, WRegionDoAttachFn *fn,
                                   void *fn_param, ExtlTab tab,
                                   WPHolder **sm_ph) {
  return doit_load(mgr, par, fp, fn, fn_param, tab, sm_ph);
}

WRegion *region_attach_helper(WRegion *mgr, WWindow *par, const WFitParams *fp,
                              WRegionDoAttachFn *fn, void *fn_param,
                              const WRegionAttachData *data) {
  if (data->type == REGION_ATTACH_NEW) {
    return doit_new(mgr, par, fp, fn, fn_param, data->u.n.fn, data->u.n.param);
...
bool region_ancestor_check(WRegion *dst, WRegion *reg) {
  ...
  for (reg2 = dst; reg2 != NULL; reg2 = REGION_MANAGER(reg2)) {
    if (reg2 == reg) return FALSE;
  }

  for (reg2 = REGION_PARENT_REG(dst); reg2 != NULL;
       reg2 = REGION_PARENT_REG(reg2)) {
    if (reg2 == reg) return FALSE;
  }
