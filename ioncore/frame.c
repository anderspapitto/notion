...
WFrameMode frame_mode(WFrame *frame) { return frame->mode; }
...
void frame_size_hints(WFrame *frame, WSizeHints *hints_ret) {
  ...
  FRAME_MX_FOR_ALL(sub, frame, tmp) { sizehints_adjust_for(hints_ret, sub); }
  ...
  if (hints_ret->max_set) {
    if (hints_ret->max_width != INT_MAX) hints_ret->max_width += woff;
    if (hints_ret->max_height != INT_MAX) hints_ret->max_height += hoff;
  }
...
static void frame_managed_rqgeom_absolute(WFrame *frame, WRegion *sub,
  ...
  if (!FORWARD_CWIN_RQGEOM(frame)) {
    ...
    rq2.geom = rq->geom;
    rq2.flags = rq->flags & (REGION_RQGEOM_WEAK_ALL | REGION_RQGEOM_TRYONLY |
...
bool frame_set_shaded(WFrame *frame, int sp) {
  ...
  if (!XOR(nset, set)) return nset;

  rq.flags = REGION_RQGEOM_H_ONLY;
  rq.geom = REGION_GEOM(frame);

  if (!nset) {
    if (!(frame->flags & FRAME_SAVED_VERT)) return FALSE;
    rq.geom.h = frame->saved_geom.h;
  } else {
    if (frame->barmode == FRAME_BAR_NONE) return FALSE;
...
void frame_managed_notify(WFrame *frame, WRegion *UNUSED(sub),
  ...
  if (how == ioncore_g.notifies.activated ||
      ...
      how == ioncore_g.notifies.sub_activity || how == ioncore_g.notifies.tag) {
...
WRegion *frame_managed_disposeroot(WFrame *frame, WRegion *reg) {
  if (DEST_EMPTY(frame) && frame->mplex.mgd != NULL &&
      frame->mplex.mgd->reg == reg && frame->mplex.mgd->mgr_next == NULL) {
...
WPHolder *frame_prepare_manage_transient(WFrame *frame,
  ...
  if (framemode_is_floating(frame_mode(frame)) ||
    ...
    return region_prepare_manage_transient_default((WRegion *)frame, transient,
                                                   param, unused);
  } else {
    return NULL;
  }
...
WRegion *frame_load(WWindow *par, const WFitParams *fp, ExtlTab tab) {
  ...
  if (DEST_EMPTY(frame) && frame->mplex.mgd == NULL) {
    if (frame->mplex.misc_phs != NULL) {
      ...
      if (fph != NULL) mplex_migrate_phs_to_fph(&frame->mplex, fph);
    }
