...
bool ioncore_set_drag_handlers(WRegion *reg, WMotionHandler *begin,
                               WMotionHandler *motion, WButtonHandler *end,
                               GrabHandler *keypress,
                               GrabKilledHandler *killed) {
  if (ioncore_pointer_grab_region() == NULL || p_motion) return FALSE;

  /* A motion handler set at this point may not set a begin handler */
  if (p_grabstate != ST_HELD && begin != NULL) return FALSE;
...
WRegion *ioncore_pointer_grab_region() {
  if (p_grabstate == ST_NO) return NULL;
  return p_reg;
}
...
static void call_motion_begin(WBinding *binding, XMotionEvent *ev, int dx,
                              int dy) {
  if (binding == NULL) return;
...
static void finish_pointer() {
  if (p_reg != NULL) window_release((WWindow *)p_reg);
  p_grabstate = ST_NO;
  watch_reset(&p_subregwatch);
}
...
static void pointer_grab_killed(WRegion *UNUSED(unused)) {
  if (p_reg != NULL && p_killed_handler != NULL) p_killed_handler(p_reg);
...
static bool listens_to(WRegion *reg, uint state, uint button, int area) {
  ...
  for (i = 0; i < n_acts; i++) {
    if (region_lookup_binding(reg, acts[i], state, button, area)) return TRUE;
  }
...
static bool ioncore_dodo_handle_buttonpress(XButtonEvent *ev, bool sub) {
  ...
  if (dblclick) {
    pressbind =
        region_lookup_binding(reg, BINDING_BUTTONDBLCLICK, state, button, area);
  }

  if (pressbind == NULL) {
    pressbind =
        region_lookup_binding(reg, BINDING_BUTTONPRESS, state, button, area);
  }
  ...
  if (pressbind == NULL && sub) {
     ...
     */
    if (!dblclick && !listens_to(reg, state, button, area)) return FALSE;
  }
  ...
  watch_setup(&p_regwatch, (Obj *)reg, NULL);
  if (subreg != NULL) watch_setup(&p_subregwatch, (Obj *)subreg, NULL);

  ioncore_grab_establish(reg, handle_key, pointer_grab_killed, 0);
  p_grabstate = ST_HELD;

  if (pressbind != NULL) call_button(pressbind, ev);
...
bool ioncore_do_handle_buttonpress(XButtonEvent *ev) {
  ...
  if (ev->subwindow != None && ev->state != 0) {
    ...
    ev2.subwindow = None;
    if (XTranslateCoordinates(ioncore_g.dpy, ev->window, ev2.window, ev->x,
                              ev->y, &(ev2.x), &(ev2.y), &(ev2.subwindow))) {
      if (ioncore_dodo_handle_buttonpress(&ev2, TRUE)) return TRUE;
    }
  }
...
bool ioncore_do_handle_buttonrelease(XButtonEvent *ev) {
  ...
  if (p_button != ev->button) return FALSE;

  if (p_reg != NULL) {
    if (p_motion == FALSE) {
      p_clickcnt = 1;
      binding = region_lookup_binding(p_reg, BINDING_BUTTONCLICK, p_state,
                                      p_button, p_area);
      if (binding != NULL) call_button(binding, ev);
    } else {
      call_motion_end(ev);
    }
  }
...
void ioncore_do_handle_motionnotify(XMotionEvent *ev) {
  ...
  if (p_reg == NULL) return;

  if (!p_motion) {
    if (motion_in_threshold(ev->x_root, ev->y_root)) return;
    binding = region_lookup_binding(p_reg, BINDING_BUTTONMOTION, p_state,
                                    p_button, p_area);
  }
