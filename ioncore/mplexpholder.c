...
bool mplexpholder_init(WMPlexPHolder *ph, WMPlex *mplex, WStacking *st,
  ...
  if (st != NULL) {
    ...
    if (param == NULL) param = &dummy_param;
    ...
    if (!(param->flags & MPLEX_ATTACH_UNNUMBERED)) {
      ...
      or_after = llist_index_to_after(mplex->mx_list, mplex->mx_current, index);
      after = (index == LLIST_INDEX_LAST
                   ? (or_after != NULL ? LIST_LAST(or_after->phs, next, prev)
...
WMPlexPHolder *create_mplexpholder(WMPlex *mplex, WStacking *st,
                                   WMPlexAttachParams *param) {
...
static WRegion *recreate_handler(WWindow *par, const WFitParams *fp,
                                 void *rp_) {
  RP *rp = (RP *)rp_;
...
static WRegion *mplexpholder_attach_recreate(WMPlexPHolder *ph, int flags,
  ...
  return (flags & PHOLDER_ATTACH_RETURN_CREATEROOT ? (WRegion *)res
                                                   : rp.reg_ret);
}
...
WRegion *mplexpholder_do_attach(WMPlexPHolder *ph, int flags,
  ...
  if (mplex == NULL) return mplexpholder_attach_recreate(ph, flags, data);
...
bool mplexpholder_move(WMPlexPHolder *ph, WMPlex *mplex, WMPlexPHolder *after,
                       WLListNode *or_after) {
  mplexpholder_do_unlink(ph, ph->mplex);

  ph->mplex = mplex;

  if (mplex == NULL) return TRUE;
...
bool mplexpholder_do_goto(WMPlexPHolder *ph) {
  ...
  if (reg != NULL) {
    ...
    return (fph != NULL ? pholder_do_goto((WPHolder *)fph) : FALSE);
  }
}
...
WRegion *mplexpholder_do_target(WMPlexPHolder *ph) {
  ...
  if (reg != NULL) {
    ...
    return (fph != NULL ? pholder_do_target((WPHolder *)fph) : NULL);
  }
}
...
void mplex_move_phs_before(WMPlex *mplex, WLListNode *node) {
  ...
  after = (or_after != NULL ? LIST_LAST(or_after->phs, next, prev)
                            : LIST_LAST(mplex->misc_phs, next, prev));
...
void mplex_flatten_phs(WMPlex *mplex) {
  ...
  FOR_ALL_NODES_ON_LLIST(node, mplex->mx_list, tmp) {
    WMPlexPHolder *last =
        (mplex->misc_phs == NULL ? NULL : mplex->misc_phs->prev);
