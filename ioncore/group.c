...
WStacking *group_get_stacking(WGroup *ws) {
  ...
  return (par == NULL ? NULL : window_get_stacking(par));
}

WStacking **group_get_stackingp(WGroup *ws) {
  WWindow *par = REGION_PARENT(ws);

  return (par == NULL ? NULL : window_get_stackingp(par));
}
...
WRegion *group_iter(WGroupIterTmp *tmp) { return stacking_iter_mgr(tmp); }
...
bool group_fitrep(WGroup *ws, WWindow *par, const WFitParams *fp) {
  ...
  if (par == NULL) {
    if (fp->mode & REGION_FIT_WHATEVER) return TRUE;
    REGION_GEOM(ws) = fp->g;
  } else {
  ...
  if (unweaved != NULL) stacking_weave(&par->stacking, &unweaved, FALSE);
...
static void group_map(WGroup *ws) {
  ...
  FOR_ALL_MANAGED_BY_GROUP(ws, reg, tmp) { region_map(reg); }
}
...
static bool group_managed_prepare_focus(WGroup *ws, WRegion *reg, int flags,
  ...
  if (mplex != NULL) {
    ...
    return mplex_do_prepare_focus(mplex, node, st, flags, res);
  } else {
...
void group_managed_remove(WGroup *ws, WRegion *reg) {
  ...
  if (st != NULL) {
    if (st == ws->bottom) group_do_set_bottom(ws, NULL);

    if (st == ws->managed_stdisp) ws->managed_stdisp = NULL;
...
bool group_init(WGroup *ws, WWindow *par, const WFitParams *fp,
  ...
  ws->dummywin =
      XCreateWindow(ioncore_g.dpy, par->win, fp->g.x, fp->g.y, 1, 1, 0,
...
void group_deinit(WGroup *ws) {
  ...
  FOR_ALL_MANAGED_BY_GROUP(ws, reg, tmp) { destroy_obj((Obj *)reg); }
  ...
  while (ws->phs != NULL) grouppholder_do_unlink(ws->phs);
...
bool group_rescue_clientwins(WGroup *ws, WRescueInfo *info) {
  ...
  return region_rescue_some_clientwins((WRegion *)ws, info,
                                       (WRegionIterator *)group_iter, &tmp);
}

WPHolder *group_get_rescue_pholder_for(WGroup *ws, WRegion *forwhat) {
...
void group_bottom_set(WGroup *grp) { CALL_DYN(group_bottom_set, grp, (grp)); }
...
static void group_do_set_bottom(WGroup *grp, WStacking *st) {
  ...
  if (!OBJ_IS_BEING_DESTROYED(grp)) {
    bool noremanage = ((was == st) || (was == NULL && std == NULL) ||
                       (st != NULL && st == std) || (st == NULL && was == std));

    if (!noremanage && (st == NULL || HAS_DYN(st->reg, region_manage_stdisp))) {
...
WStacking *group_do_add_managed_default(WGroup *ws, WRegion *reg, int level,
  ...
  if (region_is_fully_mapped((WRegion *)ws)) region_map(reg);
...
bool group_do_attach_final(WGroup *ws, WRegion *reg,
  ...
  level = (stabove != NULL
               ? stabove->level
               : (param->level_set ? param->level : STACKING_LEVEL_NORMAL));

  /* Fit */
  szplcy =
      (param->szplcy_set ? param->szplcy
                         : (param->bottom ? SIZEPOLICY_FULL_EXACT
                                          : SIZEPOLICY_VISIBILITY_CONSTRAINED));

  if (!param->whatever) {
    weak =
        (param->geom_weak_set ? param->geom_weak
                              : (param->geom_set ? 0 : REGION_RQGEOM_WEAK_ALL));
  ...
  if (stabove != NULL) st->above = stabove;

  if (param->bottom) group_do_set_bottom(ws, st);
...
void groupattachparams_get(WGroupAttachParams *par, ExtlTab tab,
                           const char *sub) {
  int tmp;
  bool tmpb;
  char *tmps;
  ...
  if (extl_table_gets_i(tab, "sizepolicy", &tmp)) {
    ...
    if (string2sizepolicy(tmps, &par->szplcy)) par->szplcy_set = 1;
    free(tmps);
  }
  ...
  if (extl_table_gets_t(tab, "geom", &g)) {
    ...
    if (extl_table_gets_i(g, "x", &(par->geom.x))) n++;
    if (extl_table_gets_i(g, "y", &(par->geom.y))) n++;
    if (extl_table_gets_i(g, "w", &(par->geom.w))) n++;
    if (extl_table_gets_i(g, "h", &(par->geom.h))) n++;

    if (n == 4) par->geom_set = 1;
...
void group_manage_stdisp(WGroup *ws, WRegion *stdisp,
  ...
  if (b != NULL && !OBJ_IS_BEING_DESTROYED(b) &&
      HAS_DYN(b, region_manage_stdisp)) {
    region_manage_stdisp(b, stdisp, di);
    if (REGION_MANAGER(stdisp) == b) return;
  }
  ...
  if (ws->managed_stdisp != NULL && ws->managed_stdisp->reg == stdisp) {
    if (ws->managed_stdisp->szplcy == szplcy) return;
    ws->managed_stdisp->szplcy = szplcy;
  } else {
    region_detach_manager(stdisp);
    ws->managed_stdisp =
        group_do_add_managed(ws, stdisp, STACKING_LEVEL_ON_TOP, szplcy);
  }
...
static void group_remanage_stdisp(WGroup *ws) {
  ...
  if (mplex != NULL && mplex->mx_current != NULL &&
...
void group_managed_rqgeom(WGroup *ws, WRegion *reg, const WRQGeomParams *rq, WRectangle *geomret) {
  ...
  if (!(rq->flags & REGION_RQGEOM_TRYONLY)) region_fitrep(reg, NULL, &fp);
}
...
static WStacking *prv(WGroup *ws, WStacking *st, bool wrap) {
  return (st != ws->managed_list ? st->mgr_prev : (wrap ? st->mgr_prev : NULL));
}
...
static WStacking *do_get_next(WGroup *ws, WStacking *sti, NxtFn *fn, bool wrap,
                              bool sti_ok) {
...
static WStacking *group_do_navi_first(WGroup *ws, WRegionNavi nh) {
  ...
  if (nh == REGION_NAVI_ANY && ws->current_managed != NULL &&
...
static WStacking *group_do_navi_next(WGroup *ws, WStacking *st, WRegionNavi nh,
                                     bool wrap) {
  if (st == NULL) return group_do_navi_first(ws, nh);
...
WStacking *group_find_stacking(WGroup *ws, WRegion *r) {
  if (r == NULL || REGION_MANAGER(r) != (WRegion *)ws) return NULL;
...
ExtlTab group_get_configuration(WGroup *ws) {
  ...
  FOR_ALL_NODES_IN_GROUP(ws, st, tmp) {
    ...
    if (subtab != extl_table_none()) {
      extl_table_sets_s(subtab, "sizepolicy", sizepolicy2string(st->szplcy));
      ...
      if (ws->bottom == st) extl_table_sets_b(subtab, "bottom", TRUE);
