...
WRegion *xwindow_region_of_t(Window win, const ClassDescr *descr) {
  ...
  if (!obj_is((Obj *)reg, descr)) return NULL;
...
Window create_xwindow(WRootWin *rw, Window par, const WRectangle *geom) {
  ...
  window = XCreateSimpleWindow(ioncore_g.dpy, par, geom->x, geom->y, w, h, 0, 0,
                               BlackPixel(ioncore_g.dpy, rw->xscr));
  /* p[0] = name; */
...
bool xwindow_pointer_pos(Window rel, int *px, int *py) {
  ...
  int rx = 0, ry = 0;
  uint mask = 0;
  return XQueryPointer(ioncore_g.dpy, rel, &realroot, &win, &rx, &ry, px, py,
                       &mask);
}
