...
static const uint modmasks[N_MODS] = {ShiftMask, LockMask, ControlMask,
                                      Mod1Mask,  Mod2Mask, Mod3Mask,
                                      Mod4Mask,  Mod5Mask};

static XModifierKeymap *modmap = NULL;

#define KNOWN_MODIFIERS_MASK                                             \
...
WBinding *bindmap_lookup_binding(WBindmap *bindmap, int act, uint state,
                                 uint kcb) {
  return do_bindmap_lookup_binding(bindmap, act, state, kcb, 0);
}

WBinding *bindmap_lookup_binding_area(WBindmap *bindmap, int act, uint state,
                                      uint kcb, int area) {
  WBinding *binding;
...
int ioncore_modstate() {
  ...
  for (j = 0; j < N_MODS * modmap->max_keypermod; j++) {
    ...
    if (b < 32) {
      if (keys[b] & (1 << a)) state |= modmasks[j / modmap->max_keypermod];
    }
  }
...
static void evil_grab_key(Display *display, uint keycode, uint modifiers,
  ...
  for (i = 0; i < N_EVILLOCKS; i++) {
    ...
    for (j = i; j < N_EVILLOCKS; j++) {
      ...
      XGrabKey(display, keycode, mods, grab_window, owner_events, pointer_mode,
               keyboard_mode);
      if (i == j) continue;
      XGrabKey(display, keycode,
               modifiers | evillockmasks[i] | evillockmasks[j], grab_window,
...
static void evil_grab_button(Display *display, uint button, uint modifiers,
  ...
  for (i = 0; i < N_EVILLOCKS; i++) {
    ...
    for (j = i; j < N_EVILLOCKS; j++) {
      if (evillockmasks[j] == 0) continue;
      mods |= evillockmasks[j];
      XGrabButton(display, button, mods, grab_window, owner_events, event_mask,
                  pointer_mode, keyboard_mode, confine_to, cursor);
      if (i == j) continue;
