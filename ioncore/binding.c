...
static const uint modmasks[N_MODS] = {ShiftMask, LockMask, ControlMask, Mod1Mask,  Mod2Mask, Mod3Mask, Mod4Mask,  Mod5Mask};
...
static uint evillockmasks[N_EVILLOCKS] = {0, 0, LockMask};
static const KeySym evillocks[N_LOOKUPEVIL] = {XK_Num_Lock, XK_Scroll_Lock};
static uint evilignoremask = LockMask;
static void lookup_evil_locks();
static void evil_grab_key(Display *display, uint keycode, uint modifiers, Window grab_window, bool owner_events, int pointer_mode, int keyboard_mode);
...
static void bindmap_deinit(WBindmap *bindmap) {
    ...
    while (bindmap->rbind_list != NULL) {
        region_remove_bindmap(bindmap->rbind_list->reg, bindmap);
    }

    if (bindmap->bindings == NULL) return;
    ...
    FOR_ALL_BINDINGS(b, node, bindmap->bindings) {
        ...
        *b2 = *b;
        if (b->act == BINDING_KEYPRESS) {
            for (rbind = bindmap->rbind_list; rbind != NULL;
                 rbind = rbind->bm_next)
...
bool bindmap_remove_binding(WBindmap *bindmap, const WBinding *b) {
    ...
    Rb_node node = NULL;
    int found = 0;
    if (bindmap->bindings == NULL) return FALSE;
    node = rb_find_gkey_n(bindmap->bindings, b, (Rb_compfn *)compare_bindings, &found);
...
static WBinding *search_binding(WBindmap *bindmap, WBinding *binding) {
    ...
    if (bindmap->bindings == NULL) return NULL;
...
static WBinding *do_bindmap_lookup_binding(WBindmap *bindmap, int act,
...
WBinding *bindmap_lookup_binding_area(WBindmap *bindmap, int act, uint state, uint kcb, int area) {
    WBinding *binding;
...
bool ioncore_ismod(int keycode) {
    int j;
    for (j = 0; j < N_MODS * modmap->max_keypermod; j++) {
        if (modmap->modifiermap[j] == keycode) return TRUE;
    }
    return FALSE;
}
...
static void evil_grab_key(Display *display, uint keycode, uint modifiers,
    ...
    if (modifiers == AnyModifier) return;
    for (i = 0; i < N_EVILLOCKS; i++) {
        if (evillockmasks[i] == 0) continue;
        mods = modifiers;
        for (j = i; j < N_EVILLOCKS; j++) {
            if (evillockmasks[j] == 0) continue;
            mods |= evillockmasks[j];
            XGrabKey(display, keycode, mods, grab_window, owner_events,
                     pointer_mode, keyboard_mode);
            if (i == j) continue;
...
static void evil_grab_button(Display *display, uint button, uint modifiers,
    ...
    for (i = 0; i < N_EVILLOCKS; i++) {
        ...
        for (j = i; j < N_EVILLOCKS; j++) {
            if (evillockmasks[j] == 0) continue;
            mods |= evillockmasks[j];
            XGrabButton(display, button, mods, grab_window, owner_events,
