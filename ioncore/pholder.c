...
bool pholder_init(WPHolder *UNUSED(ph)) { return TRUE; }

void pholder_deinit(WPHolder *UNUSED(ph)) {}

WRegion *pholder_do_attach(WPHolder *ph, int flags, WRegionAttachData *data)
...
bool pholder_attach_mcfgoto(WPHolder *ph, int flags, WRegion *reg) {
  ...
  if (!pholder_attach(ph, flags, reg)) return FALSE;

  if (cf) region_goto(reg);
...
WRegion *pholder_target(WPHolder *ph) { return pholder_do_target(ph); }

static bool pholder_do_check_reparent_default(WPHolder *ph, WRegion *reg) {
  WRegion *target = pholder_do_target(ph);

  return (target == NULL ? FALSE : region_ancestor_check(target, reg));
}
...
bool pholder_goto(WPHolder *ph) { return pholder_do_goto(ph); }
...
WPHolder *region_managed_get_pholder(WRegion *reg, WRegion *mgd) {
  WPHolder *ret = NULL;
  CALL_DYN_RET(ret, WPHolder *, region_managed_get_pholder, reg, (reg, mgd));
  return ret;
}
...
WPHolder *region_get_rescue_pholder_for(WRegion *reg, WRegion *mgd) {
  if (OBJ_IS_BEING_DESTROYED(reg) || reg->flags & REGION_CWINS_BEING_RESCUED) {
    ...
    CALL_DYN_RET(ret, WPHolder *, region_get_rescue_pholder_for, reg,
                 (reg, mgd));
    return ret;
  }
}
...
static DynFunTab pholder_dynfuntab[] = {
    ...
    {(DynFun *)pholder_stale, (DynFun *)pholder_stale_default},
