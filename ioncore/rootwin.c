...
static int my_error_handler(Display *dpy, XErrorEvent *ev) {
  ...
  if (request[0] == '\0') snprintf(request, 64, "<unknown request>");

  if (ev->minor_code != 0) {
    warn("[%d] %s (%d.%d) %#lx: %s", ev->serial, request, ev->request_code,
         ev->minor_code, ev->resourceid, msg);
  } else {
...
static void scan_initial_windows(WRootWin *rootwin) {
  ...
  for (i = 0; i < nwins; i++) {
    ...
    if (hints != NULL) XFree((void *)hints);
  }
...
void rootwin_manage_initial_windows(WRootWin *rootwin) {
  ...
  for (i = 0; i < nwins; i++) {
    if (XWINDOW_REGION_OF(wins[i]) != NULL) wins[i] = None;
    if (wins[i] == None) continue;
    if (XGetTransientForHint(ioncore_g.dpy, wins[i], &tfor)) continue;
...
static void preinit_gr(WRootWin *rootwin) {
  ...
  gcvmask = (GCLineStyle | GCLineWidth | GCFillStyle | GCJoinStyle |
             GCCapStyle | GCFunction | GCSubwindowMode | GCForeground);
...
void rootwin_deinit(WRootWin *rw) {
  ...
  FOR_ALL_SCREENS_W_NEXT(scr, next) {
    if (REGION_MANAGER(scr) == (WRegion *)rw) destroy_obj((Obj *)scr);
  }
...
WScreen *rootwin_current_scr(WRootWin *rootwin) {
  ...
  FOR_ALL_SCREENS(scr) {
    if (REGION_MANAGER(scr) == (WRegion *)rootwin && REGION_IS_MAPPED(scr)) {
      fb = scr;
      if (REGION_IS_ACTIVE(scr)) return scr;
    }
  }
