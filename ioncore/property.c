...
static ulong xwindow_get_property_(Window win, Atom atom, Atom type,
  ...
  do {
    status = XGetWindowProperty(ioncore_g.dpy, win, atom, 0L, n32expected,
                                False, type, &real_type, format, &n, &extra, p);

    if (status != Success || *p == NULL) return -1;

    if (extra == 0 || !more) break;
...
ulong xwindow_get_property(Window win, Atom atom, Atom type, ulong n32expected,
                           bool more, uchar **p) {
  int format = 0;
...
void xwindow_set_string_property(Window win, Atom a, const char *value) {
  if (value == NULL) {
    ...
    XChangeProperty(ioncore_g.dpy, win, a, XA_STRING, 8, PropModeReplace,
...
void xwindow_set_integer_property(Window win, Atom a, int value) {
  ...
  XChangeProperty(ioncore_g.dpy, win, a, XA_INTEGER, 32, PropModeReplace,
                  (uchar *)data, 1);
}
...
void xwindow_set_state_property(Window win, int state) {
  ...
  XChangeProperty(ioncore_g.dpy, win, ioncore_g.atom_wm_state,
                  ioncore_g.atom_wm_state, 32, PropModeReplace, (uchar *)data,
                  2);
}
...
char **xwindow_get_text_property(Window win, Atom a, int *nret) {
  ...
  if (!ok || n == 0 || list == NULL) return NULL;
...
#endif
      st = XmbTextListToTextProperty(ioncore_g.dpy, (char **)ptr, n, XTextStyle,
                                     &prop);
    ok = (st >= 0);
  } else {
...
ExtlTab ioncore_x_get_window_property(int win, int atom, int atom_type,
  ...
  if (p == NULL) return extl_table_none();
 ...
 */
EXTL_EXPORT
void ioncore_x_change_property(int win, int atom, int atom_type, int format,
                               const char *mode, ExtlTab tab) {
...
ExtlTab ioncore_x_get_text_property(int win, int atom) {
  ...
  if (list != NULL) {
    if (n != 0) {
      tab = extl_create_table();
      for (i = 0; i < n; i++) extl_table_seti_s(tab, i + 1, list[i]);
    }
...
/*EXTL_DOC
 * Set a \code{UTF8\_STRING} property for a window. The fields of \var{tab}
 * starting
