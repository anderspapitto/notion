...
static void grab_kb_ptr(Window win, Window confine_to, int cursor,
  ...
  XGrabKeyboard(ioncore_g.dpy, win, False, GrabModeAsync, GrabModeAsync,
                CurrentTime);
...
static void do_grab_install(GrabStatus *grab) {
  ...
  grab_kb_ptr(region_root_of(grab->holder), grab->confine_to, grab->cursor,
              grab->eventmask);
  current_grab = grab;
}
...
static void mark_for_removal(GrabStatus *grab, bool killed) {
  ...
  if (grabs[idx_grab - 1].remove) do_grab_remove();
}
...
static void do_holder_remove(WRegion *holder, bool killed) {
  ...
  for (i = idx_grab - 1; i >= 0; i--) {
    if (grabs[i].holder == holder) mark_for_removal(grabs + i, killed);
  }
}
...
bool ioncore_handle_grabs(XEvent *ev) {
  ...
  while (current_grab && current_grab->remove) do_grab_remove();
  ...
  if (ev->type != KeyRelease && ev->type != KeyPress) return FALSE;
...
bool ioncore_grab_held() { return idx_grab > 0; }
...
void ioncore_grab_confine_to(Window confine_to) {
  if (current_grab != NULL) {
    ...
    XGrabPointer(ioncore_g.dpy, region_root_of(current_grab->holder), True,
