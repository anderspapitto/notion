...
WPHolder *groupcw_prepare_manage(WGroupCW *cwg, const WClientWin *cwin,
  ...
  if (!MANAGE_PRIORITY_OK(priority, MANAGE_PRIORITY_GROUP)) return NULL;

  /* Only catch windows with transient mode set to current here. */
  if (clientwin_get_transient_mode(cwin) != TRANSIENT_MODE_CURRENT) return NULL;

  return groupcw_transient_pholder(cwg, cwin, param);
}

static bool groupcw_should_manage_transient(WGroupCW *cwg, WClientWin *tfor) {
  WRegion *mgr;

  if (group_find_stacking(&cwg->grp, (WRegion *)tfor)) return TRUE;
...
WPHolder *groupcw_prepare_manage_transient(WGroupCW *cwg,
                                           ...
                                           int unused) {
  WPHolder *ph = region_prepare_manage_transient_default(
      (WRegion *)cwg, transient, param, unused);
...
void groupcw_toggle_transients_pos(WGroupCW *cwg) {
  ...
  FOR_ALL_NODES_IN_GROUP(&cwg->grp, st, tmp) {
    ...
    if (st->reg != NULL) {
      ...
      sizepolicy(&st->szplcy, st->reg, NULL, REGION_RQGEOM_WEAK_ALL, &fp);
...
const char *groupcw_displayname(WGroupCW *cwg) {
  ...
  if (name == NULL) name = region_name((WRegion *)cwg);
...
static void group_migrate_phs_to_ph(WGroup *group, WPHolder *rph) {
  ...
  for (ph = phs; ph != NULL; ph = ph->next) ph->group = NULL;
}
...
bool groupcw_rescue_clientwins(WGroupCW *cwg, WRescueInfo *info) {
  ...
  if (cwg->grp.phs != NULL) {
    ...
    if (rph != NULL) group_migrate_phs_to_ph(&cwg->grp, rph);
  }
...
bool groupcw_init(WGroupCW *cwg, WWindow *parent, const WFitParams *fp) {
  ...
  if (!group_init(&(cwg->grp), parent, fp, "Notion GroupCW")) return FALSE;
...
void groupcw_deinit(WGroupCW *cwg) { group_deinit(&(cwg->grp)); }
...
WRegion *groupcw_load(WWindow *par, const WFitParams *fp, ExtlTab tab) {
  ...
  if (cwg->grp.managed_list == NULL) {
    if (cwg->grp.phs != NULL) {
      ...
      if (ph != NULL) group_migrate_phs_to_ph(&cwg->grp, ph);
    }
    destroy_obj((Obj *)cwg);
    return NULL;
  }
...
static DynFunTab groupcw_dynfuntab[] = {
    {(DynFun *)region_prepare_manage, (DynFun *)groupcw_prepare_manage},
    ...
    */
    ...
    {(DynFun *)region_displayname, (DynFun *)groupcw_displayname},

    {region_managed_notify, groupcw_managed_notify},

    {group_bottom_set, groupcw_bottom_set},
