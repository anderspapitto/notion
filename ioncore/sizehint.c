...
static void correct_aspect(int max_w, int max_h, const WSizeHints *hints,
                           int *wret, int *hret) {
  if (!hints->aspect_set) return;

  if (*wret * hints->max_aspect.y > *hret * hints->max_aspect.x) {
    do_correct_aspect(max_w, max_h, hints->min_aspect.x, hints->min_aspect.y,
                      wret, hret);
  }

  if (*wret * hints->min_aspect.y < *hret * hints->min_aspect.x) {
    do_correct_aspect(max_w, max_h, hints->max_aspect.x, hints->max_aspect.y,
                      wret, hret);
  }
}
...
void sizehints_correct(const WSizeHints *hints, int *wp, int *hp, bool min,
  ...
  if (tw >= 0 && th >= 0) correct_aspect(tw, th, hints, &tw, &th);

  if (hints->inc_set) {
    if (tw > 0) tw = (tw / hints->width_inc) * hints->width_inc;
    if (th > 0) th = (th / hints->height_inc) * hints->height_inc;
  }
...
void sizehints_adjust_for(WSizeHints *hints, WRegion *reg) {
  ...
  if (tmp_hints.min_set) {
    if (!hints->min_set) {
      ...
      hints->min_width = maxof(hints->min_width, tmp_hints.min_width);
      hints->min_height = maxof(hints->min_height, tmp_hints.min_height);
    }
  }

  if (tmp_hints.max_set && hints->max_set) {
    hints->max_width = maxof(hints->max_width, tmp_hints.max_width);
    hints->max_height = maxof(hints->max_height, tmp_hints.max_height);
  } else {
    hints->max_set = FALSE;
  }
}
