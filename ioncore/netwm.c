...
void netwm_init_rootwin(WRootWin *rw) {
  ...
  XChangeProperty(ioncore_g.dpy, WROOTWIN_ROOT(rw),
                  atom_net_supporting_wm_check, XA_WINDOW, 32, PropModeReplace,
                  (uchar *)&(rw->dummy_win), 1);
  XChangeProperty(ioncore_g.dpy, rw->dummy_win, atom_net_supporting_wm_check,
                  XA_WINDOW, 32, PropModeReplace, (uchar *)&(rw->dummy_win), 1);
  XChangeProperty(ioncore_g.dpy, WROOTWIN_ROOT(rw), atom_net_supported, XA_ATOM,
...
bool netwm_check_initial_fullscreen(WClientWin *cwin) {
  ...
  n = xwindow_get_property(cwin->win, atom_net_wm_state, XA_ATOM, 1, TRUE,
                           (uchar **)&data);

  if (n < 0) return FALSE;

  for (i = 0; i < n; i++) {
    if (data[i] == (long)atom_net_wm_state_fullscreen) return TRUE;
  }
...
void netwm_update_state(WClientWin *cwin) {
  ...
  if (REGION_IS_FULLSCREEN(cwin)) data[n++] = atom_net_wm_state_fullscreen;
  if (region_is_activity_r(&(cwin->region)))
    data[n++] = atom_net_wm_state_demands_attention;

  XChangeProperty(ioncore_g.dpy, cwin->win, atom_net_wm_state, XA_ATOM, 32,
                  PropModeReplace, (uchar *)data, n);
}
...
static void netwm_state_change_rq(WClientWin *cwin,
  ...
  if (!REGION_IS_FULLSCREEN(cwin)) {
    if (ev->data.l[0] == _NET_WM_STATE_ADD ||
      ...
      if (!region_enter_fullscreen(grp, sw)) cwin->flags &= ~CLIENTWIN_FS_RQ;
    } else {
      /* Should not be set.. */
      cwin->flags &= ~CLIENTWIN_FS_RQ;
    }
...
void netwm_set_active(WRegion *reg) {
  ...
  if (OBJ_IS(reg, WClientWin)) data[0] = region_xwindow(reg);

  /* The spec doesn't say how multihead should be handled, so
   * we just update the root window the window is on.
   */
  XChangeProperty(ioncore_g.dpy, region_root_of(reg), atom_net_active_window,
                  XA_WINDOW, 32, PropModeReplace, (uchar *)data, 1);
}
...
void ioncore_screens_updated(WRootWin *rw) {
  ...
  XChangeProperty(ioncore_g.dpy, WROOTWIN_ROOT(rw), atom_net_virtual_roots,
                  XA_WINDOW, 32, PropModeReplace, (uchar *)virtualroots,
                  n_screens);
