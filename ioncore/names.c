...
static bool initialise_ns(WNamespace *ns) {
  if (ns->initialised) return TRUE;

  if (ns->rb == NULL) ns->rb = make_rb();
...
static int parseinst(const char *name, const char **startinst) {
  ...
  *startinst = name + l;
  ...
  if (inst < 0 || p3 != name + l - 1) return -1;
...
static bool separated(const WRegionNameInfo *ni1, const WRegionNameInfo *ni2,
                      int *i1ret) {
  int l1, l2;
  int i1, i2;
  ...
  *i1ret = i1;
  ...
  if (memcmp(ni1->name, ni2->name, l1) != 0) return TRUE;
...
static bool do_use_name(WRegion *reg, WNamespace *ns, const char *name,
  ...
  if (!make_full_name(&ni, name, inst, parsed_inst >= 0)) return FALSE;
...
EXTL_SAFE
EXTL_EXPORT_MEMBER
const char *region_name(WRegion *reg) { return reg->ni.name; }
...
static bool do_set_name(bool (*fn)(WRegion *reg, WNamespace *ns, const char *p),
  ...
  if (!initialise_ns(ns)) return FALSE;
...
bool region_register(WRegion *reg) {
  ...
  if (!initialise_ns(&ioncore_internal_ns)) return FALSE;
...
bool clientwin_register(WClientWin *cwin) {
  ...
  if (!initialise_ns(&ioncore_clientwin_ns)) return FALSE;
...
bool clientwin_set_name(WClientWin *cwin, const char *p) {
  return do_set_name(use_name_anyinst, (WRegion *)cwin, &ioncore_clientwin_ns,
                     p);
}
...
static WRegion *do_lookup_region(WNamespace *ns, const char *cname,
  ...
  if (cname == NULL || !ns->initialised) return NULL;
  ...
  if (typenam != NULL && !obj_is_str((Obj *)node->v.val, typenam)) return NULL;
...
static bool do_list(ExtlFn fn, WNamespace *ns, const char *typenam) {
  ...
  rb_traverse(node, ns->rb) {
    ...
    if (typenam != NULL && !obj_is_str((Obj *)reg, typenam)) continue;

    if (!extl_iter_obj(fn, (Obj *)reg)) return FALSE;
  }
