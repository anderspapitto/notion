 ...
 */
EXTL_EXPORT_MEMBER
void input_scrollup(WInput *input) { CALL_DYN(input_scrollup, input, (input)); }
...
void input_refit(WInput *input) { input_do_refit(input, NULL); }
...
void input_updategr(WInput *input) {
  ...
  brush = gr_get_brush(input->win.win, region_rootwin_of((WRegion *)input),
                       input_style(input));

  if (brush == NULL) return;

  if (input->brush != NULL) grbrush_release(input->brush);
  input->brush = brush;
  input_refit(input);
...
bool input_init(WInput *input, WWindow *par, const WFitParams *fp) {
  ...
  if (!window_init((WWindow *)input, par, fp, "WInput")) return FALSE;

  win = input->win.win;

  input->brush =
      gr_get_brush(win, region_rootwin_of((WRegion *)par), input_style(input));

  if (input->brush == NULL) goto fail;
 ...
 */
EXTL_EXPORT_MEMBER
void input_cancel(WInput *input) { region_defer_rqdispose((WRegion *)input); }
...
void mod_query_get_minimum_extents(GrBrush *brush, bool with_spacing, int *w,
                                   int *h) {
  GrBorderWidths bdw;
  GrFontExtents fnte;
  int spc;
