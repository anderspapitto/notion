...
static void wmsg_draw(WMessage *wmsg, bool complete) {
  ...
  if (WMSG_BRUSH(wmsg) == NULL) return;

  get_geom(wmsg, FALSE, &geom);

  grbrush_begin(WMSG_BRUSH(wmsg), &geom, (complete ? 0 : GRBRUSH_NO_CLEAR_OK));

  grbrush_set_attr(WMSG_BRUSH(wmsg), REGION_IS_ACTIVE(wmsg)
                                         ? GR_ATTR(active)
                                         : GR_ATTR(inactive));

  draw_listing(WMSG_BRUSH(wmsg), &geom, &(wmsg->listing), FALSE, GRATTR_NONE);
...
static void wmsg_scrollup(WMessage *wmsg) {
  if (scrollup_listing(&(wmsg->listing))) wmsg_draw(wmsg, TRUE);
}

static void wmsg_scrolldown(WMessage *wmsg) {
  if (scrolldown_listing(&(wmsg->listing))) wmsg_draw(wmsg, TRUE);
}
...
static void wmsg_deinit(WMessage *wmsg) {
  if (wmsg->listing.strs != NULL) deinit_listing(&(wmsg->listing));
