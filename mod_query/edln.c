...
#define UPDATE(X) edln->ui_update(edln->uiptr, X, 0)

#define UPDATE_MOVED(X) edln->ui_update(edln->uiptr, X, EDLN_UPDATE_MOVED)

#define UPDATE_CHANGED(X) \
  edln->ui_update(edln->uiptr, X, EDLN_UPDATE_MOVED | EDLN_UPDATE_CHANGED)

#define UPDATE_CHANGED_NOMOVE(X) \
  edln->ui_update(edln->uiptr, X, EDLN_UPDATE_CHANGED)
...
static bool edln_pspc(Edln *edln, int n) {
  ...
  if (edln->mark > edln->point) edln->mark += n;
...
static bool edln_rspc(Edln *edln, int n) {
  ...
  if (n + edln->point >= edln->psize) n = edln->psize - edln->point;
...
bool edln_insstr_n(Edln *edln, const char *str, int l, bool update,
                   bool movepoint) {
  if (!edln_pspc(edln, l)) return FALSE;

  memmove(&(edln->p[edln->point]), str, l);
  if (movepoint) {
    edln->point += l;
    if (update) UPDATE_CHANGED(edln->point - l);
  } else {
    if (update) UPDATE_CHANGED_NOMOVE(edln->point - l);
  }
...
bool edln_transpose_chars(Edln *edln) {
  ...
  if ((edln->point == 0) || (edln->psize < 2)) return FALSE;

  pos = edln->point;
  if (edln->point == edln->psize) pos = pos - str_prevoff(edln->p, edln->point);
  ...
  if (edln->point != edln->psize) edln->point += off1;
...
bool edln_transpose_words(Edln *edln) {
  ...
  if ((edln->point == edln->psize) || (edln->psize < 3)) return FALSE;
  ...
  m2 = edln->point;
  edln_skip_word(edln);
  if (edln->point == m2) goto noact;
  m4 = edln->point;
  edln_bskip_word(edln);
  if (edln->point == m1) goto noact;
  m3 = edln->point;
...
void edln_skip_word(Edln *edln) {
  ...
  while (edln->point < edln->psize) {
    ...
    if (ISALNUM(c)) goto fnd;
    if (do_edln_forward(edln) == 0) break;
  }
  UPDATE_MOVED(oldp);
  return;
...
static void do_set_mark(Edln *edln, int nm) {
  int m = edln->mark;
  edln->mark = nm;
  if (m != -1) UPDATE(m < edln->point ? m : edln->point);
}

void edln_set_mark(Edln *edln) { do_set_mark(edln, edln->point); }

void edln_clear_mark(Edln *edln) { do_set_mark(edln, -1); }

static void edln_do_copy(Edln *edln, bool del) {
  int beg, end;

  if (edln->mark < 0 || edln->point == edln->mark) return;
...
void edln_cut(Edln *edln) { edln_do_copy(edln, TRUE); }

void edln_copy(Edln *edln) { edln_do_copy(edln, FALSE); }
...
bool edln_set_context(Edln *edln, const char *str) {
  ...
  if (edln->context != NULL) free(edln->context);
  edln->context = s;
...
static void edln_do_set_hist(Edln *edln, int e, bool match) {
  ...
  if (str != NULL) {
    ...
    edln->point = (match ? MINOF(edln->point, edln->psize) : edln->psize);
    edln->mark = -1;
    edln->modified = FALSE;
    UPDATE_NEW();
  }
...
static int search(Edln *edln, int from, bool bwd, bool match) {
  ...
  if (match && edln->point > 0) {
    ...
    if (tmpstr == NULL) return edln->histent;
...
void edln_history_prev(Edln *edln, bool match) {
  ...
  if (e >= 0) edln_do_set_hist(edln, e, match);
}

void edln_history_next(Edln *edln, bool match) {
  int e = edln->histent;

  if (edln->histent < 0) return;
...
bool edln_init(Edln *edln, const char *p) {
  ...
  if (!edln_initstr(edln, p)) return FALSE;
...
char *edln_finish(Edln *edln) {
  ...
  if (p != NULL) {
    ...
    if (hist != NULL) mod_query_history_push_(hist);
  }
