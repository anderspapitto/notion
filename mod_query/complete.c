...
static void edln_do_set_completion(Edln *edln, const char *comp, int len,
  ...
  if (beg != NULL) edln_insstr_n(edln, beg, strlen(beg), FALSE, TRUE);

  if (len > 0) edln_insstr_n(edln, comp, len, FALSE, TRUE);

  if (end != NULL) edln_insstr_n(edln, end, strlen(end), FALSE, FALSE);

  if (edln->ui_update != NULL) {
    edln->ui_update(edln->uiptr, 0,
                    EDLN_UPDATE_MOVED | EDLN_UPDATE_CHANGED | EDLN_UPDATE_NEW);
  }
}

void edln_set_completion(Edln *edln, const char *comp, const char *beg,
                         const char *end) {
...
int edln_do_completions(Edln *edln, char **completions, int ncomp,
  ...
  if (ncomp == 0) {
    ...
    if (!nosort) qsort(completions, ncomp, sizeof(char **), compare);
    len = get_common_part_rmdup(completions, &ncomp);
  }

  if (setcommon) edln_do_set_completion(edln, completions[0], len, beg, end);
...
bool complproxy_init(WComplProxy *proxy, WEdln *wedln, int id, int cycle) {
  ...
  if (!watch_setup(&(proxy->wedln_watch), (Obj *)wedln, NULL)) return FALSE;
