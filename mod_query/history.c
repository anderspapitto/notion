...
void mod_query_history_push_(char *str) {
  ...
  hist_head--;
  if (hist_head < 0) hist_head = HISTORY_SIZE - 1;
...
void mod_query_history_clear() {
  while (hist_count != 0) {
    free(hist[hist_head]);
    hist_count--;
    if (++hist_head == HISTORY_SIZE) hist_head = 0;
  }
...
static bool match(const char *h, const char *b, bool exact) {
  ...
  return (exact ? strcmp(h, b) == 0 : strncmp(h, b, strlen(b)) == 0);
}
...
int mod_query_history_search(const char *s, int from, bool bwd, bool exact) {
  while (1) {
    ...
    if (i < 0) return -1;
    if (match(hist[i], s, exact)) return from;
    if (bwd)
      from--;
    else
      from++;
