...
#define DSTRSECT(LEN, A)                                                     \
  if (LEN > 0) {                                                             \
    tx += wedln_draw_strsect(wedln, geom->x + tx, ty, str, LEN, GR_ATTR(A)); \
    str += LEN;                                                              \
    len -= LEN;                                                              \
  }
...
static void wedln_do_draw_str_box(WEdln *wedln, const WRectangle *geom,
                                  const char *str, int cursor, int mark,
                                  int tx) {
  ...
  if (mark <= cursor) {
    ...
    if (len == 0) {
      tx +=
          wedln_draw_strsect(wedln, geom->x + tx, ty, " ", 1, GR_ATTR(cursor));
    } else {
...
static void wedln_draw_str_box(WEdln *wedln, const WRectangle *geom, int vstart,
                               const char *str, int dstart, int point,
                               int mark) {
  int tx = 0;
...
static bool wedln_update_cursor(WEdln *wedln, int iw) {
  ...
  if (point < wedln->vstart) wedln->vstart = point;

  if (wedln->vstart == point) return FALSE;
...
static void wedln_calc_size(WEdln *wedln, WRectangle *geom) {
  ...
  if (WEDLN_BRUSH(wedln) == NULL) return;

  if (wedln->prompt != NULL) {
    wedln->prompt_w = grbrush_get_text_width(WEDLN_BRUSH(wedln), wedln->prompt,
                                             wedln->prompt_len);
  }

  if (wedln->info != NULL) {
    wedln->info_w = grbrush_get_text_width(WEDLN_BRUSH(wedln), wedln->info,
                                           wedln->info_len);
  }
...
void wedln_draw_completions(WEdln *wedln, int mode) {
  ...
  if (wedln->compl_list.strs != NULL && WEDLN_BRUSH(wedln) != NULL) {
    ...
    draw_listing(WEDLN_BRUSH(wedln), &geom, &(wedln->compl_list), mode,
                 GR_ATTR(selection));
  }
}
...
void wedln_draw_textarea(WEdln *wedln) {
  ...
  if (wedln->prompt != NULL) {
    grbrush_draw_string(WEDLN_BRUSH(wedln), geom.x, ty, wedln->prompt,
                        wedln->prompt_len, TRUE);
  }
  ...
  if (wedln->info != NULL) {
    ...
    grbrush_draw_string(WEDLN_BRUSH(wedln), x, ty, wedln->info, wedln->info_len,
                        TRUE);
...
static void wedln_draw_(WEdln *wedln, bool complete, bool completions) {
  ...
  if (completions) wedln_draw_completions(wedln, LISTING_DRAW_ALL);
...
void wedln_scrollup_completions(WEdln *wedln) {
  if (wedln->compl_list.strs == NULL) return;
...
bool wedln_do_set_completions(WEdln *wedln, char **ptr, int n, char *beg,
                              char *end, int cycle, bool nosort) {
  int sel = -1;

  if (wedln->compl_beg != NULL) free(wedln->compl_beg);

  if (wedln->compl_end != NULL) free(wedln->compl_end);
...
static ExtlExportedFn *sc_safe_fns[] = {
    (ExtlExportedFn *)&complproxy_set_completions, NULL};
...
bool wedln_next_completion(WEdln *wedln) {
  ...
  if (wedln->compl_current_id != wedln->compl_waiting_id) return FALSE;

  if (wedln->compl_list.nstrs <= 0) return FALSE;
  ...
  if (n != wedln->compl_list.selected_str) wedln_do_select_completion(wedln, n);
...
void wedln_complete(WEdln *wedln, const char *cycle, const char *mode) {
  ...
  if (mode != NULL) {
    ...
    if (!valid) {
      wedln_set_info(wedln, (wedln->compl_history_mode ? TR("history") : NULL));
    }
  }
  ...
  if (valid && cyclei != 0 && mod_query_config.autoshowcompl &&
    ...
    if (!wedln_do_call_completor(wedln, wedln_alloc_compl_id(wedln), cyclei)) {
 ...
 */
EXTL_EXPORT_MEMBER
bool wedln_is_histcompl(WEdln *wedln) { return wedln->compl_history_mode; }
...
static void wedln_update_handler(WEdln *wedln, int from, int flags) {
  ...
  if (flags & EDLN_UPDATE_NEW) wedln->vstart = 0;

  if (flags & EDLN_UPDATE_MOVED) {
    if (wedln_update_cursor(wedln, geom.w)) from = wedln->vstart;
  }
  ...
  if (update_nocompl == 0 && mod_query_config.autoshowcompl &&
    ...
    if (wedln->autoshowcompl_timer != NULL) {
      ...
      timer_set(wedln->autoshowcompl_timer,
                mod_query_config.autoshowcompl_delay, timed_complete,
                (Obj *)wedln);
    }
  }
}
...
static bool wedln_init(WEdln *wedln, WWindow *par, const WFitParams *fp,
  ...
  if (!wedln_init_prompt(wedln, params->prompt)) return FALSE;
...
static void wedln_deinit(WEdln *wedln) {
  if (wedln->prompt != NULL) free(wedln->prompt);

  if (wedln->info != NULL) free(wedln->info);

  if (wedln->compl_beg != NULL) free(wedln->compl_beg);

  if (wedln->compl_end != NULL) free(wedln->compl_end);

  if (wedln->compl_list.strs != NULL) deinit_listing(&(wedln->compl_list));

  if (wedln->autoshowcompl_timer != NULL)
    destroy_obj((Obj *)wedln->autoshowcompl_timer);

  if (wedln->cycle_bindmap != NULL) bindmap_destroy(wedln->cycle_bindmap);
...
static void wedln_do_finish(WEdln *wedln) {
  ...
  if (p != NULL) extl_call(handler, "s", NULL, p);
...
static const char *wedln_style(WEdln *UNUSED(wedln)) { return "input-edln"; }
