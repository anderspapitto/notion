...
static void string_do_calc_parts(GrBrush *brush, int maxw, char *str, int l,
                                 WListingItemInfo *iinf, int wrapw, int ciw) {
  ...
  if (iinf->part_lens != NULL) iinf->part_lens[i] = l2;
}
...
static void draw_multirow(GrBrush *brush, int x, int y, int h, char *str,
                          WListingItemInfo *iinf, int maxw, int ciw,
                          int wrapw) {
  int i, l;
...
static bool one_row_down(WListing *l, int *ip, int *rp) {
  ...
  if (i == l->nitemcol - 1) return FALSE;
...
void setup_listing(WListing *l, char **strs, int nstrs, bool onecol) {
  if (l->strs != NULL) deinit_listing(l);
...
void fit_listing(GrBrush *brush, const WRectangle *geom, WListing *l) {
  ...
  if (l->itemh > 0) visrow = h / l->itemh;

  if (visrow > nrow) visrow = nrow;
...
void deinit_listing(WListing *l) {
  if (l->strs == NULL) return;

  while (l->nstrs--) {
    free(l->strs[l->nstrs]);
    if (l->iteminfos != NULL) reset_iteminfo(&(l->iteminfos[l->nstrs]));
  }
...
static void do_draw_listing(GrBrush *brush, const WRectangle *geom, WListing *l,
                            GrAttr selattr, int mode) {
  int wrapw = grbrush_get_text_width(brush, "\\", 1);
  int ciw = grbrush_get_text_width(brush, CONT_INDENT, CONT_INDENT_LEN);
  int r, c, i, x, y;
  GrFontExtents fnte;

  if (l->nitemcol == 0 || l->visrow == 0) return;
  ...
  while (1) {
    ...
    while (r < l->visrow) {
      ...
      if (mode >= 0 || l->selected_str == i ||
          LISTING_DRAW_GET_SELECTED(mode) == i) {
        if (i == l->selected_str) grbrush_set_attr(brush, selattr);

        draw_multirow(brush, geom->x + x, y, l->itemh, l->strs[i],
                      (l->iteminfos != NULL ? &(l->iteminfos[i]) : NULL),
                      geom->w - x, ciw, wrapw);

        if (i == l->selected_str) grbrush_unset_attr(brush, selattr);
      }
...
void draw_listing(GrBrush *brush, const WRectangle *geom, WListing *l, int mode,
                  GrAttr selattr) {
  WRectangle geom2;
  GrBorderWidths bdw;

  grbrush_begin(brush, geom,
                GRBRUSH_AMEND | GRBRUSH_KEEP_ATTR | GRBRUSH_NEED_CLIP);

  if (mode == LISTING_DRAW_COMPLETE) grbrush_clear_area(brush, geom);
...
static bool do_scrolldown_listing(WListing *l, int n) {
  ...
  while (--bc > 0) one_row_down(l, &bi, &br);

  while (n > 0) {
    if (!one_row_down(l, &bi, &br)) break;
    one_row_down(l, &i, &r);
    ret = TRUE;
    n--;
  }
...
bool scrollup_listing(WListing *l) { return do_scrollup_listing(l, l->visrow); }
...
static int listing_first_row_of_item(WListing *l, int i) {
  ...
  for (j = 0; j < fci; j++) r += ITEMROWS(l, j);
