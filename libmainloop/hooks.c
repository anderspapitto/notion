...
WHook *mainloop_register_hook(const char *name, WHook *hk) {
  ...
  if (named_hooks == NULL) {
    named_hooks = make_rb();
    if (named_hooks == NULL) return NULL;
  }
...
WHook *mainloop_get_hook(const char *name) {
  ...
  if (named_hooks != NULL) {
    ...
    if (found) return (WHook *)rb_val(node);
  }
...
WHook *create_hook() { CREATEOBJ_IMPL(WHook, hook, (p)); }
...
WHookItem *hook_find_extl(WHook *hk, ExtlFn efn) {
  ...
  for (hi = hk->items; hi != NULL; hi = hi->next) {
    if (extl_fn_eq(hi->efn, efn)) return hi;
  }
...
bool hook_add(WHook *hk, WHookDummy *fn) {
  ...
  if (hook_find(hk, fn)) return FALSE;
...
bool hook_add_extl(WHook *hk, ExtlFn efn) {
  ...
  if (hook_find_extl(hk, efn)) return FALSE;
...
static bool marshall_alt_v(bool (*fn)(), void *UNUSED(param)) { return fn(); }
...
bool hook_call_alt(const WHook *hk, void *p, WHookMarshall *m,
