...
static void do_timer_set() {
  ...
  if (libmainloop_get_timeout(&val.it_value)) {
    ...
    if ((setitimer(ITIMER_REAL, &val, NULL))) had_tmr = TRUE;
  } else if (!had_tmr) {
...
void timer_do_set(WTimer *timer, uint msecs, WTimerHandler *handler, Obj *obj, ExtlFn fn) {
  ...
  while (q != NULL) {
    if (TIMEVAL_LATER(q->when, timer->when)) break;
    qptr = &(q->next);
    q = q->next;
  }
...
void timer_set(WTimer *timer, uint msecs, WTimerHandler *handler, Obj *obj) {
...
void timer_deinit(WTimer *timer) { timer_reset(timer); }
...
WTimer *create_timer_extl_owned() {
  ...
  if (timer != NULL) ((Obj *)timer)->flags |= OBJ_EXTL_OWNED;
  return timer;
}
