...
pid_t mainloop_fork(void (*fn)(void *p), void *fnp, int *infd, int *outfd,
  ...
  if (infd != NULL) {
    if (mypipe(infds) != 0) return -1;
  }

  if (outfd != NULL) {
    if (mypipe(outfds) != 0) goto err1;
  }

  if (errfd != NULL) {
    if (mypipe(errfds) != 0) goto err2;
  }
  ...
  if (pid != 0) {
    ...
    if (outfd != NULL) {
      if (!unblock(outfds[0])) goto err3;
      *outfd = outfds[0];
      close(outfds[1]);
    }

    if (errfd != NULL) {
      if (!unblock(errfds[0])) goto err3;
      *errfd = errfds[0];
      close(errfds[1]);
    }
    ...
    if (infd != NULL) duppipe(0, 0, infds);
    if (outfd != NULL) duppipe(1, 1, outfds);
    if (errfd != NULL) duppipe(2, 1, errfds);
...
static void do_spawn(void *spawnp) {
  ...
  if (p->initenv) p->initenv(p->initenvp);
...
bool mainloop_register_input_fd_extlfn(int fd, ExtlFn fn) {
  ...
  if (p != NULL) {
    *(ExtlFn *)p = extl_ref_fn(fn);
    if (mainloop_register_input_fd(fd, p, process_pipe)) return TRUE;
    extl_unref_fn(*(ExtlFn *)p);
    free(p);
  }
  return FALSE;
...
pid_t mainloop_popen_bgread(const char *cmd, void (*initenv)(void *p), void *p,
                            ExtlFn handler, ExtlFn errhandler) {
  pid_t pid = -1;
  int fd = -1, errfd = -1;
  ExtlFn none = extl_fn_none();

  pid = mainloop_do_spawn(cmd, initenv, p, NULL, (handler != none ? &fd : NULL),
                          (errhandler != none ? &errfd : NULL));

  if (pid > 0) {
    if (handler != none) {
      if (!mainloop_register_input_fd_extlfn(fd, handler)) goto err;
    }
    if (errhandler != extl_fn_none()) {
      if (!mainloop_register_input_fd_extlfn(errfd, errhandler)) goto err;
    }
  }
