...
static WSplitRegion *get_node_check(WTiling *ws, WRegion *reg) {
  ...
  if (node == NULL || REGION_MANAGER(reg) != (WRegion *)ws) return NULL;
...
bool tiling_fitrep(WTiling *ws, WWindow *par, const WFitParams *fp) {
  ...
  if (ws->split_tree != NULL) {
    ...
    if (!ok) split_resize(ws->split_tree, &(fp->g), PRIMN_ANY, PRIMN_ANY);
  }

  return TRUE;
}

void tiling_managed_rqgeom(WTiling *ws, WRegion *mgd, const WRQGeomParams *rq,
                           WRectangle *geomret) {
...
bool tiling_managed_maximize(WTiling *ws, WRegion *mgd, int dir, int action) {
  ...
  if (node != NULL && ws->split_tree != NULL) {
    ...
    if (action == RESTORE && ret) split_regularise_stdisp(ws->stdispnode);
    return ret;
  } else
    return FALSE;
}
...
void tiling_map(WTiling *ws) {
  ...
  if (ws->split_tree != NULL) split_map(ws->split_tree);
}
...
void tiling_unmap(WTiling *ws) {
  ...
  if (ws->split_tree != NULL) split_unmap(ws->split_tree);
}
...
bool tiling_managed_prepare_focus(WTiling *ws, WRegion *reg, int flags,
                                  WPrepareFocusResult *res) {
  WSplitRegion *node;

  if (!region_prepare_focus((WRegion *)ws, flags, res)) return FALSE;
...
void tiling_stacking(WTiling *ws, Window *bottomret, Window *topret) {
  ...
  if (ws->split_tree != None) split_stacking(ws->split_tree, &sbottom, &stop);

  *bottomret = ws->dummywin;
  *topret = (stop != None ? stop : ws->dummywin);
}

Window tiling_xwindow(const WTiling *ws) { return ws->dummywin; }

/*}}}*/

/*{{{ Status display support code */

static bool regnodefilter(WSplit *split) { return OBJ_IS(split, WSplitRegion); }
...
void tiling_unmanage_stdisp(WTiling *ws, bool permanent, bool nofocus) {
  ...
  if (od != NULL) {
    if (!nofocus && REGION_IS_ACTIVE(od) &&
      ...
      setfocus = TRUE;
      tofocus = (WSplitRegion *)split_nextto(
          (WSplit *)(ws->stdispnode), PRIMN_ANY, PRIMN_ANY, regnodefilter);
    }
...
static void tiling_create_stdispnode(WTiling *ws, WRegion *stdisp, int corner,
                                     int orientation, bool fullsize) {
  ...
  split = create_splitsplit(
      wg, (orientation == REGION_ORIENTATION_HORIZONTAL ? SPLIT_VERTICAL
                                                        : SPLIT_HORIZONTAL));
...
void tiling_manage_stdisp(WTiling *ws, WRegion *stdisp,
  ...
  if (ws->stdispnode == NULL) {
    tiling_create_stdispnode(ws, stdisp, di->pos, orientation, di->fullsize);
    if (ws->stdispnode == NULL) return;
  } else {
  ...
  if (mcf && act) region_set_focus(stdisp);
}
...
bool tiling_managed_add_default(WTiling *ws, WRegion *reg) {
  ...
  if (TILING_STDISP_OF(ws) != reg) {
    if (!ptrlist_insert_last(&(ws->managed_list), reg)) return FALSE;
  }
  ...
  if (REGION_IS_MAPPED(ws)) region_map(reg);

  if (region_may_control_focus((WRegion *)ws)) {
    WRegion *curr = tiling_current(ws);
    if (curr == NULL || !REGION_IS_ACTIVE(curr)) region_warp(reg);
  }
...
bool tiling_do_attach_initial(WTiling *ws, WRegion *reg) {
  ...
  if (ws->split_tree == NULL) return FALSE;
...
bool tiling_init(WTiling *ws, WWindow *parent, const WFitParams *fp,
  ...
  ws->dummywin =
      XCreateWindow(ioncore_g.dpy, parent->win, fp->g.x, fp->g.y, 1, 1, 0,
                    CopyFromParent, InputOnly, CopyFromParent, 0, NULL);
  if (ws->dummywin == None) return FALSE;
  ...
  ws->reg.flags |= (REGION_GRAB_ON_PARENT | REGION_PLEASE_WARP);
...
void tiling_deinit(WTiling *ws) {
  ...
  FOR_ALL_MANAGED_BY_TILING(reg, ws, tmp) { destroy_obj((Obj *)reg); }

  FOR_ALL_MANAGED_BY_TILING(reg, ws, tmp) { assert(FALSE); }

  if (ws->split_tree != NULL) destroy_obj((Obj *)(ws->split_tree));
...
WRegion *tiling_managed_disposeroot(WTiling *ws, WRegion *reg) {
  ...
  if (ws->batchop) return reg;

  FOR_ALL_MANAGED_BY_TILING(mgd, ws, tmp) {
    if (mgd != TILING_STDISP_OF(ws) && mgd != reg) return reg;
  }
...
bool tiling_rescue_clientwins(WTiling *ws, WRescueInfo *info) {
  ...
  return region_rescue_some_clientwins((WRegion *)ws, info,
                                       (WRegionIterator *)ptrlist_iter, &tmp);
}
...
void tiling_managed_remove(WTiling *ws, WRegion *reg) {
  ...
  if (node == (WSplitRegion *)(ws->stdispnode)) ws->stdispnode = NULL;
  ...
  if (!norestore && other != NULL && act && mcf) region_warp(other);
}
...
static bool find_ph(WSplit *split) {
  ...
  if (sr == NULL || sr->reg == NULL) return FALSE;
...
WPHolder *tiling_get_rescue_pholder_for(WTiling *ws, WRegion *mgd) {
  ...
  if (node == NULL) {
    if (ws->split_tree != NULL) {
      split_current_todir(ws->split_tree, PRIMN_ANY, PRIMN_ANY, find_ph);
    }
  } else {
    while (node != NULL) {
      split_nextto(node, PRIMN_ANY, PRIMN_ANY, find_ph);
      if (find_ph_result != NULL) break;
...
WRegion *tiling_do_navi_next(WTiling *ws, WRegion *reg, WRegionNavi nh,
                             bool nowrap, bool any) {
  ...
  if (reg == NULL) reg = tiling_current(ws);

  if (reg != NULL) {
    WSplitRegion *node = get_node_check(ws, reg);
    if (node != NULL) {
      nxt = node_reg(split_nextto((WSplit *)node, hprimn, vprimn, filter));
    }
  }

  if (nxt == NULL && !nowrap) {
    nxt = node_reg(split_current_todir(
        ws->split_tree, primn_none2any(primn_invert(hprimn)),
...
WRegion *tiling_do_navi_first(WTiling *ws, WRegionNavi nh, bool any) {
  ...
  return node_reg(split_current_todir(ws->split_tree, hprimn, vprimn, filter));
}
...
WRegion *tiling_navi_first(WTiling *ws, WRegionNavi nh, WRegionNaviData *data) {
...
static bool get_split_dir_primn(const char *str, int *dir, int *primn) {
  ...
  if (!ioncore_string_to_navi(str, &nh)) return FALSE;
...
static WFrame *tiling_do_split(WTiling *ws, WSplit *node, const char *dirstr,
                               int minw, int minh) {
  int dir, primn, mins;
  bool floating = FALSE;
  WFrame *newframe;
  ...
  if (!get_split_dir_primn_float(dirstr, &dir, &primn, &floating)) return NULL;

  mins = (dir == SPLIT_VERTICAL ? minh : minw);

  if (!floating) {
    nnode = splittree_split(node, dir, primn, mins, ws->create_frame_fn,
                            REGION_PARENT(ws));
  } else {
...
void tiling_unsplit_at(WTiling *ws, WRegion *reg) {
  ...
  if (reg == NULL || REGION_MANAGER(reg) != (WRegion *)ws) return;
...
WRegion *tiling_current(WTiling *ws) {
  ...
  if (ws->split_tree != NULL) {
    node = (WSplitRegion *)split_current_todir(ws->split_tree, PRIMN_ANY,
                                               PRIMN_ANY, NULL);
  }
...
WSplitSplit *tiling_set_floating_extl(WTiling *ws, WSplitSplit *split,
                                      const char *how) {
  if (!check_node(ws, (WSplit *)split)) return NULL;
...
bool tiling_set_floating_at_extl(WTiling *ws, WRegion *reg, const char *how, const char *dirstr) {
  ...
  if (dirstr != NULL) {
    ...
    if (!ioncore_string_to_navi(dirstr, &nh)) return FALSE;
  ...
  while (TRUE) {
    ...
    if (!OBJ_IS(split->tl, WSplitST) && !OBJ_IS(split->br, WSplitST)) {
      ...
      if (tmp == PRIMN_ANY || (node == split->tl && tmp == PRIMN_BR) ||
...
WSplit *load_splitregion(WTiling *ws, const WRectangle *geom, ExtlTab tab) {
  ...
  region_attach_helper((WRegion *)ws, par, &fp, (WRegionDoAttachFn *)do_attach,
                       &node, &data);
...
WRegion *tiling_load(WWindow *par, const WFitParams *fp, ExtlTab tab) {
  ...
  if (extl_table_gets_t(tab, "split_tree", &treetab)) ci = FALSE;

  ws = create_tiling(par, fp, NULL, ci);

  if (ws == NULL) {
    if (!ci) extl_unref_table(treetab);
    return NULL;
  }
...
static DynFunTab tiling_dynfuntab[] = {
    {region_map, tiling_map},

    {region_unmap, tiling_unmap},

    {region_do_set_focus, tiling_do_set_focus},

    {(DynFun *)region_fitrep, (DynFun *)tiling_fitrep},

    {region_managed_rqgeom, tiling_managed_rqgeom},

    {(DynFun *)region_managed_maximize, (DynFun *)tiling_managed_maximize},

    {region_managed_remove, tiling_managed_remove},

    {(DynFun *)region_managed_prepare_focus,
     (DynFun *)tiling_managed_prepare_focus},

    {(DynFun *)region_prepare_manage, (DynFun *)tiling_prepare_manage},

    {(DynFun *)region_rescue_clientwins, (DynFun *)tiling_rescue_clientwins},

    {(DynFun *)region_get_rescue_pholder_for,
     (DynFun *)tiling_get_rescue_pholder_for},

    {(DynFun *)region_get_configuration, (DynFun *)tiling_get_configuration},

    {(DynFun *)region_managed_disposeroot,
     (DynFun *)tiling_managed_disposeroot},

    {(DynFun *)region_current, (DynFun *)tiling_current},

    {(DynFun *)tiling_managed_add, (DynFun *)tiling_managed_add_default},

    {region_manage_stdisp, tiling_manage_stdisp},

    {region_unmanage_stdisp, tiling_unmanage_stdisp},

    {(DynFun *)tiling_load_node, (DynFun *)tiling_load_node_default},

    {region_restack, tiling_restack},

    {region_stacking, tiling_stacking},

    {(DynFun *)region_navi_first, (DynFun *)tiling_navi_first},

    {(DynFun *)region_navi_next, (DynFun *)tiling_navi_next},

    {(DynFun *)region_xwindow, (DynFun *)tiling_xwindow},
