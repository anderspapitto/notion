...
void mod_tiling_set(ExtlTab tab) {
  ...
  if (extl_table_gets_i(tab, "raise_delay", &d)) {
    if (d >= 0) mod_tiling_raise_delay = d;
  }
}
...
bool mod_tiling_init() {
  if (!init_hooks()) goto err;

  mod_tiling_tiling_bindmap = ioncore_alloc_bindmap("WTiling", NULL);

  if (mod_tiling_tiling_bindmap == NULL) goto err;

  if (!mod_tiling_register_exports()) goto err;

  if (!register_regions()) goto err;
