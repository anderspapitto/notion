...
static void splitfloat_set_borderlines(WSplitFloat *split) {
  ...
  split->tlpwin->bline =
      (dir == SPLIT_HORIZONTAL ? GR_BORDERLINE_RIGHT : GR_BORDERLINE_BOTTOM);

  split->brpwin->bline =
      (dir == SPLIT_HORIZONTAL ? GR_BORDERLINE_LEFT : GR_BORDERLINE_TOP);
}

bool splitfloat_init(WSplitFloat *split, const WRectangle *geom, WTiling *ws,
                     int dir) {
  WFitParams fp;
  ...
  if (split->tlpwin == NULL) return FALSE;
...
static void stack_stacking_reg(WRegion *reg, Window *bottomret,
                               Window *topret) {
  Window b = None, t = None;

  if (reg != NULL) {
    region_stacking(reg, &b, &t);
    if (*bottomret == None) *bottomret = b;
    if (t != None) *topret = t;
  }
}

static void stack_stacking_split(WSplit *split, Window *bottomret,
                                 Window *topret) {
  Window b = None, t = None;
...
static void splitfloat_stacking(WSplitFloat *split, Window *bottomret,
                                Window *topret) {
  *bottomret = None;
  *topret = None;
...
#define TL_BORDER(SF)                                            \
  ((SF)->ssplit.dir == SPLIT_VERTICAL ? (SF)->tlpwin->bdw.bottom \
                                      : (SF)->tlpwin->bdw.right)

#define BR_BORDER(SF)                                         \
  ((SF)->ssplit.dir == SPLIT_VERTICAL ? (SF)->brpwin->bdw.top \
                                      : (SF)->brpwin->bdw.left)
...
static int splitfloat_get_handle(WSplitFloat *split, int dir, WSplit *other) {
  assert(other == split->ssplit.tl || other == split->ssplit.br);

  if (dir != split->ssplit.dir) return 0;
  ...
  if (dir == SPLIT_VERTICAL) {
    ...
    if (adjust) {
      adjust_sizes(&ntlg.h, &nbrg.h, ng->h,
                   ...
                   splitfloat_get_max(split, dir, split->ssplit.br), vprimn);
    }
    ...
    if (adjust) {
      adjust_sizes(&ntlg.w, &nbrg.w, ng->w,
                   ...
                   splitfloat_get_min(split, dir, split->ssplit.br),
                   splitfloat_get_max(split, dir, split->ssplit.tl),
                   splitfloat_get_max(split, dir, split->ssplit.br), hprimn);
    }
...
static void calc_amount(int *amount, int *oamount, int rs, WSplitSplit *p,
                        int omax, const WRectangle *ng, const WRectangle *og) {
  *oamount = 0;
...
WSplitRegion *splittree_split_floating(WSplit *node, int dir, int primn,
  ...
  if (primn == PRIMN_TL) {
    ...
    gnc = gn;
    splitfloat_tl_pwin_to_cnt(sf, &gnc);
    goc = go;
    splitfloat_br_pwin_to_cnt(sf, &goc);
  } else {
    calc_tlg_brg(&(node->geom), so, sn, dir, &go, &gn);
    splitfloat_update_handles(sf, &go, &gn);
    goc = go;
    splitfloat_tl_pwin_to_cnt(sf, &goc);
    gnc = gn;
    splitfloat_br_pwin_to_cnt(sf, &gnc);
  }
  ...
  split_do_resize(node, &goc, (dir == SPLIT_HORIZONTAL ? primn : PRIMN_ANY),
