...
WSplitRegion *splittree_node_of(WRegion *reg) {
  ...
  if (split_of_map != NULL) {
    ...
    if (found) return (WSplitRegion *)(node->v.val);
  }
...
bool splittree_set_node_of(WRegion *reg, WSplitRegion *split) {
  ...
  if (split_of_map == NULL) {
    ...
    if (split_of_map == NULL) return FALSE;
  }

  node = rb_find_pkey_n(split_of_map, reg, &found);
  if (found) rb_delete_node(node);
...
bool splitregion_init(WSplitRegion *split, const WRectangle *geom,
  ...
  split->reg = reg;
  if (reg != NULL) set_node_of_reg(reg, split);
  return TRUE;
}
...
void splittree_begin_resize() { saw_stdisp = NULL; }
...
void splittree_scan_stdisp_rootward(WSplit *node) {
  if (node->parent != NULL) splittree_scan_stdisp_rootward_(node->parent);
}
...
static WSplitST *splittree_scan_stdisp(WSplit *node_, bool set_saw) {
  ...
  if (r == NULL) r = OBJ_CAST(node->br, WSplitST);

  if (r != NULL) {
    if (set_saw) saw_stdisp = r;
    return r;
  }

  r = splittree_scan_stdisp(node->tl, set_saw);
  if (r == NULL) r = splittree_scan_stdisp(node->br, set_saw);
...
static bool stdisp_immediate_child(WSplitSplit *node) {
  return (node != NULL &&
          (OBJ_IS(node->tl, WSplitST) || OBJ_IS(node->br, WSplitST)));
}
...
static WSplit *dodge_stdisp(WSplit *node, bool keep_within) {
  ...
  if (stdisp == NULL) return node;

  stdispp = OBJ_CAST(((WSplit *)stdisp)->parent, WSplitSplit);

  if (stdispp == NULL) return node;
  ...
  if ((WSplit *)stdispp == node) {
     ...
     */
    return (keep_within ? node
                        : (stdispp->tl == (WSplit *)stdisp ? stdispp->br
                                                           : stdispp->tl));
  }
...
static void splitst_do_resize(WSplitST *node, const WRectangle *ng,
  ...
  if (node->regnode.reg == NULL) {
    ...
    splitregion_do_resize(&(node->regnode), ng, hprimn, vprimn, transpose);
  }
}
...
static void adjust_sizes(int *tls_, int *brs_, int nsize, int sz, int tlmin,
                         int brmin, int tlmax, int brmax, int primn) {
  int tls = *tls_;
  int brs = *brs_;
...
static void get_minmaxunused(WSplit *node, int dir, int *min, int *max,
                             int *unused) {
...
void splitsplit_do_resize(WSplitSplit *node, const WRectangle *ng,
...

  {
    ...
    int dir =
        ((transpose && !stdisp_immediate_child(node)) ? other_dir(node->dir)
                                                      : node->dir);
    ...
    if (sz > 2) {
      if (primn == PRIMN_ANY && (tlunused >= 0 || brunused >= 0)) {
        if (nsize <= tlused + brused) {
          ...
          adjust_sizes(&tls, &brs, nsize, sz, tlmin, brmin, tlused, brused,
                       primn);
        } else {
          /* Just expand or shrink unused space */
          adjust_sizes(&tls, &brs, nsize, sz, tlused, brused,
                       (tlunused < 0 ? tlused : tlmax),
                       (brunused < 0 ? brused : brmax), primn);
        }

      } else {
        adjust_sizes(&tls, &brs, nsize, sz, tlmin, brmin, tlmax, brmax, primn);
      }
    }
...
void split_do_resize(WSplit *node, const WRectangle *ng, WPrimn hprimn,
                     WPrimn vprimn, bool transpose) {
  CALL_DYN(split_do_resize, node, (node, ng, hprimn, vprimn, transpose));
}

void split_resize(WSplit *node, const WRectangle *ng, WPrimn hprimn,
                  WPrimn vprimn) {
...
bool splits_are_related(WSplit *p, WSplit *node) {
  ...
  return node->parent != NULL ? splits_are_related(p, (WSplit *)node->parent)
                              : FALSE;
}
...
WSplit *maxparentdir_rel(WSplit *p, WSplit *node, int dir) {
  ...
  if (OBJ_IS(p, WSplitSplit)) {
    ...
    if (OBJ_IS(sp->tl, WSplitST)) return maxparentdir_rel(sp->br, node, dir);
    if (OBJ_IS(sp->br, WSplitST)) return maxparentdir_rel(sp->tl, node, dir);

    if (sp->dir != dir) {
      return splits_are_related(sp->tl, node)
...
WRectangle stdisp_recommended_geom(WSplitST *st, WRectangle wsg) {
  ...
  if (!is_lt(ori, st->corner)) *xy(&stg, ori) = *wh(&wsg, ori) - *wh(&stg, ori);
...
void grow_by_stdisp_wh(WRectangle *geom, WSplitST *st) {
  ...
  if (is_lt(ori, st->corner)) *xy(geom, ori) = 0;
  *wh(geom, ori) += *wh(&stg, ori);
}

bool frame_neighbors_stdisp(WFrame *frame, WSplitST *st) {
  return geom_overlaps_stgeom_xy(REGION_GEOM(frame), st, REGION_GEOM(st)) &&
...
bool geom_clashes_stdisp(WRectangle geom, WSplitST *st) {
  WRectangle stg = REGION_GEOM(st->regnode.reg);
  int ori = flip_orientation(st->orientation);
  return is_lt(ori, st->corner) ? *xy(&geom, ori) == 0
                                : *xy(&geom, ori) + *wh(&geom, ori) ==
                                      *xy(&stg, ori) + *wh(&stg, ori);
}

bool is_same_dir(int dir, int ori) {
  return (dir == SPLIT_HORIZONTAL && ori == REGION_ORIENTATION_HORIZONTAL) ||
         (dir == SPLIT_VERTICAL && ori == REGION_ORIENTATION_VERTICAL);
}

bool is_maxed(WFrame *frame, int dir) {
  return dir == SPLIT_HORIZONTAL
...
bool update_geom_from_stdisp(WFrame *frame, WRectangle *ng, int dir) {
  ...
  if (!OBJ_IS(ws, WTiling) || ((WTiling *)ws)->stdispnode == NULL) return FALSE;

  st = ((WTiling *)ws)->stdispnode;

  if (st->fullsize || !frame_neighbors_stdisp(frame, st)) return FALSE;
  ...
  if (!is_same_dir(dir, st->orientation) &&
    ...
    if (is_lt(ori, st->corner)) *xy(ng, ori) += *wh(&stg, ori);
...
bool splitregion_do_restore(WSplitRegion *node, int dir) {
  ...
  if (!OBJ_IS(node->reg, WFrame)) return FALSE;
  ...
  other_max = dir == SPLIT_HORIZONTAL ? frame->flags & FRAME_MAXED_VERT
                                      : frame->flags & FRAME_MAXED_HORIZ;
...
bool splitst_do_restore(WSplit *UNUSED(node), int UNUSED(dir)) { return FALSE; }
...
void splitregion_do_maxhelper(WSplitRegion *node, int dir, int action) {
  WFrame *frame;
  if (!OBJ_IS(node->reg, WFrame)) return;
  frame = (WFrame *)node->reg;
  ...
  if (action == SET_KEEP) frame->flags |= FRAME_KEEP_FLAGS;
  if (action == RM_KEEP) frame->flags &= ~FRAME_KEEP_FLAGS;
}

void splitst_do_maxhelper(WSplit *UNUSED(node), int UNUSED(dir),
                          int UNUSED(action)) {
  return;
}
...
bool savedgeom_clashes_stdisp(WFrame *frame, int dir) {
  ...
  if (!OBJ_IS(ws, WTiling) || ((WTiling *)ws)->stdispnode == NULL) return TRUE;

  st = ((WTiling *)ws)->stdispnode;
  ori = flip_orientation(st->orientation);

  return !is_same_dir(dir, st->orientation) &&
...
bool splitregion_do_verify(WSplitRegion *node, int dir) {
  ...
  if (savedgeom_clashes_stdisp(frame, dir)) return FALSE;

  return ret;
}

bool splitst_do_verify(WSplit *UNUSED(node), int UNUSED(dir)) { return TRUE; }
...
bool split_maximize(WSplit *node, int dir, int action) {
  ...
  if (action == RESTORE) return split_do_restore(p, dir);
  if (action == VERIFY) return split_do_verify(p, dir);
...
static void splitsplit_do_rqsize(WSplitSplit *p, WSplit *node,
  ...
  if (((WSplit *)p)->parent == NULL /*||
    ...
    splitinner_do_rqsize(((WSplit *)p)->parent, (WSplit *)p, ha, va, &pg,
                         tryonly);
  }
  ...
  if (p->dir == SPLIT_VERTICAL) {
    ...
    adjust_sizes(&(ng.h), &(og.h), pg.h, ng.h + og.h, node->min_h, other->min_h,
                 node->max_h, other->max_h,
    ...
    adjust_sizes(&(ng.w), &(og.w), pg.w, ng.w + og.w, node->min_w, other->min_w,
                 node->max_w, other->max_w,
                 PRIMN_TL /* node is passed as tl param */);
    if (thisnode == PRIMN_TL)
      og.x = pg.x + pg.w - og.w;
    else
...
static void initra(RootwardAmount *ra, int p, int s, int op, int os, bool any) {
  ra->any = any;
  ra->tl = op - p;
  ra->br = (p + s) - (op + os);
  if (any) {
...
void split_do_rqgeom_(WSplit *node, const WRectangle *ng, bool hany, bool vany,
                      WRectangle *rg, bool tryonly) {
...
WSplit *split_find_root(WSplit *split) {
  if (split->parent == NULL) return split;
...
void splittree_rqgeom(WSplit *sub, int flags, const WRectangle *geom_,
  ...
  if (OBJ_IS(sub, WSplitST)) {
    ...
    if (sub_as_stdisp->orientation == REGION_ORIENTATION_HORIZONTAL) {
      if (geom_->h == geom.h) return;
      geom.h = geom_->h;
    } else {
      if (geom_->w == geom.w) return;
      geom.w = geom_->w;
    }
    split_update_bounds(root, TRUE);
  }
  ...
  bnd(&(geom.x), &(geom.w), sub->geom.x, sub->geom.w, sub->min_w, sub->max_w);
  bnd(&(geom.y), &(geom.h), sub->geom.y, sub->geom.h, sub->min_h, sub->max_h);
...
ExtlTab split_rqgeom(WSplit *node, ExtlTab g) {
  ...
  if (extl_table_gets_i(g, "x", &(geom.x))) flags &= ~REGION_RQGEOM_WEAK_X;
  if (extl_table_gets_i(g, "y", &(geom.y))) flags &= ~REGION_RQGEOM_WEAK_Y;
  if (extl_table_gets_i(g, "w", &(geom.w))) flags &= ~REGION_RQGEOM_WEAK_W;
  if (extl_table_gets_i(g, "h", &(geom.h))) flags &= ~REGION_RQGEOM_WEAK_H;
...
WSplitRegion *splittree_split(WSplit *node, int dir, WPrimn primn, int minsize,
  ...
  if (primn != PRIMN_TL && primn != PRIMN_BR) primn = PRIMN_BR;
  if (dir != SPLIT_HORIZONTAL && dir != SPLIT_VERTICAL) dir = SPLIT_VERTICAL;
  ...
  if (dir == SPLIT_VERTICAL) {
    if (primn == PRIMN_BR) fp.g.y += so;
    fp.g.h = sn;
  } else {
    if (primn == PRIMN_BR) fp.g.x += so;
    fp.g.w = sn;
  }
  ...
  if (dir == SPLIT_VERTICAL) {
    ng.h = so;
    if (primn == PRIMN_TL) ng.y += sn;
  } else {
    ng.w = so;
    if (primn == PRIMN_TL) ng.x += sn;
  }

  split_do_resize(node, &ng, (dir == SPLIT_HORIZONTAL ? primn : PRIMN_ANY),
                  (dir == SPLIT_VERTICAL ? primn : PRIMN_ANY), FALSE);
...
static bool defaultfilter(WSplit *node) {
  return (OBJ_IS(node, WSplitRegion) && ((WSplitRegion *)node)->reg != NULL);
}

static WSplit *split_current_todir_default(WSplit *node, WPrimn UNUSED(hprimn),
                                           WPrimn UNUSED(vprimn),
                                           WSplitFilter *filter) {
...
static WSplit *splitsplit_current_todir(WSplitSplit *node, WPrimn hprimn,
                                        WPrimn vprimn, WSplitFilter *filter) {
  ...
  if (ret == NULL) ret = split_current_todir(second, hprimn, vprimn, filter);
  if (ret == NULL && filter != NULL) {
    if (filter((WSplit *)node)) ret = (WSplit *)node;
  }
 ...
 */
WSplit *splitsplit_nextto(WSplitSplit *node, WSplit *child, WPrimn hprimn,
                          WPrimn vprimn, WSplitFilter *filter) {
  WPrimn primn = (node->dir == SPLIT_HORIZONTAL ? hprimn : vprimn);
  ...
  if (split != NULL) {
    nnode = split_current_todir(split, primn_none2any(primn_invert(hprimn)),
                                primn_none2any(primn_invert(vprimn)), filter);
  }
...
WSplit *splitinner_nextto(WSplitInner *node, WSplit *child, WPrimn hprimn,
                          WPrimn vprimn, WSplitFilter *filter) {
  WSplit *ret = NULL;
...
WSplit *split_nextto(WSplit *node, WPrimn hprimn, WPrimn vprimn,
  ...
  while (node->parent != NULL) {
    WSplit *ret = splitinner_nextto(node->parent, node, hprimn, vprimn, filter);
...
void splitinner_mark_current_default(WSplitInner *split,
                                     WSplit *UNUSED(child)) {
...
static void splitregion_stacking(WSplitRegion *split, Window *bottomret,
                                 Window *topret) {
  *bottomret = None;
  *topret = None;
  if (split->reg != NULL) region_stacking(split->reg, bottomret, topret);
}

void splitsplit_stacking(WSplitSplit *split, Window *bottomret,
                         Window *topret) {
...
static void splitregion_restack(WSplitRegion *split, Window other, int mode) {
  if (split->reg != NULL) region_restack(split->reg, other, mode);
}
...
static void splitregion_map(WSplitRegion *split) {
  if (split->reg != NULL) region_map(split->reg);
}
...
void split_map(WSplit *split) { CALL_DYN(split_map, split, (split)); }

static void splitregion_unmap(WSplitRegion *split) {
  if (split->reg != NULL) region_unmap(split->reg);
}
...
void split_unmap(WSplit *split) { CALL_DYN(split_unmap, split, (split)); }
...
void splitsplit_flip_default(WSplitSplit *split) {
  ...
  split->tl = split->br;
  split->br = tmp;
  split->current = (split->current == SPLIT_CURRENT_TL ? SPLIT_CURRENT_BR
                                                       : SPLIT_CURRENT_TL);
...
typedef enum { FLIP_VERTICAL, FLIP_HORIZONTAL, FLIP_NONE, FLIP_ANY } FlipDir;
...
static void do_flip(WSplit *split) {
  ...
  if (ss != NULL) {
    if ((flipdir == FLIP_ANY ||
        ...
        !OBJ_IS(ss->tl, WSplitST) && !OBJ_IS(ss->br, WSplitST)) {
      splitsplit_flip_(ss);
    }
  }

  if (OBJ_IS(ss, WSplitInner)) splitinner_forall((WSplitInner *)ss, do_flip);
}
...
static bool split_fliptrans_to(WSplit *node, const WRectangle *geom, bool trans,
                               FlipDir flip) {
  WRectangle rg;
  WSplit *node2;
  ...
  if (node == NULL || node2 != node) return FALSE;
  ...
  if (flip != FLIP_NONE) splittree_flip_dir(node, flip);
...
EXTL_SAFE
EXTL_EXPORT_MEMBER
WSplitInner *split_parent(WSplit *split) { return split->parent; }
...
EXTL_SAFE
EXTL_EXPORT_MEMBER
WSplit *splitsplit_tl(WSplitSplit *split) { return split->tl; }
...
EXTL_SAFE
EXTL_EXPORT_MEMBER
WSplit *splitsplit_br(WSplitSplit *split) { return split->br; }
...
static bool splitsplit_get_config(WSplitSplit *node, ExtlTab *ret) {
  ...
  extl_table_sets_s(tab, "dir",
                    (node->dir == SPLIT_VERTICAL ? "vertical" : "horizontal"));
