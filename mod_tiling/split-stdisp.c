...
#define STDISP_GROWS_L_TO_R(STDISP)                                        \
  (STDISP_IS_HORIZONTAL(STDISP) && ((STDISP)->corner == MPLEX_STDISP_TL || \
                                    (STDISP)->corner == MPLEX_STDISP_BL))

#define STDISP_GROWS_R_TO_L(STDISP)                                        \
  (STDISP_IS_HORIZONTAL(STDISP) && ((STDISP)->corner == MPLEX_STDISP_TR || \
                                    (STDISP)->corner == MPLEX_STDISP_BR))

#define STDISP_GROWS_T_TO_B(STDISP)                                      \
  (STDISP_IS_VERTICAL(STDISP) && ((STDISP)->corner == MPLEX_STDISP_TL || \
                                  (STDISP)->corner == MPLEX_STDISP_TR))

#define STDISP_GROWS_B_TO_T(STDISP)                                      \
  (STDISP_IS_VERTICAL(STDISP) && ((STDISP)->corner == MPLEX_STDISP_BL || \
                                  (STDISP)->corner == MPLEX_STDISP_BR))
...
int stdisp_recommended_w(WSplitST *stdisp) {
  if (stdisp->regnode.reg == NULL) return CF_STDISP_MIN_SZ;
...
static void rot_para_right(WSplitSplit *a, WSplitSplit *p, WSplit *y) {
  rotate_right(a, p, y);
...
static void rot_para_left(WSplitSplit *a, WSplitSplit *p, WSplit *y) {
  rotate_left(a, p, y);
  if (a->dir == SPLIT_VERTICAL) {
    GEOM(p).y = GEOM(a).y;
    GEOM(p).h = GEOM(a).h;
...
static bool do_try_sink_stdisp_orth(WSplitSplit *p, WSplitST *stdisp,
  ...
  if (STDISP_GROWS_T_TO_B(stdisp) || STDISP_GROWS_L_TO_R(stdisp)) {
    if (STDISP_GROWS_L_TO_R(stdisp)) {
      ...
      if (other->tl->geom.w >= stdisp_recommended_w(stdisp)) doit = TRUE;
    } else { /* STDISP_GROWS_T_TO_B */
      assert(other->dir == SPLIT_VERTICAL);
      if (other->tl->geom.h >= stdisp_recommended_h(stdisp)) doit = TRUE;
    }
    ...
    if (STDISP_GROWS_R_TO_L(stdisp)) {
      assert(other->dir == SPLIT_HORIZONTAL);
      if (other->br->geom.w >= stdisp_recommended_w(stdisp)) doit = TRUE;
    } else { /* STDISP_GROWS_B_TO_T */
      assert(other->dir == SPLIT_VERTICAL);
      if (other->br->geom.h >= stdisp_recommended_h(stdisp)) doit = TRUE;
    }
...
static bool do_try_sink_stdisp_para(WSplitSplit *p, WSplitST *stdisp,
  ...
  if (!force) {
    if (STDISP_IS_HORIZONTAL(stdisp)) {
      if (stdisp_recommended_w(stdisp) >= GEOM(p).w) return FALSE;
    } else {
      if (stdisp_recommended_h(stdisp) >= GEOM(p).h) return FALSE;
    }
  }
...
bool split_try_sink_stdisp(WSplitSplit *node, bool iterate, bool force) {
  ...
  while (more) {
    ...
    if (!stdisp_dir_ok(node, st)) break;

    if (other->dir == other_dir(node->dir)) {
      if (!do_try_sink_stdisp_orth(node, st, other, force)) break;
    } else /*if(br->dir==node->dir)*/ {
      if (!do_try_sink_stdisp_para(node, st, other, force)) break;
    }
    didsomething = TRUE;
    more = iterate;
  }
...
static bool do_try_unsink_stdisp_orth(WSplitSplit *a, WSplitSplit *p,
  ...
  if (STDISP_GROWS_T_TO_B(stdisp) || STDISP_GROWS_L_TO_R(stdisp)) {
    if (STDISP_GROWS_L_TO_R(stdisp)) {
      ...
      if (GEOM(stdisp).w < stdisp_recommended_w(stdisp)) doit = TRUE;
    } else { /* STDISP_GROWS_T_TO_B */
      assert(a->dir == SPLIT_VERTICAL);
      if (GEOM(stdisp).h < stdisp_recommended_h(stdisp)) doit = TRUE;
    }
...
bool split_try_unsink_stdisp(WSplitSplit *node, bool iterate, bool force) {
  ...
  while (more) {
    ...
    if (p->dir == other_dir(node->dir)) {
      if (!do_try_unsink_stdisp_orth(p, node, st, force)) break;
    } else /*if(p->dir==node->dir)*/ {
      if (!do_try_unsink_stdisp_para(p, node, st, force)) break;
    }
