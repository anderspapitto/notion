...
static bool sm_do_manage(WClientWin *cwin, const WManageParams *param) {
  ...
  if (param->tfor != NULL) return FALSE;
...
int mod_sm_init() {
  if (ioncore_g.sm_client_id != NULL) mod_sm_set_ion_id(ioncore_g.sm_client_id);

  if (!mod_sm_init_session()) goto err;

  if (extl_sessiondir() == NULL) mod_sm_set_sessiondir();

  if (!mod_sm_register_exports()) goto err;
