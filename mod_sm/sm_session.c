...
void mod_sm_set_ion_id(const char *client_id) {
  if (sm_client_id) free(sm_client_id);
...
char *mod_sm_get_ion_id() { return sm_client_id; }
...
static void sm_process_messages(int UNUSED(fd), void *UNUSED(data)) {
  ...
  if (IceProcessMessages(ice_sm_conn, NULL, &ret) ==
...
static void sm_set_other_properties() {
  ...
  if (sdir == NULL || cid == NULL) return;

  restart_hint_val.value = &restart_hint;
  restart_hint_val.length = 1;

  restart_val =
      (SmPropValue *)malloc((ioncore_g.argc + 4) * sizeof(SmPropValue));
...
static void sm_save_yourself_phase2(SmcConn conn,
...
static void sm_save_complete(SmcConn UNUSED(conn),
                             SmPointer UNUSED(client_data)) {
  if (save_complete_fn) {
    save_complete_fn();
    save_complete_fn = NULL;
...
bool mod_sm_init_session() {
  ...
  if ((sm_conn =
           SmcOpenConnection(NULL, /* network ids */
                             NULL, /* context */
...
static void sm_exit() { sm_die(sm_conn, NULL); }

static void sm_restart() { ioncore_do_restart(); }
...
void mod_sm_smhook(int what) {
  ...
  switch (what) {
    ...
    case IONCORE_SM_RESTART:
      ...
      SmcRequestSaveYourself(sm_conn, SmSaveBoth, False, SmInteractStyleAny,
                             False, False);
      save_complete_fn = &sm_exit;
      break;
