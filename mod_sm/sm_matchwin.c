...
Window mod_sm_get_client_leader(Window window) {
  ...
  if (XGetWindowProperty(ioncore_g.dpy, window, atom, 0L, 1L, False,
...
char *mod_sm_get_window_cmd(Window window) {
  ...
  if (XGetCommand(ioncore_g.dpy, window, &cmd_argv, &cmd_argc) &&
      (cmd_argc > 0))
    ;
  ...
  if (cmd_argc > 0) {
    for (i = 0; i < cmd_argc; i++) len += strlen(cmd_argv[i]) + 1;
...
static void free_win_match(WWinMatch *match) {
  ...
  if (match->pholder != NULL) destroy_obj((Obj *)match->pholder);

  if (match->client_id) free(match->client_id);
  if (match->window_role) free(match->window_role);
  if (match->wclass) free(match->wclass);
  if (match->wm_name) free(match->wm_name);
  if (match->wm_cmd) free(match->wm_cmd);
  free(match);
}
...
#endif
  while (match_list) free_win_match(match_list);
}

void mod_sm_start_purge_timer() {
  if (purge_timer == NULL) purge_timer = create_timer();
  if (purge_timer != NULL) {
    timer_set(purge_timer, TIME_OUT, (WTimerHandler *)mod_sm_purge_matches,
              NULL);
  }
}
