...
void statusbar_calculate_xs(WStatusBar *sb) {
  ...
  if (sb->brush == NULL || sb->elems == NULL) return;
  ...
  if (nleft > 0) calc_elems_x(&g, sb->elems, nleft);

  if (nright > 0) calc_elems_x_ra(&g, sb->elems + sb->nelems - nright, nright);
}

static void draw_elems(GrBrush *brush, WRectangle *g, int ty, WSBElem *elems,
                       int nelems, bool needfill) {
  int prevx = g->x;
  int maxx = g->x + g->w;
  ...
  while (nelems > 0) {
    ...
    if (elems->type == WSBELEM_TEXT || elems->type == WSBELEM_METER) {
      const char *s = (elems->text != NULL ? elems->text : STATUSBAR_NX_STR);
