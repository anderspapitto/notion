...
bool statusbar_init(WStatusBar *p, WWindow *parent, const WFitParams *fp) {
  if (!window_init(&(p->wwin), parent, fp, "WStatusBar")) return FALSE;
    ...
    if (extl_table_geti_t(t, i + 1, &tt)) {
      if (extl_table_gets_i(tt, "type", &(el[i].type))) {
        if (el[i].type == WSBELEM_TEXT || el[i].type == WSBELEM_STRETCH) {
          ...
          if (tmp == NULL || strcmp(tmp, "systray") == 0) systrayidx = i;
...
static void free_sbelems(WSBElem *el, int n) {
  ...
  for (i = 0; i < n; i++) {
    if (el[i].text != NULL) free(el[i].text);
    if (el[i].tmpl != NULL) free(el[i].tmpl);
    if (el[i].meter != STRINGID_NONE) stringstore_free(el[i].meter);
    if (el[i].attr != STRINGID_NONE) stringstore_free(el[i].attr);
    if (el[i].traywins != NULL) ptrlist_clear(&el[i].traywins);
  }
...
static void calc_elem_w(WStatusBar *p, WSBElem *el, GrBrush *brush) {
  ...
  if (el->type == WSBELEM_METER) {
    ...
    str = el->text;
    el->text_w =
        (str != NULL ? grbrush_get_text_width(brush, str, strlen(str)) : 0);
    el->max_w = el->text_w;
  }
}
...
static void statusbar_calc_widths(WStatusBar *sb) {
  ...
  for (i = 0; i < sb->nelems; i++) calc_elem_w(sb, &(sb->elems[i]), sb->brush);
}
...
static void statusbar_do_update_natural_size(WStatusBar *p) {
  ...
  for (i = 0; i < p->nelems; i++) totw += p->elems[i].max_w;
...
static WSBElem *statusbar_associate_systray(WStatusBar *sb, WRegion *reg) {
  ...
  if (cwin != NULL) extl_table_gets_s(cwin->proptab, "statusbar", &name);

  for (i = 0; i < sb->nelems; i++) {
    const char *meter;

    if (sb->elems[i].type != WSBELEM_SYSTRAY) continue;
    ...
    if (strcmp(meter, "systray") == 0) fbel = &sb->elems[i];
  }
...
static void statusbar_calc_systray_w(WStatusBar *p) {
  ...
  for (i = 0; i < p->nelems; i++) {
    if (p->elems[i].type == WSBELEM_SYSTRAY) do_calc_systray_w(p, &p->elems[i]);
  }
}
...
static void statusbar_arrange_systray(WStatusBar *p) {
  ...
  for (i = 0; i < p->nelems; i++) {
    ...
    if (el->type != WSBELEM_SYSTRAY) continue;
    x = el->x;
    FOR_ALL_ON_PTRLIST(WRegion *, reg, el->traywins, tmp) {
...
static WRegion *statusbar_do_attach_final(WStatusBar *sb, WRegion *reg,
                                          void *UNUSED(unused)) {
  WFitParams fp;
  WSBElem *el;

  if (!ptrlist_insert_last(&sb->traywins, (Obj *)reg)) return NULL;
  ...
  if (REGION_IS_MAPPED(sb)) region_map(reg);
...
static WPHolder *statusbar_prepare_manage(WStatusBar *sb,
  ...
  return (WPHolder *)create_basicpholder(
      (WRegion *)sb, ((WBasicPHolderHandler *)statusbar_attach_ph));
}
...
bool statusbar_fitrep(WStatusBar *sb, WWindow *par, const WFitParams *fp) {
  ...
  if (!window_fitrep(&(sb->wwin), par, fp)) return FALSE;
...
WPHolder *statusbar_prepare_manage_transient(WStatusBar *sb,
  ...
  if (mgr == NULL) mgr = (WRegion *)region_screen_of((WRegion *)sb);

  if (mgr != NULL)
    return region_prepare_manage(mgr, cwin, param, MANAGE_PRIORITY_NONE);
  else
    return NULL;
}
...
void mod_statusbar__set_template_parser(ExtlFn fn) {
  if (parse_template_fn_set) extl_unref_fn(parse_template_fn);
...
void statusbar_set_template(WStatusBar *sb, const char *tmpl) {
  ...
  if (ok) statusbar_set_template_table(sb, t);
}
...
static void reset_stretch(WStatusBar *sb) {
  ...
  for (i = 0; i < sb->nelems; i++) sb->elems[i].stretch = 0;
}
...
static void spread_stretch(WStatusBar *sb) {
  ...
  for (i = 0; i < sb->nelems; i++) {
    ...
    if (el->type != WSBELEM_METER && el->type != WSBELEM_SYSTRAY) continue;
...
static void statusbar_rearrange(WStatusBar *sb, bool rs) {
  ...
  if (rs) statusbar_arrange_systray(sb);
}
...
void statusbar_update(WStatusBar *sb, ExtlTab t) {
  ...
  for (i = 0; i < sb->nelems; i++) {
    ...
    if (el->type != WSBELEM_METER) continue;
...
void statusbar_updategr(WStatusBar *p) {
  ...
  nbrush = gr_get_brush(p->wwin.win, region_rootwin_of((WRegion *)p),
                        "stdisp-statusbar");
  if (nbrush == NULL) return;

  if (p->brush != NULL) grbrush_release(p->brush);
...
WStatusBar *mod_statusbar_find_suitable(WClientWin *cwin,
  ...
  for (sb = statusbars; sb != NULL; sb = sb->sb_next) {
    /*if(!sb->is_auto)
        continue;*/
    if (!sb->systray_enabled) continue;
    if (!region_same_rootwin((WRegion *)sb, (WRegion *)cwin)) continue;
    break;
  }
...
bool statusbar_is_systray_extl(WStatusBar *sb) { return sb->systray_enabled; }
...
static DynFunTab statusbar_dynfuntab[] = {
    ...
    {(DynFun *)region_fitrep, (DynFun *)statusbar_fitrep},
