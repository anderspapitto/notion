...
static bool is_systray(WClientWin *cwin) {
  ...
  if (atom__kde_net_wm_system_tray_window_for == None) {
    atom__kde_net_wm_system_tray_window_for =
        XInternAtom(ioncore_g.dpy, "_KDE_NET_WM_SYSTEM_TRAY_WINDOW_FOR", False);
  }
  if (XGetWindowProperty(
          ioncore_g.dpy, cwin->win, atom__kde_net_wm_system_tray_window_for, 0,
          sizeof(Atom), False, AnyPropertyType, &actual_type, &actual_format,
...
static bool clientwin_do_manage_hook(WClientWin *cwin,
  ...
  if (!is_systray(cwin)) return FALSE;
...
bool mod_statusbar_init() {
  ...
  if (mod_statusbar_statusbar_bindmap == NULL) return FALSE;
