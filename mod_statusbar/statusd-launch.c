...
static bool process_pipe(int fd, ExtlFn fn, bool *doneseen, bool *eagain) {
  char buf[BL];
  int n;
...
static bool wait_statusd_init(int outfd, int errfd, ExtlFn dh, ExtlFn eh) {
  ...
  while (1) {
    ...
    if (now.tv_sec > endtime.tv_sec) {
      ...
      if (now.tv_usec >= endtime.tv_usec) goto timeout;
      tv.tv_sec = 0;
    ...
    if (retval > 0) {
      if (FD_ISSET(errfd, &rfds)) {
        if (!process_pipe(errfd, eh, &dummy, &eagain)) return FALSE;
      }
      if (FD_ISSET(outfd, &rfds)) {
        if (!process_pipe(outfd, dh, &doneseen, &eagain)) return FALSE;
        if (doneseen) {
          /* Read rest of errors. */
          bool ok;
          do {
...
EXTL_EXPORT
int mod_statusbar__launch_statusd(const char *cmd, ExtlFn initdatahandler,
                                  ExtlFn initerrhandler, ExtlFn datahandler,
                                  ExtlFn errhandler) {
  pid_t pid;
  int outfd = -1, errfd = -1;
  ...
  pid = mainloop_do_spawn(cmd, NULL, NULL, NULL, &outfd, &errfd);
  ...
  if (!mainloop_register_input_fd_extlfn(outfd, datahandler)) goto err;

  if (!mainloop_register_input_fd_extlfn(errfd, errhandler)) goto err2;
