...
static bool extl_table_getis(ExtlTab tab, int i, const char *s, char c,
  ...
  if (!extl_table_geti_t(tab, i, &sub)) return FALSE;
...
static void menu_draw_entry(WMenu *menu, int i, const WRectangle *igeom,
  ...
  if (menu->entry_brush == NULL) return;
...
void menu_draw(WMenu *menu, bool complete) {
  ...
  if (menu->brush == NULL) return;

  get_outer_geom(menu, &geom);

  grbrush_begin(menu->brush, &geom, (complete ? 0 : GRBRUSH_NO_CLEAR_OK));
...
static void menu_calc_size(WMenu *menu, bool maxexact, int maxw, int maxh,
                           int *w_ret, int *h_ret) {
  ...
  for (i = 0; i < menu->n_entries; i++) {
    ...
    if (extl_table_getis(menu->tab, i + 1, "name", 's', &str)) {
      menu->entries[i].title =
          grbrush_make_label(menu->entry_brush, str, maxew);
      free(str);
    }
  }
}
...
static void menu_firstfit(WMenu *menu, bool submenu, const WRectangle *refg) {
  ...
  if (!(menu->last_fp.mode & REGION_FIT_BOUNDS)) {
    ...
    if (!submenu) {
      const WRectangle *maxg = &REGION_GEOM(REGION_PARENT((WRegion *)menu));
    ...
    if (submenu) {
      ...
      if (geom.x + geom.w > maxg->x + maxg->w) geom.x = maxg->x;

      geom.y = MINOF(b - geom.h, t);
      if (geom.y < maxg->y) geom.y = maxg->y;
    } else {
      geom.x = maxg->x;
...
static void menu_do_refit(WMenu *menu, WWindow *par, const WFitParams *oldfp) {
  ...
  if (!(menu->last_fp.mode & REGION_FIT_BOUNDS)) {
    ...
    int ydiff =
        (REGION_GEOM(menu).y + REGION_GEOM(menu).h - (oldfp->g.y + oldfp->g.h));
    geom.x = MAXOF(0, MINOF(maxg->x + xdiff, maxg->x + maxg->w - geom.w));
    geom.y =
        MAXOF(0, MINOF(maxg->y + maxg->h + ydiff, maxg->y + maxg->h) - geom.h);
  }
...
static void calc_entry_dimens(WMenu *menu) {
  ...
  for (i = 1; i <= n; i++) {
    if (extl_table_getis(menu->tab, i, "name", 's', &str)) {
      int w = grbrush_get_text_width(menu->entry_brush, str, strlen(str));
      if (w > maxw) maxw = w;
      free(str);
    }
  }
...
static bool menu_init_gr(WMenu *menu, WRootWin *rootwin, Window win) {
  ...
  const char *style =
      (menu->big_mode
           ? "input-menu-big"
           : (menu->pmenu_mode ? "input-menu-pmenu" : "input-menu-normal"));

  const char *entry_style =
      (menu->big_mode ? "tab-menuentry-big"
                      : (menu->pmenu_mode ? "tab-menuentry-pmenu"
  ...
  if (menu->entry_brush != NULL) grbrush_release(menu->entry_brush);
  if (menu->brush != NULL) grbrush_release(menu->brush);
...
void menu_updategr(WMenu *menu) {
  if (!menu_init_gr(menu, region_rootwin_of((WRegion *)menu), MENU_WIN(menu))) {
    return;
  }
...
bool menu_init(WMenu *menu, WWindow *par, const WFitParams *fp,
  ...
  if (params->pmenu_mode) {
    ...
    if (menu->selected_entry < 0) menu->selected_entry = 0;
    if (params->initial > menu->n_entries) menu->selected_entry = 0;
  }
  ...
  if (!window_init((WWindow *)menu, par, fp, "WMenu")) goto fail;

  win = menu->win.win;

  if (!menu_init_gr(menu, region_rootwin_of((WRegion *)par), win)) goto fail2;
...
static void deinit_entries(WMenu *menu) {
  ...
  for (i = 0; i < menu->n_entries; i++) {
    ...
    if (menu->entries[i].title != NULL) free(menu->entries[i].title);
  }
...
void menu_deinit(WMenu *menu) {
  ...
  if (menu->submenu != NULL) destroy_obj((Obj *)menu->submenu);
...
static void menu_activated(WMenu *menu) { window_draw((WWindow *)menu, FALSE); }
...
static void menu_managed_remove(WMenu *menu, WRegion *sub) {
  ...
  if (sub != (WRegion *)menu->submenu) return;

  menu->submenu = NULL;

  region_unset_manager(sub, (WRegion *)menu);

  if (mcf) region_do_set_focus((WRegion *)menu, FALSE);
}
...
static void show_sub(WMenu *menu, int n) {
  ...
  fnp.tab = extl_table_none();
  {
    ...
    if (fnp.tab == extl_table_none()) return;
  }
...
void menu_stacking(WMenu *menu, Window *bottomret, Window *topret) {
  ...
  *bottomret = MENU_WIN(menu);
  if (*topret == None) *topret = MENU_WIN(menu);
}
...
static void menu_do_select_nth(WMenu *menu, int n) {
  ...
  if (n >= 0) {
    ...
    if (menu->entries[n].flags & WMENUENTRY_SUBMENU && menu->pmenu_mode) {
      show_sub(menu, n);
    }
  }
  ...
  if (drawfull) {
    ...
    /* !!!BEGIN!!! */
    if (oldn != -1) menu_draw_entry(menu, oldn, &igeom, TRUE);
...
void menu_select_nth(WMenu *menu, int n) {
  if (n < 0) n = 0;
  if (n >= menu->n_entries) n = menu->n_entries - 1;
...
void menu_select_prev(WMenu *menu) {
  menu_select_nth(menu, (menu->selected_entry <= 0 ? menu->n_entries - 1
                                                   : menu->selected_entry - 1));
}
...
static void menu_do_finish(WMenu *menu) {
  ...
  if (!region_rqdispose((WRegion *)head)) {
    if (head->submenu != NULL) destroy_obj((Obj *)head->submenu);
  }

  if (ok) extl_call(handler, "t", NULL, tab);
 ...
 */
EXTL_EXPORT_MEMBER
void menu_cancel(WMenu *menu) { region_defer_rqdispose((WRegion *)menu); }
...
void mod_menu_set(ExtlTab tab) {
  ...
  if (extl_table_gets_i(tab, "scroll_amount", &a)) scroll_amount = MAXOF(0, a);
  if (extl_table_gets_i(tab, "scroll_delay", &t)) scroll_time = MAXOF(0, t);
}
...
enum { D_LEFT, D_RIGHT, D_DOWN, D_UP };
...
static void scroll_left(WTimer *timer, WMenu *menu) {
  if (menu != NULL) {
    ...
    if (scrolld_subs(menu, D_LEFT) > 0) {
      timer_set(timer, scroll_time, (WTimerHandler *)scroll_left, (Obj *)menu);
      return;
    }
  }
}
...
static void scroll_up(WTimer *timer, WMenu *menu) {
  if (menu != NULL) {
    ...
    if (scrolld_subs(menu, D_UP) > 0) {
      timer_set(timer, scroll_time, (WTimerHandler *)scroll_up, (Obj *)menu);
...
static void scroll_right(WTimer *timer, WMenu *menu) {
  if (menu != NULL) {
    ...
    if (scrolld_subs(menu, D_RIGHT) > 0) {
      timer_set(timer, scroll_time, (WTimerHandler *)scroll_right, (Obj *)menu);
      return;
    }
  }
}
...
static void scroll_down(WTimer *timer, WMenu *menu) {
  if (menu != NULL) {
    ...
    if (scrolld_subs(menu, D_DOWN) > 0) {
      timer_set(timer, scroll_time, (WTimerHandler *)scroll_down, (Obj *)menu);
      return;
    }
  }
}

static void end_scroll(WMenu *menu) { reset_scroll_timer(); }
...
int menu_entry_at_root(WMenu *menu, int root_x, int root_y) {
  ...
  if (x < 0 || x >= ig.w || y < 0 || y >= ig.h) return -1;

  entry = y / (menu->entry_h + menu->entry_spacing);
  if (entry < 0 || entry >= menu->vis_entries) return -1;
...
int menu_entry_at_root_tree(WMenu *menu, int root_x, int root_y,
  ...
  *realmenu = menu;
  ...
  if (!menu->pmenu_mode) return menu_entry_at_root(menu, root_x, root_y);
...
static void menu_select_entry_at(WMenu *menu, int px, int py) {
  ...
  if (entry >= 0) menu_do_select_nth(menu, entry);
}
...
void menu_motion(WMenu *menu, XMotionEvent *ev, int UNUSED(dx),
                 int UNUSED(dy)) {
...
void menu_button(WMenu *menu, XButtonEvent *ev) {
  int entry = menu_entry_at_root_tree(menu, ev->x_root, ev->y_root, &menu);
  if (entry >= 0) menu_select_nth(menu, entry);
}

int menu_press(WMenu *menu, XButtonEvent *ev, WRegion **UNUSED(reg_ret)) {
  menu_button(menu, ev);
  menu = menu_head(menu);
  ioncore_set_drag_handlers((WRegion *)menu, NULL,
                            (WMotionHandler *)menu_motion,
                            (WButtonHandler *)menu_release, NULL, NULL);
  return 0;
}
...
static void menu_insstr(WMenu *menu, const char *buf, size_t n) {
  ...
  if (oldlen != 0) memcpy(newta, menu->typeahead, oldlen);
  if (n != 0) memcpy(newta + oldlen, buf, n);
  newta[oldlen + n] = '\0';
  newta_orig = newta;
  ...
  if (menu->typeahead != NULL) free(menu->typeahead);
