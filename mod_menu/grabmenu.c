...
static void grabkilled_handler(WRegion *reg) { destroy_obj((Obj *)reg); }
...
WMenu *mod_menu_do_grabmenu(WMPlex *mplex, ExtlFn handler, ExtlTab tab,
  ...
  if (!ioncore_current_key(&kcb, &state, &sub)) return NULL;
  ...
  menu = (WMenu *)mplex_do_attach_new(
  ...
  ioncore_grab_establish((WRegion *)menu, grabmenu_handler, grabkilled_handler,
                         0);
